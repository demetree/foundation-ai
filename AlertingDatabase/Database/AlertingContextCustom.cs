...
/// <summry>
/// 
/// This is an extension class to the Alerting Context that is separated from the main AlertingContext.cs so that it won't be clobbered if the EF Core Power tools
...
public partial class AlertingContext : DbContext
{
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        if (!optionsBuilder.IsConfigured)
        {
            ...
            var connectionString = configuration.GetConnectionString("Alerting");
    ...
    public AlertingContext(ILogger logger, LogLevel level)
        : base(CreateOptionsWithDebugLogging(logger, level))
    {
    }


    private static DbContextOptions<AlertingContext> CreateOptionsWithDebugLogging(ILogger logger, LogLevel level)
    {
        var optionsBuilder = new DbContextOptionsBuilder<AlertingContext>();
