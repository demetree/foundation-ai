...
-- ALTER INDEX ALL ON `CompiledGlb` DISABLE
...
-- ALTER INDEX ALL ON `CompiledGlb` REBUILD
...
CREATE TABLE `ConnectorType`(
	...
	`minAngleDegrees` FLOAT NULL,		-- Minimum angle of rotation for hinge-type connectors (null = no minimum)
	`maxAngleDegrees` FLOAT NULL,		-- Maximum angle of rotation for hinge-type connectors (null = no maximum)
	`snapIncrementDegrees` FLOAT NULL,		-- Snap increment for click hinges (e.g. 22.5 degrees, null = continuous)
	`clutchForceNewtons` FLOAT NULL,		-- Force needed to disconnect this connector type (null = unknown)
...
CREATE TABLE `ConnectorTypeCompatibility`(
	...
	UNIQUE `UC_ConnectorTypeCompatibility_maleConnectorTypeId_femaleConnectorTypeId_Unique`( `maleConnectorTypeId`, `femaleConnectorTypeId` ) 		-- Uniqueness enforced on the ConnectorTypeCompatibility table's maleConnectorTypeId and femaleConnectorTypeId fields.
);
...
CREATE TABLE `BrickColour`(
	...
	UNIQUE `UC_BrickColour_rebrickableColorId_Unique`( `rebrickableColorId` ) 		-- Uniqueness enforced on the BrickColour table's rebrickableColorId field.
);
...
CREATE TABLE `BrickPart`(
	...
	`momentOfInertiaX` FLOAT NULL,		-- Rotational inertia about X axis (kg·m², null if unknown)
	`momentOfInertiaY` FLOAT NULL,		-- Rotational inertia about Y axis (kg·m², null if unknown)
	`momentOfInertiaZ` FLOAT NULL,		-- Rotational inertia about Z axis (kg·m², null if unknown)
	`frictionCoefficient` FLOAT NULL,		-- Surface friction coefficient for physics (null if unknown)
	`materialType` VARCHAR(50) NULL,		-- Material type: ABS, Rubber, Metal, Fabric, etc. (null if unknown)
	`centerOfMassX` FLOAT NULL,		-- Center of mass X offset from part origin in LDU (null if unknown)
	`centerOfMassY` FLOAT NULL,		-- Center of mass Y offset from part origin in LDU (null if unknown)
	`centerOfMassZ` FLOAT NULL,		-- Center of mass Z offset from part origin in LDU (null if unknown)
	...
	`boundingBoxMinX` FLOAT NULL,		-- Axis-aligned bounding box minimum X in LDU
	`boundingBoxMinY` FLOAT NULL,		-- Axis-aligned bounding box minimum Y in LDU
	`boundingBoxMinZ` FLOAT NULL,		-- Axis-aligned bounding box minimum Z in LDU
	`boundingBoxMaxX` FLOAT NULL,		-- Axis-aligned bounding box maximum X in LDU
	`boundingBoxMaxY` FLOAT NULL,		-- Axis-aligned bounding box maximum Y in LDU
	`boundingBoxMaxZ` FLOAT NULL,		-- Axis-aligned bounding box maximum Z in LDU
	...
	UNIQUE `UC_BrickPart_rebrickablePartNum_Unique`( `rebrickablePartNum` ) 		-- Uniqueness enforced on the BrickPart table's rebrickablePartNum field.
);
...
CREATE TABLE `BrickConnection`(
	...
	`angleDegrees` FLOAT NULL,		-- Current angle for rotational connections (null = default/fixed)
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`projectId`) REFERENCES `Project`(`id`),		-- Foreign key to the Project table.
...
CREATE TABLE `SubmodelInstance`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`submodelId`) REFERENCES `Submodel`(`id`),		-- Foreign key to the Submodel table.
	FOREIGN KEY (`parentSubmodelId`) REFERENCES `Submodel`(`id`)		-- Foreign key to the Submodel table.
);
...
CREATE INDEX `I_SubmodelInstance_tenantGuid_parentSubmodelId` ON `SubmodelInstance` (`tenantGuid`, `parentSubmodelId`);
...
CREATE TABLE `ModelSubFile`(
	...
	UNIQUE `UC_ModelSubFile_tenantGuid_modelDocumentId_fileName_Unique`( `tenantGuid`, `modelDocumentId`, `fileName` ) 		-- Uniqueness enforced on the ModelSubFile table's tenantGuid and modelDocumentId and fileName fields.
);
...
CREATE TABLE `ModelBuildStep`(
	...
	`rotationX` FLOAT NULL,		-- ROTSTEP X rotation angle in degrees
	`rotationY` FLOAT NULL,		-- ROTSTEP Y rotation angle in degrees
	`rotationZ` FLOAT NULL,		-- ROTSTEP Z rotation angle in degrees
...
CREATE TABLE `BrickElement`(
	...
	FOREIGN KEY (`brickPartId`) REFERENCES `BrickPart`(`id`),		-- Foreign key to the BrickPart table.
	FOREIGN KEY (`brickColourId`) REFERENCES `BrickColour`(`id`),		-- Foreign key to the BrickColour table.
	UNIQUE `UC_BrickElement_elementId_Unique`( `elementId` ) 		-- Uniqueness enforced on the BrickElement table's elementId field.
);
...
CREATE TABLE `LegoMinifig`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_LegoMinifig_figNumber_Unique`( `figNumber` ) 		-- Uniqueness enforced on the LegoMinifig table's figNumber field.
);
...
CREATE TABLE `RebrickableUserLink`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_RebrickableUserLink_tenantGuid_Unique`( `tenantGuid` ) 		-- Uniqueness enforced on the RebrickableUserLink table's tenantGuid field.
);
...
CREATE TABLE `UserPartList`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_UserPartList_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the UserPartList table's tenantGuid and name fields.
);
...
CREATE TABLE `UserPartListItem`(
	...
	FOREIGN KEY (`brickPartId`) REFERENCES `BrickPart`(`id`),		-- Foreign key to the BrickPart table.
	FOREIGN KEY (`brickColourId`) REFERENCES `BrickColour`(`id`),		-- Foreign key to the BrickColour table.
	UNIQUE `UC_UserPartListItem_tenantGuid_userPartListId_brickPartId_brickColourId_Unique`( `tenantGuid`, `userPartListId`, `brickPartId`, `brickColourId` ) 		-- Uniqueness enforced on the UserPartListItem table's tenantGuid and userPartListId and brickPartId and brickColourId fields.
);
...
CREATE TABLE `UserSetList`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_UserSetList_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the UserSetList table's tenantGuid and name fields.
);
...
CREATE TABLE `UserSetListItem`(
	...
	FOREIGN KEY (`userSetListId`) REFERENCES `UserSetList`(`id`),		-- Foreign key to the UserSetList table.
	FOREIGN KEY (`legoSetId`) REFERENCES `LegoSet`(`id`),		-- Foreign key to the LegoSet table.
	UNIQUE `UC_UserSetListItem_tenantGuid_userSetListId_legoSetId_Unique`( `tenantGuid`, `userSetListId`, `legoSetId` ) 		-- Uniqueness enforced on the UserSetListItem table's tenantGuid and userSetListId and legoSetId fields.
);
...
CREATE TABLE `BuildManualPage`(
	...
	`backgroundTheme` VARCHAR(50) NULL,		-- Background theme for the page (e.g. Blueprint, Clean White, Dark Mode)
	`layoutPreset` VARCHAR(50) NULL,		-- Layout preset (e.g. SingleStep, Grid, TwoColumn)
	`backgroundColorHex` VARCHAR(10) NULL,		-- Custom background colour hex from LPub3D PAGE BACKGROUND COLOR (e.g. #FFFFFF)
	`versionNumber` INT NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
CREATE TABLE `BuildManualPageChangeHistory`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`buildManualPageId` INT NOT NULL,		-- Link to the BuildManualPage table.
	`versionNumber` INT NOT NULL,		-- This is the version number that is being historized.
	`timeStamp` DATETIME NOT NULL,		-- The time that the record version was created.
	`userId` INT NOT NULL,
	`data` TEXT NOT NULL,		-- This stores the JSON representing the object's historical state.
...
CREATE TABLE `BuildManualStep`(
	...
	`renderImagePath` TEXT NULL,		-- Base64 data URI of the generated render image for this step
	`pliImagePath` TEXT NULL,		-- Base64 data URI of the generated Parts List Indicator (PLI) image for this step
...
CREATE TABLE `BuildManualStepChangeHistory`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`buildManualStepId` INT NOT NULL,		-- Link to the BuildManualStep table.
	`versionNumber` INT NOT NULL,		-- This is the version number that is being historized.
	`timeStamp` DATETIME NOT NULL,		-- The time that the record version was created.
	`userId` INT NOT NULL,
	`data` TEXT NOT NULL,		-- This stores the JSON representing the object's historical state.
...
CREATE TABLE `BuildStepPartChangeHistory`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`buildStepPartId` INT NOT NULL,		-- Link to the BuildStepPart table.
	`versionNumber` INT NOT NULL,		-- This is the version number that is being historized.
	`timeStamp` DATETIME NOT NULL,		-- The time that the record version was created.
	`userId` INT NOT NULL,
	`data` TEXT NOT NULL,		-- This stores the JSON representing the object's historical state.
...
CREATE TABLE `BuildStepAnnotationChangeHistory`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`buildStepAnnotationId` INT NOT NULL,		-- Link to the BuildStepAnnotation table.
	`versionNumber` INT NOT NULL,		-- This is the version number that is being historized.
	`timeStamp` DATETIME NOT NULL,		-- The time that the record version was created.
	`userId` INT NOT NULL,
	`data` TEXT NOT NULL,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX `I_BuildStepAnnotationChangeHistory_tenantGuid_buildStepAnnotatio` ON `BuildStepAnnotationChangeHistory` (`tenantGuid`, `buildStepAnnotationId`, `versionNumber`, `timeStamp`, `userId`);
...
CREATE TABLE `UserProfilePreferredTheme`(
	...
	FOREIGN KEY (`userProfileId`) REFERENCES `UserProfile`(`id`),		-- Foreign key to the UserProfile table.
	FOREIGN KEY (`legoThemeId`) REFERENCES `LegoTheme`(`id`),		-- Foreign key to the LegoTheme table.
	UNIQUE `UC_UserProfilePreferredTheme_tenantGuid_userProfileId_legoThemeId_Unique`( `tenantGuid`, `userProfileId`, `legoThemeId` ) 		-- Uniqueness enforced on the UserProfilePreferredTheme table's tenantGuid and userProfileId and legoThemeId fields.
);
...
CREATE TABLE `UserSetOwnership`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`legoSetId`) REFERENCES `LegoSet`(`id`),		-- Foreign key to the LegoSet table.
	UNIQUE `UC_UserSetOwnership_tenantGuid_legoSetId_Unique`( `tenantGuid`, `legoSetId` ) 		-- Uniqueness enforced on the UserSetOwnership table's tenantGuid and legoSetId fields.
);
...
CREATE TABLE `UserFollow`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`followerTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user who is following
	`followedTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user being followed
	`followedDate` DATETIME NOT NULL,		-- Date/time the follow relationship was created
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_UserFollow_followerTenantGuid_followedTenantGuid_Unique`( `followerTenantGuid`, `followedTenantGuid` ) 		-- Uniqueness enforced on the UserFollow table's followerTenantGuid and followedTenantGuid fields.
);
...
INSERT INTO `ActivityEventType` ( `name`, `description`, `iconCssClass`, `sequence`, `objectGuid` ) VALUES  ( 'CommittedVersion', 'User committed a new version of a MOC', 'fas fa-code-commit', 8, 'ae100001-0001-4000-8000-000000000008' );

INSERT INTO `ActivityEventType` ( `name`, `description`, `iconCssClass`, `sequence`, `objectGuid` ) VALUES  ( 'ForkedMoc', 'User forked another builder''s MOC', 'fas fa-code-branch', 9, 'ae100001-0001-4000-8000-000000000009' );

INSERT INTO `ActivityEventType` ( `name`, `description`, `iconCssClass`, `sequence`, `objectGuid` ) VALUES  ( 'AddedCollaborator', 'User invited a collaborator to a MOC', 'fas fa-user-friends', 10, 'ae100001-0001-4000-8000-000000000010' );
...
CREATE TABLE `PublishedMoc`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`projectId`) REFERENCES `Project`(`id`),		-- Foreign key to the Project table.
	FOREIGN KEY (`forkedFromMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_PublishedMoc_tenantGuid_name_Unique`( `tenantGuid`, `name` ) ,		-- Uniqueness enforced on the PublishedMoc table's tenantGuid and name fields.
	UNIQUE `UC_PublishedMoc_tenantGuid_slug_Unique`( `tenantGuid`, `slug` ) 		-- Uniqueness enforced on the PublishedMoc table's tenantGuid and slug fields.
);
...
CREATE INDEX `I_PublishedMoc_tenantGuid_forkedFromMocId` ON `PublishedMoc` (`tenantGuid`, `forkedFromMocId`);
...
CREATE TABLE `MocVersion`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_MocVersion_tenantGuid_publishedMocId_versionNumber_Unique`( `tenantGuid`, `publishedMocId`, `versionNumber` ) 		-- Uniqueness enforced on the MocVersion table's tenantGuid and publishedMocId and versionNumber fields.
);
-- Index on the MocVersion table's tenantGuid field.
CREATE INDEX `I_MocVersion_tenantGuid` ON `MocVersion` (`tenantGuid`);

-- Index on the MocVersion table's tenantGuid,publishedMocId fields.
CREATE INDEX `I_MocVersion_tenantGuid_publishedMocId` ON `MocVersion` (`tenantGuid`, `publishedMocId`);

-- Index on the MocVersion table's tenantGuid,active fields.
CREATE INDEX `I_MocVersion_tenantGuid_active` ON `MocVersion` (`tenantGuid`, `active`);
...
CREATE TABLE `MocFork`(
	...
	`forkerTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user who created the fork
	`forkedDate` DATETIME NOT NULL,		-- Date/time the fork was created
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`forkedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	FOREIGN KEY (`sourceMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	FOREIGN KEY (`mocVersionId`) REFERENCES `MocVersion`(`id`),		-- Foreign key to the MocVersion table.
	UNIQUE `UC_MocFork_forkedMocId_Unique`( `forkedMocId` ) 		-- Uniqueness enforced on the MocFork table's forkedMocId field.
);
-- Index on the MocFork table's forkedMocId field.
CREATE INDEX `I_MocFork_forkedMocId` ON `MocFork` (`forkedMocId`);

-- Index on the MocFork table's sourceMocId field.
CREATE INDEX `I_MocFork_sourceMocId` ON `MocFork` (`sourceMocId`);

-- Index on the MocFork table's mocVersionId field.
CREATE INDEX `I_MocFork_mocVersionId` ON `MocFork` (`mocVersionId`);

-- Index on the MocFork table's active field.
CREATE INDEX `I_MocFork_active` ON `MocFork` (`active`);

-- Index on the MocFork table's deleted field.
CREATE INDEX `I_MocFork_deleted` ON `MocFork` (`deleted`);
...
CREATE TABLE `MocCollaborator`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`publishedMocId` INT NOT NULL,		-- The MOC being shared
	`collaboratorTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the collaborator being granted access
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_MocCollaborator_tenantGuid_publishedMocId_collaboratorTenantGuid_Unique`( `tenantGuid`, `publishedMocId`, `collaboratorTenantGuid` ) 		-- Uniqueness enforced on the MocCollaborator table's tenantGuid and publishedMocId and collaboratorTenantGuid fields.
);
-- Index on the MocCollaborator table's tenantGuid field.
CREATE INDEX `I_MocCollaborator_tenantGuid` ON `MocCollaborator` (`tenantGuid`);

-- Index on the MocCollaborator table's tenantGuid,publishedMocId fields.
CREATE INDEX `I_MocCollaborator_tenantGuid_publishedMocId` ON `MocCollaborator` (`tenantGuid`, `publishedMocId`);

-- Index on the MocCollaborator table's tenantGuid,active fields.
CREATE INDEX `I_MocCollaborator_tenantGuid_active` ON `MocCollaborator` (`tenantGuid`, `active`);
...
CREATE TABLE `MocLike`(
	...
	`likerTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user who liked
	`likedDate` DATETIME NOT NULL,		-- Date/time the like was registered
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_MocLike_publishedMocId_likerTenantGuid_Unique`( `publishedMocId`, `likerTenantGuid` ) 		-- Uniqueness enforced on the MocLike table's publishedMocId and likerTenantGuid fields.
);
...
CREATE TABLE `MocComment`(
	...
	`commenterTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user who posted the comment
...
CREATE TABLE `MocFavourite`(
	...
	`userTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user who favourited
	`favouritedDate` DATETIME NOT NULL,		-- Date/time the favourite was added
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_MocFavourite_publishedMocId_userTenantGuid_Unique`( `publishedMocId`, `userTenantGuid` ) 		-- Uniqueness enforced on the MocFavourite table's publishedMocId and userTenantGuid fields.
);
...
CREATE TABLE `SharedInstruction`(
	...
	FOREIGN KEY (`buildManualId`) REFERENCES `BuildManual`(`id`),		-- Foreign key to the BuildManual table.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_SharedInstruction_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the SharedInstruction table's tenantGuid and name fields.
);
...
CREATE INDEX `I_SharedInstructionChangeHistory_tenantGuid_sharedInstructionId` ON `SharedInstructionChangeHistory` (`tenantGuid`, `sharedInstructionId`, `versionNumber`, `timeStamp`, `userId`);
...
CREATE TABLE `UserAchievement`(
	...
	UNIQUE `UC_UserAchievement_tenantGuid_achievementId_Unique`( `tenantGuid`, `achievementId` ) 		-- Uniqueness enforced on the UserAchievement table's tenantGuid and achievementId fields.
);
...
CREATE TABLE `UserBadgeAssignment`(
	...
	`awardedByTenantGuid` CHAR(38) NULL,		-- Tenant GUID of the moderator who awarded the badge (null for automatic badges)
	...
	UNIQUE `UC_UserBadgeAssignment_tenantGuid_userBadgeId_Unique`( `tenantGuid`, `userBadgeId` ) 		-- Uniqueness enforced on the UserBadgeAssignment table's tenantGuid and userBadgeId fields.
);
...
CREATE TABLE `BuildChallengeEntry`(
	...
	FOREIGN KEY (`buildChallengeId`) REFERENCES `BuildChallenge`(`id`),		-- Foreign key to the BuildChallenge table.
	FOREIGN KEY (`publishedMocId`) REFERENCES `PublishedMoc`(`id`),		-- Foreign key to the PublishedMoc table.
	UNIQUE `UC_BuildChallengeEntry_tenantGuid_buildChallengeId_Unique`( `tenantGuid`, `buildChallengeId` ) 		-- Uniqueness enforced on the BuildChallengeEntry table's tenantGuid and buildChallengeId fields.
);
...
CREATE TABLE `ContentReport`(
	...
	`reporterTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the user submitting the report
	...
	`reviewerTenantGuid` CHAR(38) NULL,		-- Tenant GUID of the moderator who reviewed (null if pending)
...
CREATE TABLE `ModerationAction`(
	`id` BIGINT PRIMARY KEY NOT NULL,
	`moderatorTenantGuid` CHAR(38) NOT NULL,		-- Tenant GUID of the moderator who took the action
	...
	`targetTenantGuid` CHAR(38) NULL,		-- Tenant GUID of the user the action was taken against (null for content-only actions)
...
CREATE TABLE `ApiKey`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_ApiKey_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the ApiKey table's tenantGuid and name fields.
);
...
CREATE TABLE `CompiledGlb`(
	...
	`glbData` BLOB NULL,		-- The compiled GLB binary data
	...
	`compiledAt` DATETIME NOT NULL,		-- UTC timestamp when this GLB was compiled
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`projectId`) REFERENCES `Project`(`id`)		-- Foreign key to the Project table.
...
CREATE INDEX `I_CompiledGlb_tenantGuid` ON `CompiledGlb` (`tenantGuid`);

-- Index on the CompiledGlb table's tenantGuid,projectId fields.
CREATE INDEX `I_CompiledGlb_tenantGuid_projectId` ON `CompiledGlb` (`tenantGuid`, `projectId`);

-- Index on the CompiledGlb table's tenantGuid,active fields.
CREATE INDEX `I_CompiledGlb_tenantGuid_active` ON `CompiledGlb` (`tenantGuid`, `active`);

-- Index on the CompiledGlb table's tenantGuid,deleted fields.
CREATE INDEX `I_CompiledGlb_tenantGuid_deleted` ON `CompiledGlb` (`tenantGuid`, `deleted`);
