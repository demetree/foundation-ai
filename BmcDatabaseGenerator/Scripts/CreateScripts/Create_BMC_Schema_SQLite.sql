...
(
	...
	"parentSubmodelId" INTEGER NULL,		-- The parent submodel this instance is placed WITHIN (null = placed directly in the main model)
...
(
	...
	"visibility" VARCHAR(50) NOT NULL DEFAULT 'Public' COLLATE NOCASE,		-- MOCHub visibility level: Public, Private, Unlisted. Public = visible to everyone; Private = only owner and collaborators; Unlisted = accessible via direct link only
	"forkCount" INTEGER NOT NULL DEFAULT 0,		-- Cached count of forks created from this MOC
	"forkedFromMocId" INTEGER NULL,		-- If this MOC is a fork, the source MOC it was forked from (null = original work)
	"licenseName" VARCHAR(100) NULL COLLATE NOCASE,		-- Licence type for the MOC: CC-BY, CC-BY-SA, CC-BY-NC, CC-BY-NC-SA, CC0, AllRightsReserved (null = All Rights Reserved)
	"readmeMarkdown" TEXT NULL COLLATE NOCASE,		-- GitHub-style README displayed on the MOC repository page (supports Markdown formatting)
	"slug" VARCHAR(100) NULL COLLATE NOCASE,		-- URL-friendly slug for the MOC (e.g. 'technic-mobile-crane'). Unique per tenant.
	"defaultBranchName" VARCHAR(100) NULL COLLATE NOCASE,		-- Reserved for future branching support (default: 'main')
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	UNIQUE ( "tenantGuid", "slug") 		-- Uniqueness enforced on the PublishedMoc table's tenantGuid and slug fields.
);
...
(
	...
	"addedPartCount" INTEGER NULL,		-- Number of parts added since the previous version (null for first version)
	"removedPartCount" INTEGER NULL,		-- Number of parts removed since the previous version (null for first version)
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("publishedMocId") REFERENCES "PublishedMoc"("id"),		-- Foreign key to the PublishedMoc table.
	UNIQUE ( "tenantGuid", "publishedMocId", "versionNumber") 		-- Uniqueness enforced on the MocVersion table's tenantGuid and publishedMocId and versionNumber fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"forkedMocId" INTEGER NOT NULL,		-- The fork (child MOC) that was created
	"sourceMocId" INTEGER NOT NULL,		-- The original (parent MOC) that was forked from
	"mocVersionId" INTEGER NULL,		-- The specific version the fork was created from (null if unknown or forked from latest)
	"forkerTenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Tenant GUID of the user who created the fork
	...
	FOREIGN KEY ("mocVersionId") REFERENCES "MocVersion"("id"),		-- Foreign key to the MocVersion table.
	UNIQUE ( "forkedMocId") 		-- Uniqueness enforced on the MocFork table's forkedMocId field.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"publishedMocId" INTEGER NOT NULL,		-- The MOC being shared
	"collaboratorTenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Tenant GUID of the collaborator being granted access
	"accessLevel" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Access level granted: Read, Write, Admin
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("publishedMocId") REFERENCES "PublishedMoc"("id"),		-- Foreign key to the PublishedMoc table.
	UNIQUE ( "tenantGuid", "publishedMocId", "collaboratorTenantGuid") 		-- Uniqueness enforced on the MocCollaborator table's tenantGuid and publishedMocId and collaboratorTenantGuid fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"projectId" INTEGER NOT NULL,		-- The project this compiled GLB belongs to
	"projectVersionNumber" INTEGER NOT NULL,		-- Invalidation key — matches Project.versionNumber at compilation time. Stale when project version advances.
	...
	"triangleCount" INTEGER NULL,		-- Total triangle count in the compiled mesh
	"stepCount" INTEGER NULL,		-- Number of build steps encoded in the GLB
