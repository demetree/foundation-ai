...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"name" VARCHAR(100) NOT NULL UNIQUE COLLATE NOCASE,
	"fontAwesomeCode" VARCHAR(50) NULL COLLATE NOCASE,
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0		-- Soft deletion flag.
...
(
	...
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ResourceType table's tenantGuid and name fields.
);
...
(
	...
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"iconId" INTEGER NULL,		-- Link to the Icon table.
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Priority table's tenantGuid and name fields.
);
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the RateType table's tenantGuid and name fields.
);
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") ,		-- Uniqueness enforced on the Currency table's tenantGuid and name fields.
	UNIQUE ( "tenantGuid", "code") 		-- Uniqueness enforced on the Currency table's tenantGuid and code fields.
);
...
(
	...
	"externalMapping" VARCHAR(100) NULL COLLATE NOCASE,		-- Maps to the account type in external systems (e.g., QuickBooks account type name).
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"code" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Short code for the office (e.g., 'REC', 'ADMIN', 'FIRE').
	"contactName" VARCHAR(250) NULL COLLATE NOCASE,		-- Accountant or financial contact name for this office.
	"contactEmail" VARCHAR(250) NULL COLLATE NOCASE,		-- Accountant or financial contact email for export delivery.
	"exportFormat" VARCHAR(50) NULL DEFAULT 'CSV' COLLATE NOCASE,		-- Preferred export format: CSV, QuickBooks, Xero, etc.
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") ,		-- Uniqueness enforced on the FinancialOffice table's tenantGuid and name fields.
	UNIQUE ( "tenantGuid", "code") 		-- Uniqueness enforced on the FinancialOffice table's tenantGuid and code fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"financialOfficeId" INTEGER NOT NULL,		-- Link to the FinancialOffice table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"code" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Short code for the category (e.g., '12' for Kids Rental, '40' for Easter Brunch Supplies).
	"accountTypeId" INTEGER NOT NULL,		-- Link to AccountType — standard accounting classification (Income, Expense, COGS, Asset, Liability, Equity). Replaces the old accountType string field.
	"financialOfficeId" INTEGER NULL,		-- Optional link to FinancialOffice — scopes this category to a specific department/committee. When null, the category is tenant-wide.
	"parentFinancialCategoryId" INTEGER NULL,		-- Optional parent for sub-categories.
	"isTaxApplicable" BIT NOT NULL DEFAULT 0,		-- Whether HST/tax typically applies to transactions in this category.
	"defaultAmount" NUMERIC NULL,		-- Optional default amount for common transactions in this category.
	"externalAccountId" VARCHAR(250) NULL COLLATE NOCASE,		-- Account ID in external system (e.g., QuickBooks account ID) for sync.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	...
	UNIQUE ( "tenantGuid", "name") ,		-- Uniqueness enforced on the FinancialCategory table's tenantGuid and name fields.
	UNIQUE ( "tenantGuid", "code") 		-- Uniqueness enforced on the FinancialCategory table's tenantGuid and code fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"financialCategoryId" INTEGER NOT NULL,		-- Link to the FinancialCategory table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_FnnclCtgryChngHstry_tnntGud_fnnclCtgryd" ON "FinancialCategoryChangeHistory" ("tenantGuid", "financialCategoryId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"code" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Short tax code identifier (e.g., 'HST', 'GST', 'EXEMPT').
	"rate" NUMERIC NOT NULL DEFAULT 0,		-- Tax rate as a percentage (e.g., 15.0 for 15% HST).
	...
	"externalTaxCodeId" VARCHAR(250) NULL COLLATE NOCASE,		-- Tax code ID in external system (e.g., QuickBooks TaxCode ID).
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") ,		-- Uniqueness enforced on the TaxCode table's tenantGuid and name fields.
	UNIQUE ( "tenantGuid", "code") 		-- Uniqueness enforced on the TaxCode table's tenantGuid and code fields.
);
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"externalId" VARCHAR(100) NULL COLLATE NOCASE,
	"isRevenue" BIT NOT NULL DEFAULT 1,		-- True = Revenue (billable), False = Expense (cost)
	"isTaxable" BIT NULL DEFAULT 0,
	"defaultAmount" NUMERIC NULL,		-- Optional default value for auto-drops
	"defaultDescription" VARCHAR(500) NULL COLLATE NOCASE,		-- sometimes auto-dropped charges need a note (e.g., "Travel to site – 45 km").
	"rateTypeId" INTEGER NULL,		-- Link to RateType (e.g., 'Standard', 'Overtime')
	"currencyId" INTEGER NOT NULL,		-- Link to the Currency table.
	"financialCategoryId" INTEGER NULL,		-- Optional bridge to the general ledger. Maps this charge type to a FinancialCategory for unified Chart of Accounts reporting across both event charges and standalone transactions.
	"taxCodeId" INTEGER NULL,		-- Optional default TaxCode for charges of this type. When set, new EventCharges auto-inherit this tax rate.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	...
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ChargeType table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"chargeTypeId" INTEGER NOT NULL,		-- Link to the ChargeType table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"priorityId" INTEGER NULL,		-- Link to the Priority table.
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	...
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Tag table's tenantGuid and name fields.
);
...
(
	...
	"name" VARCHAR(100) NOT NULL UNIQUE COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"ianaTimeZone" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- e.g., 'America/St.John's' (official IANA name)
	"abbreviation" VARCHAR(50) NOT NULL COLLATE NOCASE,
	"abbreviationDaylightSavings" VARCHAR(50) NOT NULL COLLATE NOCASE,
	...
	"sequence" INTEGER NULL,		-- For sorting in drop downs
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0		-- Soft deletion flag.
...
(
	...
	"name" VARCHAR(100) NOT NULL UNIQUE COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"abbreviation" VARCHAR(10) NOT NULL COLLATE NOCASE,
	"postalCodeFormat" VARCHAR(50) NULL COLLATE NOCASE,		-- The human readable postal code format for the country, if applicable.
	"postalCodeRegEx" VARCHAR(50) NULL COLLATE NOCASE,		-- The regular expression pattern for validation of the postal code, if applicable 
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0		-- Soft deletion flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"countryId" INTEGER NOT NULL,		-- Link to the Country table.
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"abbreviation" VARCHAR(10) NOT NULL COLLATE NOCASE,
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	...
	UNIQUE ( "name", "countryId") ,		-- Uniqueness enforced on the StateProvince table's name and countryId fields.
	UNIQUE ( "abbreviation", "countryId") 		-- Uniqueness enforced on the StateProvince table's abbreviation and countryId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactTypeId" INTEGER NOT NULL,		-- Link to the ContactType table.
	"firstName" VARCHAR(250) NOT NULL COLLATE NOCASE,
	"middleName" VARCHAR(250) NULL COLLATE NOCASE,
	"lastName" VARCHAR(250) NOT NULL COLLATE NOCASE,
	"salutationId" INTEGER NULL,		-- Link to the Salutation table.
	"title" VARCHAR(250) NULL COLLATE NOCASE,
	"birthDate" DATE NULL,
	"company" VARCHAR(250) NULL COLLATE NOCASE,
	"email" VARCHAR(250) NULL COLLATE NOCASE,
	"phone" VARCHAR(50) NULL COLLATE NOCASE,
	"mobile" VARCHAR(50) NULL COLLATE NOCASE,
	"position" VARCHAR(250) NULL COLLATE NOCASE,
	"webSite" VARCHAR(1000) NULL COLLATE NOCASE,
	...
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "email", "externalId") 		-- Uniqueness enforced on the Contact table's tenantGuid and email and externalId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactId" INTEGER NOT NULL,		-- Link to the Contact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactId" INTEGER NOT NULL,		-- Link to the Contact table.
	"tagId" INTEGER NOT NULL,		-- Link to the Tag table.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactTagId" INTEGER NOT NULL,		-- Link to the ContactTag table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactId" INTEGER NOT NULL,		-- Link to the Contact table.
	"relatedContactId" INTEGER NOT NULL,		-- Link to the Contact table.
	...
	"relationshipTypeId" INTEGER NOT NULL,		-- A description of the relationship between the contact and the contact.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	UNIQUE ( "tenantGuid", "contactId", "relatedContactId") 		-- Uniqueness enforced on the ContactContact table's tenantGuid and contactId and relatedContactId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactContactId" INTEGER NOT NULL,		-- Link to the ContactContact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"officeTypeId" INTEGER NOT NULL,		-- Link to the OfficeType table.
	"timeZoneId" INTEGER NOT NULL,		-- Time zone of the office.
	"currencyId" INTEGER NOT NULL,		-- Default billing currency of the office.
	"addressLine1" VARCHAR(250) NOT NULL COLLATE NOCASE,
	"addressLine2" VARCHAR(250) NULL COLLATE NOCASE,
	"city" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"postalCode" VARCHAR(100) NULL COLLATE NOCASE,
	"stateProvinceId" INTEGER NOT NULL,		-- Link to the StateProvince table.
	"countryId" INTEGER NOT NULL,		-- Link to the Country table.
	"phone" VARCHAR(100) NULL COLLATE NOCASE,
	"email" VARCHAR(250) NULL COLLATE NOCASE,
	...
	FOREIGN KEY ("countryId") REFERENCES "Country"("id"),		-- Foreign key to the Country table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Office table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"officeId" INTEGER NOT NULL,		-- Link to the Office table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"relationshipTypeId" INTEGER NOT NULL,		-- A description of the relationship between the office and the contact.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("contactId") REFERENCES "Contact"("id"),		-- Foreign key to the Contact table.
	FOREIGN KEY ("relationshipTypeId") REFERENCES "RelationshipType"("id"),		-- Foreign key to the RelationshipType table.
	UNIQUE ( "tenantGuid", "officeId", "contactId") 		-- Uniqueness enforced on the OfficeContact table's tenantGuid and officeId and contactId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"officeContactId" INTEGER NOT NULL,		-- Link to the OfficeContact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"officeId" INTEGER NULL,		-- Optional office binding for the calendar
	"isDefault" BIT NULL,
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	...
	FOREIGN KEY ("officeId") REFERENCES "Office"("id"),		-- Foreign key to the Office table.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Calendar table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"calendarId" INTEGER NOT NULL,		-- Link to the Calendar table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ClientType table's tenantGuid and name fields.
);
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"clientTypeId" INTEGER NOT NULL,		-- Link to the ClientType table.
	"currencyId" INTEGER NOT NULL,		-- Link to the Currency table.
	"timeZoneId" INTEGER NOT NULL,		-- Link to the TimeZone table.
	"calendarId" INTEGER NULL,		-- An optional default calendar for the scheduling target's belonging to the client.
	"addressLine1" VARCHAR(250) NOT NULL COLLATE NOCASE,
	"addressLine2" VARCHAR(250) NULL COLLATE NOCASE,
	"city" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"postalCode" VARCHAR(100) NULL COLLATE NOCASE,
	...
	FOREIGN KEY ("stateProvinceId") REFERENCES "StateProvince"("id"),		-- Foreign key to the StateProvince table.
	FOREIGN KEY ("countryId") REFERENCES "Country"("id"),		-- Foreign key to the Country table.
	UNIQUE ( "tenantGuid", "name", "externalId") 		-- Uniqueness enforced on the Client table's tenantGuid and name and externalId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"clientId" INTEGER NOT NULL,		-- Link to the Client table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"relationshipTypeId" INTEGER NOT NULL,		-- A description of the relationship between the client and the contact.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("contactId") REFERENCES "Contact"("id"),		-- Foreign key to the Contact table.
	FOREIGN KEY ("relationshipTypeId") REFERENCES "RelationshipType"("id"),		-- Foreign key to the RelationshipType table.
	UNIQUE ( "tenantGuid", "clientId", "contactId") 		-- Uniqueness enforced on the ClientContact table's tenantGuid and clientId and contactId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"clientContactId" INTEGER NOT NULL,		-- Link to the ClientContact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"companyLogoFileName" VARCHAR(250) NULL COLLATE NOCASE,		-- Part of the binary data field setup
	...
	"companyLogoMimeType" VARCHAR(100) NULL COLLATE NOCASE,		-- Part of the binary data field setup
	"addressLine1" VARCHAR(250) NULL COLLATE NOCASE,
	"addressLine2" VARCHAR(250) NULL COLLATE NOCASE,
	"addressLine3" VARCHAR(250) NULL COLLATE NOCASE,
	"city" VARCHAR(100) NULL COLLATE NOCASE,
	"postalCode" VARCHAR(100) NULL COLLATE NOCASE,
	"stateProvinceId" INTEGER NULL,		-- Link to the StateProvince table.
	"countryId" INTEGER NULL,		-- Link to the Country table.
	"timeZoneId" INTEGER NULL,		-- Link to the TimeZone table.
	"phoneNumber" VARCHAR(100) NULL COLLATE NOCASE,
	"email" VARCHAR(250) NULL COLLATE NOCASE,
	"website" VARCHAR(1000) NULL COLLATE NOCASE,
	"latitude" REAL NULL,		-- Optional latitude position
	"longitude" REAL NULL,		-- Optional longitude position
	"primaryColor" VARCHAR(10) NULL COLLATE NOCASE,
	"secondaryColor" VARCHAR(10) NULL COLLATE NOCASE,
	...
	"invoiceNumberMask" VARCHAR(100) NULL COLLATE NOCASE,		-- Format mask for auto-generating invoice numbers. Supports {YYYY} for year and {NNNN} for zero-padded sequence. Default: INV-{YYYY}-{NNNN}.
	"receiptNumberMask" VARCHAR(100) NULL COLLATE NOCASE,		-- Format mask for auto-generating receipt numbers. Supports {YYYY} for year and {NNNN} for zero-padded sequence. Default: REC-{YYYY}-{NNNN}.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("countryId") REFERENCES "Country"("id"),		-- Foreign key to the Country table.
	FOREIGN KEY ("timeZoneId") REFERENCES "TimeZone"("id"),		-- Foreign key to the TimeZone table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the TenantProfile table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"tenantProfileId" INTEGER NOT NULL,		-- Link to the TenantProfile table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Qualification table's tenantGuid and name fields.
);
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the AssignmentRole table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"assignmentRoleId" INTEGER NOT NULL,		-- Link to the AssignmentRole table.
	"qualificationId" INTEGER NOT NULL,		-- Link to the Qualification table.
	...
	UNIQUE ( "tenantGuid", "assignmentRoleId", "qualificationId") 		-- Uniqueness enforced on the AssignmentRoleQualificationRequirement table's tenantGuid and assignmentRoleId and qualificationId fields.
);
...
CREATE INDEX "I_ssgnmntRlQulfctnRqurmnt_tnntGud_ssgnmntRld" ON "AssignmentRoleQualificationRequirement" ("tenantGuid", "assignmentRoleId")
;

-- Index on the AssignmentRoleQualificationRequirement table's tenantGuid,qualificationId fields.
CREATE INDEX "I_ssgnmntRlQulfctnRqurmnt_tnntGud_qulfctnd" ON "AssignmentRoleQualificationRequirement" ("tenantGuid", "qualificationId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"assignmentRoleQualificationRequirementId" INTEGER NOT NULL,		-- Link to the AssignmentRoleQualificationRequirement table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_ssgnmntRlQulfctnRqurmntChngHstry_tnntGud" ON "AssignmentRoleQualificationRequirementChangeHistory" ("tenantGuid")
;

-- Index on the AssignmentRoleQualificationRequirementChangeHistory table's tenantGuid,versionNumber fields.
CREATE INDEX "I_ssgnmntRlQulfctnRqurmntChngHstry_tnntGud_vrsnNumbr" ON "AssignmentRoleQualificationRequirementChangeHistory" ("tenantGuid", "versionNumber")
;

-- Index on the AssignmentRoleQualificationRequirementChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX "I_ssgnmntRlQulfctnRqurmntChngHstry_tnntGud_tmStmp" ON "AssignmentRoleQualificationRequirementChangeHistory" ("tenantGuid", "timeStamp")
;

-- Index on the AssignmentRoleQualificationRequirementChangeHistory table's tenantGuid,userId fields.
CREATE INDEX "I_ssgnmntRlQulfctnRqurmntChngHstry_tnntGud_usrd" ON "AssignmentRoleQualificationRequirementChangeHistory" ("tenantGuid", "userId")
;
...
CREATE INDEX "I_ntRlQulfctnRqurmntChngHstry_tnntGud_ssgnmntRlQulfctnRqurmntd" ON "AssignmentRoleQualificationRequirementChangeHistory" ("tenantGuid", "assignmentRoleQualificationRequirementId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the PaymentType table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"paymentTypeId" INTEGER NOT NULL,		-- Link to the PaymentType table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ReceiptType table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"receiptTypeId" INTEGER NOT NULL,		-- Link to the ReceiptType table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the SchedulingTargetType table's tenantGuid and name fields.
);
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"officeId" INTEGER NULL,		-- Optional office binding for a scheduling target.
	"clientId" INTEGER NOT NULL,		-- The client that this scheduling target belongs to.
	"schedulingTargetTypeId" INTEGER NOT NULL,		-- Link to the SchedulingTargetType table.
	"timeZoneId" INTEGER NOT NULL,		-- Link to the TimeZone table.
	...
	FOREIGN KEY ("timeZoneId") REFERENCES "TimeZone"("id"),		-- Foreign key to the TimeZone table.
	FOREIGN KEY ("calendarId") REFERENCES "Calendar"("id"),		-- Foreign key to the Calendar table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the SchedulingTarget table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"schedulingTargetId" INTEGER NOT NULL,		-- Link to the SchedulingTarget table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"relationshipTypeId" INTEGER NOT NULL,		-- A description of the relationship between the scheduling target and the contact.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("contactId") REFERENCES "Contact"("id"),		-- Foreign key to the Contact table.
	FOREIGN KEY ("relationshipTypeId") REFERENCES "RelationshipType"("id"),		-- Foreign key to the RelationshipType table.
	UNIQUE ( "tenantGuid", "schedulingTargetId", "contactId") 		-- Uniqueness enforced on the SchedulingTargetContact table's tenantGuid and schedulingTargetId and contactId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"schedulingTargetContactId" INTEGER NOT NULL,		-- Link to the SchedulingTargetContact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchdulngTrgtCntctChngHstry_tnntGud_vrsnNumbr" ON "SchedulingTargetContactChangeHistory" ("tenantGuid", "versionNumber")
;
...
CREATE INDEX "I_SchdulngTrgtCntctChngHstry_tnntGud_schdulngTrgtCntctd" ON "SchedulingTargetContactChangeHistory" ("tenantGuid", "schedulingTargetContactId", "versionNumber", "timeStamp", "userId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"schedulingTargetId" INTEGER NOT NULL,		-- Primary  schuduling target for this address - could be null if there is a client linked to this, so the address would be for all targets in the client.
	"clientId" INTEGER NULL,		-- Optional client level link.  The presence of a value here indicates that the address is to be shared across all scheduling targets for the client.
	"addressLine1" VARCHAR(250) NOT NULL COLLATE NOCASE,
	"addressLine2" VARCHAR(250) NULL COLLATE NOCASE,
	"city" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"postalCode" VARCHAR(100) NULL COLLATE NOCASE,
	...
	"latitude" REAL NULL,		-- Optional latitude position
	"longitude" REAL NULL,		-- Optional longitude position
	"label" VARCHAR(250) NULL COLLATE NOCASE,		-- e.g., 'Main Site', 'Patient Home', 'Hospital Ward'
	...
	FOREIGN KEY ("stateProvinceId") REFERENCES "StateProvince"("id"),		-- Foreign key to the StateProvince table.
	FOREIGN KEY ("countryId") REFERENCES "Country"("id"),		-- Foreign key to the Country table.
	UNIQUE ( "tenantGuid", "schedulingTargetId", "addressLine1", "city", "postalCode") 		-- Uniqueness enforced on the SchedulingTargetAddress table's tenantGuid and schedulingTargetId and addressLine1 and city and postalCode fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"schedulingTargetAddressId" INTEGER NOT NULL,		-- Link to the SchedulingTargetAddress table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchdulngTrgtddrssChngHstry_tnntGud_vrsnNumbr" ON "SchedulingTargetAddressChangeHistory" ("tenantGuid", "versionNumber")
;
...
CREATE INDEX "I_SchdulngTrgtddrssChngHstry_tnntGud_schdulngTrgtddrssd" ON "SchedulingTargetAddressChangeHistory" ("tenantGuid", "schedulingTargetAddressId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	FOREIGN KEY ("schedulingTargetId") REFERENCES "SchedulingTarget"("id"),		-- Foreign key to the SchedulingTarget table.
	FOREIGN KEY ("qualificationId") REFERENCES "Qualification"("id"),		-- Foreign key to the Qualification table.
	UNIQUE ( "tenantGuid", "schedulingTargetId", "qualificationId") 		-- Uniqueness enforced on the SchedulingTargetQualificationRequirement table's tenantGuid and schedulingTargetId and qualificationId fields.
);
...
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmnt_tnntGud_schdulngTrgtd" ON "SchedulingTargetQualificationRequirement" ("tenantGuid", "schedulingTargetId")
;

-- Index on the SchedulingTargetQualificationRequirement table's tenantGuid,qualificationId fields.
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmnt_tnntGud_qulfctnd" ON "SchedulingTargetQualificationRequirement" ("tenantGuid", "qualificationId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"schedulingTargetQualificationRequirementId" INTEGER NOT NULL,		-- Link to the SchedulingTargetQualificationRequirement table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmntChngHstry_tnntGud" ON "SchedulingTargetQualificationRequirementChangeHistory" ("tenantGuid")
;

-- Index on the SchedulingTargetQualificationRequirementChangeHistory table's tenantGuid,versionNumber fields.
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmntChngHstry_tnntGud_vrsnNumbr" ON "SchedulingTargetQualificationRequirementChangeHistory" ("tenantGuid", "versionNumber")
;

-- Index on the SchedulingTargetQualificationRequirementChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmntChngHstry_tnntGud_tmStmp" ON "SchedulingTargetQualificationRequirementChangeHistory" ("tenantGuid", "timeStamp")
;

-- Index on the SchedulingTargetQualificationRequirementChangeHistory table's tenantGuid,userId fields.
CREATE INDEX "I_SchdulngTrgtQulfctnRqurmntChngHstry_tnntGud_usrd" ON "SchedulingTargetQualificationRequirementChangeHistory" ("tenantGuid", "userId")
;
...
CREATE INDEX "I_tQulfctnRqurmntChngHstry_tnntGud_schdulngTrgtQulfctnRqurmntd" ON "SchedulingTargetQualificationRequirementChangeHistory" ("tenantGuid", "schedulingTargetQualificationRequirementId", "versionNumber", "timeStamp", "userId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"recurrenceFrequencyId" INTEGER NOT NULL,		-- Link to the RecurrenceFrequency table.
	"interval" INTEGER NOT NULL DEFAULT 1,		-- How often the pattern repeats (e.g., every 2 weeks)
	"untilDateTime" DATETIME NULL,		-- Recurrence ends on this date (inclusive). NULL = no end date
	"count" INTEGER NULL,		-- Maximum number of occurrences. NULL = unlimited
	"dayOfWeekMask" INTEGER NULL DEFAULT 0,		-- Bitmask for weekly recurrence:  1 = Sunday, 2 = Monday, 4 = Tuesday, 8 = Wednesday, 16 = Thursday, 32 = Friday, 64 = Saturday Example: Monday + Wednesday + Friday = 2 + 8 + 32 = 42
	"dayOfMonth" INTEGER NULL,		-- For monthly: specific day (1-31). NULL if using dayOfWeekInMonth
	"dayOfWeekInMonth" INTEGER NULL,		-- Values: 1 = first, 2 = second, 3 = third, 4 = fourth, 5 = last, -1 = second-to-last, etc. Combine with dayOfWeekMask.  
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"recurrenceRuleId" INTEGER NOT NULL,		-- Link to the RecurrenceRule table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("timeZoneId") REFERENCES "TimeZone"("id"),		-- Foreign key to the TimeZone table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ShiftPattern table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"shiftPatternId" INTEGER NOT NULL,		-- Link to the ShiftPattern table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"shiftPatternId" INTEGER NOT NULL,		-- Link to the ShiftPattern table.
	"dayOfWeek" INTEGER NOT NULL DEFAULT 1,		-- Day this rule applies to   1=Sunday..7=Saturday
	"startTime" TEXT NOT NULL,		-- Start of available window (local to pattern time zone) e.g., 07:00:00
	"hours" REAL NOT NULL DEFAULT 8,		-- Hours available from start time (handles overnight shifts cleanly)  Defaults to 8
	"label" VARCHAR(250) NULL COLLATE NOCASE,		-- e.g., Main Shift
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("shiftPatternId") REFERENCES "ShiftPattern"("id"),		-- Foreign key to the ShiftPattern table.
	UNIQUE ( "tenantGuid", "shiftPatternId", "dayOfWeek") 		-- Uniqueness enforced on the ShiftPatternDay table's tenantGuid and shiftPatternId and dayOfWeek fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"shiftPatternDayId" INTEGER NOT NULL,		-- Link to the ShiftPatternDay table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"officeId" INTEGER NULL,		-- Optional office binding for a resource.
	"resourceTypeId" INTEGER NOT NULL,		-- Link to the ResourceType table.
	"shiftPatternId" INTEGER NULL,		-- Standard shift pattern this resource follows (NULL = custom shifts via ResourceShift)
	"timeZoneId" INTEGER NOT NULL,		-- Link to the TimeZone table.
	...
	FOREIGN KEY ("shiftPatternId") REFERENCES "ShiftPattern"("id"),		-- Foreign key to the ShiftPattern table.
	FOREIGN KEY ("timeZoneId") REFERENCES "TimeZone"("id"),		-- Foreign key to the TimeZone table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Resource table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceId" INTEGER NOT NULL,		-- Link to the Resource table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"relationshipTypeId" INTEGER NOT NULL,		-- A description of the relationship between the resource and the contact.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("contactId") REFERENCES "Contact"("id"),		-- Foreign key to the Contact table.
	FOREIGN KEY ("relationshipTypeId") REFERENCES "RelationshipType"("id"),		-- Foreign key to the RelationshipType table.
	UNIQUE ( "tenantGuid", "resourceId", "contactId") 		-- Uniqueness enforced on the ResourceContact table's tenantGuid and resourceId and contactId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceContactId" INTEGER NOT NULL,		-- Link to the ResourceContact table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"officeId" INTEGER NULL,		-- Optional office binding for a rate sheet.
	"assignmentRoleId" INTEGER NULL,		-- Link to AssignmentRole. If populated, applies to anyone in this role.
	"resourceId" INTEGER NULL,		-- Link to Resource. If populated, overrides the Role rate.
	"schedulingTargetId" INTEGER NULL,		-- Link to SchedulingTarget. If populated, applies only to this project.
	"rateTypeId" INTEGER NOT NULL,		-- e.g., 'Standard', 'Overtime', 'DoubleTime', 'Travel', 'Standby'
	...
	"currencyId" INTEGER NOT NULL,		-- Link to the Currency table.
	"costRate" NUMERIC NOT NULL,		-- Internal Cost (payroll)
	"billingRate" NUMERIC NOT NULL,		-- Invoicing Cost (customre)
	"notes" TEXT NULL COLLATE NOCASE,		-- For ad-hoc notes about the entry
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("rateTypeId") REFERENCES "RateType"("id"),		-- Foreign key to the RateType table.
	FOREIGN KEY ("currencyId") REFERENCES "Currency"("id"),		-- Foreign key to the Currency table.
	UNIQUE ( "tenantGuid", "assignmentRoleId", "resourceId", "schedulingTargetId", "rateTypeId", "effectiveDate") 		-- Uniqueness enforced on the RateSheet table's tenantGuid and assignmentRoleId and resourceId and schedulingTargetId and rateTypeId and effectiveDate fields.
);
...
CREATE INDEX "I_RtSht_tnntGud_schdulngTrgtd_rsurcd_ssgnmntRld_rtTypd_ffctvDt" ON "RateSheet" ("tenantGuid", "schedulingTargetId", "resourceId", "assignmentRoleId", "rateTypeId", "effectiveDate")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"rateSheetId" INTEGER NOT NULL,		-- Link to the RateSheet table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"issuer" VARCHAR(250) NULL COLLATE NOCASE,		-- e.g., State Board of Nursing, NCCCO
	"notes" TEXT NULL COLLATE NOCASE,
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	...
	FOREIGN KEY ("resourceId") REFERENCES "Resource"("id"),		-- Foreign key to the Resource table.
	FOREIGN KEY ("qualificationId") REFERENCES "Qualification"("id"),		-- Foreign key to the Qualification table.
	UNIQUE ( "tenantGuid", "resourceId", "qualificationId") 		-- Uniqueness enforced on the ResourceQualification table's tenantGuid and resourceId and qualificationId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceQualificationId" INTEGER NOT NULL,		-- Link to the ResourceQualification table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_RsurcQulfctnChngHstry_tnntGud_rsurcQulfctnd" ON "ResourceQualificationChangeHistory" ("tenantGuid", "resourceQualificationId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"reason" VARCHAR(250) NULL COLLATE NOCASE,
	"notes" TEXT NULL COLLATE NOCASE,
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
...
CREATE INDEX "I_Rsurcvlblty_tnntGud_rsurcd_strtDtTm_ndDtTm" ON "ResourceAvailability" ("tenantGuid", "resourceId", "startDateTime", "endDateTime")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceAvailabilityId" INTEGER NOT NULL,		-- Link to the ResourceAvailability table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_RsurcvlbltyChngHstry_tnntGud_rsurcvlbltyd" ON "ResourceAvailabilityChangeHistory" ("tenantGuid", "resourceAvailabilityId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceId" INTEGER NOT NULL,		-- Link to the Resource table.
	"dayOfWeek" INTEGER NOT NULL DEFAULT 1,		-- 1=Sunday through 7=Saturday
	"timeZoneId" INTEGER NULL,		-- Link to the TimeZone table.
	"startTime" TEXT NOT NULL,		-- Shift start time (e.g., 09:00:00)
	"hours" REAL NOT NULL DEFAULT 8,		-- Hours available from start time (handles overnight shifts cleanly)  Defaults to 8
	"label" VARCHAR(250) NULL COLLATE NOCASE,		-- e.g., 'Morning Clinic', 'On-Call'
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("resourceId") REFERENCES "Resource"("id"),		-- Foreign key to the Resource table.
	FOREIGN KEY ("timeZoneId") REFERENCES "TimeZone"("id"),		-- Foreign key to the TimeZone table.
	UNIQUE ( "tenantGuid", "resourceId", "dayOfWeek") 		-- Uniqueness enforced on the ResourceShift table's tenantGuid and resourceId and dayOfWeek fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"resourceShiftId" INTEGER NOT NULL,		-- Link to the ResourceShift table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"notes" TEXT NULL COLLATE NOCASE,
	"officeId" INTEGER NULL,		-- Optional office binding for a crew.
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"avatarFileName" VARCHAR(250) NULL COLLATE NOCASE,		-- Part of the binary data field setup
	"avatarSize" BIGINT NULL,		-- Part of the binary data field setup
	...
	FOREIGN KEY ("officeId") REFERENCES "Office"("id"),		-- Foreign key to the Office table.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Crew table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"crewId" INTEGER NOT NULL,		-- Link to the Crew table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"crewId" INTEGER NOT NULL,		-- Link to the Crew table.
	"resourceId" INTEGER NOT NULL,		-- Link to the Resource table.
	"assignmentRoleId" INTEGER NULL,		-- Optional default role this member fulfils when the crew is assigned
	"sequence" INTEGER NOT NULL DEFAULT 1,		-- Display/order position within the crew (lower numbers appear first)
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	...
	FOREIGN KEY ("assignmentRoleId") REFERENCES "AssignmentRole"("id"),		-- Foreign key to the AssignmentRole table.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "crewId", "resourceId") 		-- Uniqueness enforced on the CrewMember table's tenantGuid and crewId and resourceId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"crewMemberId" INTEGER NOT NULL,		-- Link to the CrewMember table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"schedulingTargetTypeId" INTEGER NULL,		-- Optional target type
	"priorityId" INTEGER NULL,		-- Optional priority
	"defaultLocationPattern" VARCHAR(250) NULL COLLATE NOCASE,		-- e.g., 'Patient Home', 'Main Site'
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("schedulingTargetTypeId") REFERENCES "SchedulingTargetType"("id"),		-- Foreign key to the SchedulingTargetType table.
	FOREIGN KEY ("priorityId") REFERENCES "Priority"("id"),		-- Foreign key to the Priority table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the ScheduledEventTemplate table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventTemplateId" INTEGER NOT NULL,		-- Link to the ScheduledEventTemplate table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchduldvntTmpltChngHstry_tnntGud_schduldvntTmpltd" ON "ScheduledEventTemplateChangeHistory" ("tenantGuid", "scheduledEventTemplateId", "versionNumber", "timeStamp", "userId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventTemplateId" INTEGER NOT NULL,		-- Link to ScheduledEventTemplate
	"chargeTypeId" INTEGER NOT NULL,		-- Link to ChargeType (the charge to drop).
	"defaultAmount" NUMERIC NOT NULL,		-- The amount to auto-drop (can be overridden on event).
...
CREATE INDEX "I_SchduldvntTmpltChrg_tnntGud_schduldvntTmpltd" ON "ScheduledEventTemplateCharge" ("tenantGuid", "scheduledEventTemplateId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventTemplateChargeId" INTEGER NOT NULL,		-- Link to the ScheduledEventTemplateCharge table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchduldvntTmpltChrgChngHstry_tnntGud_vrsnNumbr" ON "ScheduledEventTemplateChargeChangeHistory" ("tenantGuid", "versionNumber")
;

-- Index on the ScheduledEventTemplateChargeChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX "I_SchduldvntTmpltChrgChngHstry_tnntGud_tmStmp" ON "ScheduledEventTemplateChargeChangeHistory" ("tenantGuid", "timeStamp")
;
...
CREATE INDEX "I_SchduldvntTmpltChrgChngHstry_tnntGud_schduldvntTmpltChrgd" ON "ScheduledEventTemplateChargeChangeHistory" ("tenantGuid", "scheduledEventTemplateChargeId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	FOREIGN KEY ("scheduledEventTemplateId") REFERENCES "ScheduledEventTemplate"("id"),		-- Foreign key to the ScheduledEventTemplate table.
	FOREIGN KEY ("qualificationId") REFERENCES "Qualification"("id"),		-- Foreign key to the Qualification table.
	UNIQUE ( "tenantGuid", "scheduledEventTemplateId", "qualificationId") 		-- Uniqueness enforced on the ScheduledEventTemplateQualificationRequirement table's tenantGuid and scheduledEventTemplateId and qualificationId fields.
);
...
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmnt_tnntGud_schduldvntTmpltd" ON "ScheduledEventTemplateQualificationRequirement" ("tenantGuid", "scheduledEventTemplateId")
;

-- Index on the ScheduledEventTemplateQualificationRequirement table's tenantGuid,qualificationId fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmnt_tnntGud_qulfctnd" ON "ScheduledEventTemplateQualificationRequirement" ("tenantGuid", "qualificationId")
;

-- Index on the ScheduledEventTemplateQualificationRequirement table's tenantGuid,active fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmnt_tnntGud_ctv" ON "ScheduledEventTemplateQualificationRequirement" ("tenantGuid", "active")
;

-- Index on the ScheduledEventTemplateQualificationRequirement table's tenantGuid,deleted fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmnt_tnntGud_dltd" ON "ScheduledEventTemplateQualificationRequirement" ("tenantGuid", "deleted")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventTemplateQualificationRequirementId" INTEGER NOT NULL,		-- Link to the ScheduledEventTemplateQualificationRequirement table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmntChngHstry_tnntGud" ON "ScheduledEventTemplateQualificationRequirementChangeHistory" ("tenantGuid")
;

-- Index on the ScheduledEventTemplateQualificationRequirementChangeHistory table's tenantGuid,versionNumber fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmntChngHstry_tnntGud_vrsnNumbr" ON "ScheduledEventTemplateQualificationRequirementChangeHistory" ("tenantGuid", "versionNumber")
;

-- Index on the ScheduledEventTemplateQualificationRequirementChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmntChngHstry_tnntGud_tmStmp" ON "ScheduledEventTemplateQualificationRequirementChangeHistory" ("tenantGuid", "timeStamp")
;

-- Index on the ScheduledEventTemplateQualificationRequirementChangeHistory table's tenantGuid,userId fields.
CREATE INDEX "I_SchduldvntTmpltQulfctnRqurmntChngHstry_tnntGud_usrd" ON "ScheduledEventTemplateQualificationRequirementChangeHistory" ("tenantGuid", "userId")
;
...
CREATE INDEX "I_lfctnRqurmntChngHstry_tnntGud_schduldvntTmpltQulfctnRqurmntd" ON "ScheduledEventTemplateQualificationRequirementChangeHistory" ("tenantGuid", "scheduledEventTemplateQualificationRequirementId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display and calendar event color-coding.
	"iconId" INTEGER NULL,		-- Icon to use for UI display.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	...
	"defaultPrice" NUMERIC NULL,		-- Default rental price for events of this type. Used to auto-populate charges in the booking flow.
	"chargeTypeId" INTEGER NULL,		-- Default charge type for auto-created charges.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	FOREIGN KEY ("chargeTypeId") REFERENCES "ChargeType"("id"),		-- Foreign key to the ChargeType table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the EventType table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"eventTypeId" INTEGER NOT NULL,		-- Link to the EventType table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"schedulingTargetId" INTEGER NULL,		-- The SchedulingTarget (project, patient, etc.) this event is scheduled into
	"timeZoneId" INTEGER NULL,		-- Link to the TimeZone table.
	"parentScheduledEventId" INTEGER NULL,		-- If populated, this Event is a specific "Detached" instance of a Series
	...
	"location" VARCHAR(250) NULL COLLATE NOCASE,
	"eventStatusId" INTEGER NOT NULL,		-- Status for the event
	"resourceId" INTEGER NULL,		-- Optional primary/lead resource for the event
	"crewId" INTEGER NULL,		-- Optional primary/lead crew for the event
	"priorityId" INTEGER NULL,		-- Optional priority
	"bookingSourceTypeId" INTEGER NULL,		-- Optional booking source for reservation type workflows.
	"eventTypeId" INTEGER NULL,		-- Event type category — drives UI behavior (rental vs committee event flow, required fields, default pricing).
	...
	FOREIGN KEY ("eventTypeId") REFERENCES "EventType"("id"),		-- Foreign key to the EventType table.
	UNIQUE ( "tenantGuid", "name", "startDateTime") 		-- Uniqueness enforced on the ScheduledEvent table's tenantGuid and name and startDateTime fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"chargeStatusId" INTEGER NOT NULL,		-- Link to the ChargeStatus table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventId" INTEGER NOT NULL,		-- Link to the ScheduledEvent table.
	"resourceId" INTEGER NULL,		-- Optional link to resource to bind charge to specific resources (e.g., labor cost per operator
	"chargeTypeId" INTEGER NOT NULL,		-- Link to the ChargeType table (defines revenue/expense category).
	"chargeStatusId" INTEGER NOT NULL,		-- Link to the ChargeStatus table.  Tracks the status of the charge from creation through invoicing or cancelling.
	"quantity" NUMERIC NULL DEFAULT 1,		-- Quantity (hours, units, km, etc.)
	"unitPrice" NUMERIC NULL,		-- Price per unit (can be NULL for flat fees)
	"extendedAmount" NUMERIC NOT NULL DEFAULT 0,		-- Pre-tax amount (quantity × unitPrice, or just amount for flat fees).
	"taxAmount" NUMERIC NOT NULL DEFAULT 0,		-- The calculated tax based on TaxCode.rate.
	"totalAmount" NUMERIC NOT NULL DEFAULT 0,		-- Total amount inclusive of tax (extendedAmount + taxAmount). Consistent with FinancialTransaction.totalAmount.
	"description" VARCHAR(250) NULL COLLATE NOCASE,		-- Short description or label for the charge. Falls back to ChargeType.name if not set.
	"currencyId" INTEGER NOT NULL,		-- Link to Currency table.
	"rateTypeId" INTEGER NULL,		-- Optional link to RateType (e.g., 'Overtime').
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the charge
	...
	"externalId" VARCHAR(100) NULL COLLATE NOCASE,		-- Identifier from extenral system - possibly invoice number or some other billing grouper
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	"taxCodeId" INTEGER NULL,		-- Optional link to TaxCode. When set, indicates which tax rate was applied to calculate taxAmount. Inherited from ChargeType.taxCodeId on creation.
	FOREIGN KEY ("scheduledEventId") REFERENCES "ScheduledEvent"("id"),		-- Foreign key to the ScheduledEvent table.
	FOREIGN KEY ("resourceId") REFERENCES "Resource"("id"),		-- Foreign key to the Resource table.
	FOREIGN KEY ("chargeTypeId") REFERENCES "ChargeType"("id"),		-- Foreign key to the ChargeType table.
	FOREIGN KEY ("chargeStatusId") REFERENCES "ChargeStatus"("id"),		-- Foreign key to the ChargeStatus table.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"eventChargeId" INTEGER NOT NULL,		-- Link to the EventCharge table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"periodType" VARCHAR(50) NOT NULL DEFAULT 'Month' COLLATE NOCASE,		-- Period type: Month, Quarter, Year, Custom.
	"fiscalYear" INTEGER NOT NULL,		-- The fiscal year this period belongs to.
	"periodNumber" INTEGER NOT NULL,		-- Period number within the fiscal year (1-12 for months, 1-4 for quarters, 1 for year).
	"periodStatusId" INTEGER NOT NULL,		-- Link to PeriodStatus — workflow state (Open, In Review, Closed). Replaces the old periodStatus string field.
	"closedDate" DATETIME NULL,		-- When the period was closed.
	"closedBy" VARCHAR(100) NULL COLLATE NOCASE,		-- User who closed the period.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	...
	UNIQUE ( "tenantGuid", "name") ,		-- Uniqueness enforced on the FiscalPeriod table's tenantGuid and name fields.
	UNIQUE ( "tenantGuid", "fiscalYear", "periodNumber") 		-- Uniqueness enforced on the FiscalPeriod table's tenantGuid and fiscalYear and periodNumber fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"fiscalPeriodId" INTEGER NOT NULL,		-- Link to the FiscalPeriod table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"financialCategoryId" INTEGER NOT NULL,		-- Link to the FinancialCategory (chart of accounts entry).
	"financialOfficeId" INTEGER NULL,		-- Optional link to FinancialOffice — scopes this transaction to a specific department/committee. Can be inherited from the FinancialCategory if not set directly.
	"scheduledEventId" INTEGER NULL,		-- Optional link to a ScheduledEvent when the transaction relates to a booking.
	"contactId" INTEGER NULL,		-- Optional link to the Contact who paid or was paid.
	"clientId" INTEGER NULL,		-- Optional link to the Client (vendor, customer, or supplier) for this transaction.
	"contactRole" VARCHAR(50) NULL DEFAULT 'Customer' COLLATE NOCASE,		-- Role of the linked contact: Customer, Vendor, Employee. Maps to QuickBooks entity types for sync.
	"taxCodeId" INTEGER NULL,		-- Optional link to TaxCode. Overrides the category-level isTaxApplicable for precise tax handling.
	"fiscalPeriodId" INTEGER NULL,		-- Optional link to FiscalPeriod. Auto-assigned based on transactionDate when null.
	"paymentTypeId" INTEGER NULL,		-- Optional link to PaymentType indicating how payment was made (e-transfer, cash, cheque, card, etc.).
	"transactionDate" DATETIME NOT NULL,		-- When the transaction occurred (UTC).
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,		-- Description of the transaction (e.g., 'Easter Brunch Food', 'DD Refund - Natasha Chafe').
	"amount" NUMERIC NOT NULL DEFAULT 0,		-- Transaction amount before tax. Always positive — direction determined by FinancialCategory.accountType.
	"taxAmount" NUMERIC NOT NULL DEFAULT 0,		-- Tax amount (e.g., HST). Calculated from TaxCode.rate when applicable.
	"totalAmount" NUMERIC NOT NULL DEFAULT 0,		-- Total amount inclusive of tax (amount + taxAmount).
	...
	"journalEntryType" VARCHAR(50) NULL COLLATE NOCASE,		-- Double-entry type for accounting integration: Debit or Credit. Null = auto-determined from isRevenue.
	"referenceNumber" VARCHAR(100) NULL COLLATE NOCASE,		-- Cheque number, e-transfer reference, receipt number, etc.
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the transaction.
	"currencyId" INTEGER NOT NULL,		-- Link to Currency table.
	"exportedDate" DATETIME NULL,		-- When this transaction was last exported for reporting (null = not exported yet).
	"externalId" VARCHAR(100) NULL COLLATE NOCASE,		-- Identifier from external system (e.g., QuickBooks Transaction ID).
	"externalSystemName" VARCHAR(50) NULL COLLATE NOCASE,		-- Name of the external system (e.g., 'QuickBooks', 'Xero') for multi-system tracking.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"financialTransactionId" INTEGER NOT NULL,		-- Link to the FinancialTransaction table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_FnnclTrnsctnChngHstry_tnntGud_fnnclTrnsctnd" ON "FinancialTransactionChangeHistory" ("tenantGuid", "financialTransactionId", "versionNumber", "timeStamp", "userId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"financialCategoryId" INTEGER NOT NULL,		-- The category this budget applies to.
	"fiscalPeriodId" INTEGER NOT NULL,		-- The fiscal period this budget covers.
	"financialOfficeId" INTEGER NULL,		-- Optional link to FinancialOffice — scopes this budget to a specific department/committee. Can be inherited from the FinancialCategory if not set directly.
	"budgetedAmount" NUMERIC NOT NULL DEFAULT 0,		-- The planned/budgeted amount for this category in this period.
	"revisedAmount" NUMERIC NULL,		-- Optional revised budget amount (after mid-period adjustments).
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the budget line (e.g., justification for revision).
	"currencyId" INTEGER NOT NULL,		-- Link to Currency table.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	...
	FOREIGN KEY ("financialOfficeId") REFERENCES "FinancialOffice"("id"),		-- Foreign key to the FinancialOffice table.
	FOREIGN KEY ("currencyId") REFERENCES "Currency"("id"),		-- Foreign key to the Currency table.
	UNIQUE ( "tenantGuid", "financialCategoryId", "fiscalPeriodId") 		-- Uniqueness enforced on the Budget table's tenantGuid and financialCategoryId and fiscalPeriodId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"budgetId" INTEGER NOT NULL,		-- Link to the Budget table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,
	"providerType" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Provider type identifier: 'manual', 'stripe', 'square', 'moneris'.
	"isActive" BIT NOT NULL DEFAULT 1,		-- Whether this provider is currently active.
	"apiKeyEncrypted" TEXT NULL COLLATE NOCASE,		-- Encrypted API key for the payment provider.
	"merchantId" VARCHAR(100) NULL COLLATE NOCASE,		-- Merchant account identifier with the provider.
	"webhookSecret" TEXT NULL COLLATE NOCASE,		-- Encrypted webhook validation secret for the provider.
	"processingFeePercent" NUMERIC NULL,		-- Provider processing fee percentage (e.g., 2.9 for Stripe).
	"processingFeeFixed" NUMERIC NULL,		-- Provider fixed processing fee per transaction (e.g., $0.30).
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the provider configuration.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the PaymentProvider table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"paymentProviderId" INTEGER NOT NULL,		-- Link to the PaymentProvider table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"paymentMethodId" INTEGER NOT NULL,		-- How the payment was made (Cash, E-Transfer, Credit Card, etc.).
	"paymentProviderId" INTEGER NULL,		-- Optional link to payment processor (null for cash/cheque).
	"scheduledEventId" INTEGER NULL,		-- Optional link to a ScheduledEvent (e.g., booking payment).
	"financialTransactionId" INTEGER NULL,		-- Optional link to a FinancialTransaction (e.g., bar tab payment).
	"eventChargeId" INTEGER NULL,		-- Optional link to a specific EventCharge (e.g., damage deposit payment).
	"transactionDate" DATETIME NOT NULL,		-- When the payment occurred (UTC).
	"amount" NUMERIC NOT NULL DEFAULT 0,		-- Gross payment amount.
	"processingFee" NUMERIC NOT NULL DEFAULT 0,		-- Fee deducted by the payment provider.
	"netAmount" NUMERIC NOT NULL DEFAULT 0,		-- Net amount received (amount - processingFee).
	"currencyId" INTEGER NOT NULL,		-- Link to Currency table.
	"status" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Payment status: pending, completed, failed, refunded.
	"providerTransactionId" VARCHAR(250) NULL COLLATE NOCASE,		-- Transaction ID from the payment provider (e.g., Stripe charge ID).
	"providerResponse" TEXT NULL COLLATE NOCASE,		-- JSON response from the payment provider for audit purposes.
	"payerName" VARCHAR(250) NULL COLLATE NOCASE,		-- Name of the person who paid.
	"payerEmail" VARCHAR(250) NULL COLLATE NOCASE,		-- Email of the payer.
	"payerPhone" VARCHAR(50) NULL COLLATE NOCASE,		-- Phone number of the payer.
	"receiptNumber" VARCHAR(100) NULL COLLATE NOCASE,		-- Generated receipt number.
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the payment.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"paymentTransactionId" INTEGER NOT NULL,		-- Link to the PaymentTransaction table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_PymntTrnsctnChngHstry_tnntGud_pymntTrnsctnd" ON "PaymentTransactionChangeHistory" ("tenantGuid", "paymentTransactionId", "versionNumber", "timeStamp", "userId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"invoiceNumber" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Auto-generated sequential invoice number (e.g., 'INV-2025-0001').
	"clientId" INTEGER NOT NULL,		-- The client being invoiced.
	"contactId" INTEGER NULL,		-- Optional billing contact person.
	"scheduledEventId" INTEGER NULL,		-- Optional link to the event this invoice relates to.
	"financialOfficeId" INTEGER NULL,		-- Optional issuing financial office.
	"invoiceStatusId" INTEGER NOT NULL,		-- Current invoice status (Draft, Sent, Paid, etc.).
	"currencyId" INTEGER NOT NULL,		-- Currency for all amounts on this invoice.
	"taxCodeId" INTEGER NULL,		-- Default tax code applied to line items.
	"invoiceDate" DATETIME NOT NULL,		-- Date the invoice was issued (UTC).
	"dueDate" DATETIME NOT NULL,		-- Payment due date (UTC).
	"subtotal" NUMERIC NOT NULL DEFAULT 0,		-- Sum of line item amounts before tax.
	"taxAmount" NUMERIC NOT NULL DEFAULT 0,		-- Total tax amount.
	"totalAmount" NUMERIC NOT NULL DEFAULT 0,		-- Grand total (subtotal + taxAmount).
	"amountPaid" NUMERIC NOT NULL DEFAULT 0,		-- Running total of payments received against this invoice.
	...
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes or payment terms.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("currencyId") REFERENCES "Currency"("id"),		-- Foreign key to the Currency table.
	FOREIGN KEY ("taxCodeId") REFERENCES "TaxCode"("id"),		-- Foreign key to the TaxCode table.
	UNIQUE ( "tenantGuid", "invoiceNumber") 		-- Uniqueness enforced on the Invoice table's tenantGuid and invoiceNumber fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"invoiceId" INTEGER NOT NULL,		-- Link to the Invoice table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"invoiceId" INTEGER NOT NULL,		-- Parent invoice.
	"eventChargeId" INTEGER NULL,		-- Optional link to the source EventCharge this line item was created from.
	"financialCategoryId" INTEGER NULL,		-- Optional revenue category for reporting.
	"description" VARCHAR(500) NOT NULL COLLATE NOCASE,		-- Line item description (e.g., 'Hall Rental - Saturday Dec 14').
	"quantity" NUMERIC NOT NULL DEFAULT 1,		-- Quantity (hours, units, etc.).
	"unitPrice" NUMERIC NOT NULL DEFAULT 0,		-- Price per unit.
	"amount" NUMERIC NOT NULL DEFAULT 0,		-- Extended amount (quantity × unitPrice).
	"taxAmount" NUMERIC NOT NULL DEFAULT 0,		-- Tax for this line item.
	"totalAmount" NUMERIC NOT NULL DEFAULT 0,		-- Line total (amount + taxAmount).
	"sequence" INTEGER NULL,		-- Display order on the invoice.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("invoiceId") REFERENCES "Invoice"("id"),		-- Foreign key to the Invoice table.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"receiptNumber" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- Auto-generated sequential receipt number (e.g., 'REC-2025-0001').
	"receiptTypeId" INTEGER NOT NULL,		-- Type of receipt (Official, Summary, etc.).
	"invoiceId" INTEGER NULL,		-- Optional link to the invoice this receipt is for.
	"paymentTransactionId" INTEGER NULL,		-- Optional link to the payment transaction.
	"financialTransactionId" INTEGER NULL,		-- Optional link to the financial transaction.
	"clientId" INTEGER NULL,		-- Optional payer client.
	"contactId" INTEGER NULL,		-- Optional payer contact.
	"currencyId" INTEGER NOT NULL,		-- Currency for the amount.
	"receiptDate" DATETIME NOT NULL,		-- Date the receipt was issued (UTC).
	"amount" NUMERIC NOT NULL DEFAULT 0,		-- Amount received.
	"paymentMethod" VARCHAR(100) NULL COLLATE NOCASE,		-- How the payment was made (e.g., 'E-Transfer', 'Cash').
	"description" VARCHAR(500) NULL COLLATE NOCASE,		-- Description of what the payment was for.
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional additional notes.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("contactId") REFERENCES "Contact"("id"),		-- Foreign key to the Contact table.
	FOREIGN KEY ("currencyId") REFERENCES "Currency"("id"),		-- Foreign key to the Currency table.
	UNIQUE ( "tenantGuid", "receiptNumber") 		-- Uniqueness enforced on the Receipt table's tenantGuid and receiptNumber fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"receiptId" INTEGER NOT NULL,		-- Link to the Receipt table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactId" INTEGER NOT NULL,		-- The contact that is the target of the interaction.
	"initiatingContactId" INTEGER NULL,		-- Optional contact that initiated the interaction.  This would be staff of the company using the scheduler
	"interactionTypeId" INTEGER NOT NULL,		-- Link to the InteractionType table.
	"scheduledEventId" INTEGER NULL,		-- The optional event that the interaction is regarding.
	"startTime" DATETIME NOT NULL,
	"endTime" DATETIME NULL,
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the interaction
	"location" TEXT NULL COLLATE NOCASE,		-- Optional location details about the interaction
	"priorityId" INTEGER NULL,		-- Optional priority for the interaction.
	"externalId" VARCHAR(100) NULL COLLATE NOCASE,
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactInteractionId" INTEGER NOT NULL,		-- Link to the ContactInteraction table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_CntctntrctnChngHstry_tnntGud_cntctntrctnd" ON "ContactInteractionChangeHistory" ("tenantGuid", "contactInteractionId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	FOREIGN KEY ("scheduledEventId") REFERENCES "ScheduledEvent"("id"),		-- Foreign key to the ScheduledEvent table.
	FOREIGN KEY ("calendarId") REFERENCES "Calendar"("id"),		-- Foreign key to the Calendar table.
	UNIQUE ( "tenantGuid", "scheduledEventId", "calendarId") 		-- Uniqueness enforced on the EventCalendar table's tenantGuid and scheduledEventId and calendarId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"predecessorEventId" INTEGER NOT NULL,		-- The task that must happen first
	"successorEventId" INTEGER NOT NULL,		-- The task that waits
	"dependencyTypeId" INTEGER NOT NULL,		-- Link to the DependencyType table.
	...
	UNIQUE ( "tenantGuid", "predecessorEventId", "successorEventId") 		-- Uniqueness enforced on the ScheduledEventDependency table's tenantGuid and predecessorEventId and successorEventId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventDependencyId" INTEGER NOT NULL,		-- Link to the ScheduledEventDependency table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchduldvntDpndncyChngHstry_tnntGud_vrsnNumbr" ON "ScheduledEventDependencyChangeHistory" ("tenantGuid", "versionNumber")
;
...
CREATE INDEX "I_SchduldvntDpndncyChngHstry_tnntGud_schduldvntDpndncyd" ON "ScheduledEventDependencyChangeHistory" ("tenantGuid", "scheduledEventDependencyId", "versionNumber", "timeStamp", "userId")
;
...
CREATE INDEX "I_SchduldvntQulfctnRqurmnt_tnntGud_schduldvntd" ON "ScheduledEventQualificationRequirement" ("tenantGuid", "scheduledEventId")
;

-- Index on the ScheduledEventQualificationRequirement table's tenantGuid,qualificationId fields.
CREATE INDEX "I_SchduldvntQulfctnRqurmnt_tnntGud_qulfctnd" ON "ScheduledEventQualificationRequirement" ("tenantGuid", "qualificationId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"scheduledEventQualificationRequirementId" INTEGER NOT NULL,		-- Link to the ScheduledEventQualificationRequirement table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
CREATE INDEX "I_SchduldvntQulfctnRqurmntChngHstry_tnntGud" ON "ScheduledEventQualificationRequirementChangeHistory" ("tenantGuid")
;

-- Index on the ScheduledEventQualificationRequirementChangeHistory table's tenantGuid,versionNumber fields.
CREATE INDEX "I_SchduldvntQulfctnRqurmntChngHstry_tnntGud_vrsnNumbr" ON "ScheduledEventQualificationRequirementChangeHistory" ("tenantGuid", "versionNumber")
;

-- Index on the ScheduledEventQualificationRequirementChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX "I_SchduldvntQulfctnRqurmntChngHstry_tnntGud_tmStmp" ON "ScheduledEventQualificationRequirementChangeHistory" ("tenantGuid", "timeStamp")
;

-- Index on the ScheduledEventQualificationRequirementChangeHistory table's tenantGuid,userId fields.
CREATE INDEX "I_SchduldvntQulfctnRqurmntChngHstry_tnntGud_usrd" ON "ScheduledEventQualificationRequirementChangeHistory" ("tenantGuid", "userId")
;
...
CREATE INDEX "I_vntQulfctnRqurmntChngHstry_tnntGud_schduldvntQulfctnRqurmntd" ON "ScheduledEventQualificationRequirementChangeHistory" ("tenantGuid", "scheduledEventQualificationRequirementId", "versionNumber", "timeStamp", "userId")
;
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("scheduledEventId") REFERENCES "ScheduledEvent"("id"),		-- Foreign key to the ScheduledEvent table.
	UNIQUE ( "tenantGuid", "scheduledEventId", "exceptionDateTime") 		-- Uniqueness enforced on the RecurrenceException table's tenantGuid and scheduledEventId and exceptionDateTime fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"recurrenceExceptionId" INTEGER NOT NULL,		-- Link to the RecurrenceException table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"glCode" VARCHAR(100) NULL COLLATE NOCASE,		-- The accounting code
	"isRestricted" BIT NOT NULL DEFAULT 0,		-- Legal restriction on funds
	"goalAmount" NUMERIC NULL,
	"notes" TEXT NULL COLLATE NOCASE,
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Fund table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"fundId" INTEGER NOT NULL,		-- Link to the Fund table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Campaign table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"campaignId" INTEGER NOT NULL,		-- Link to the Campaign table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"campaignId" INTEGER NULL,		-- Optional link to parent campaign
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"costPerUnit" NUMERIC NULL,		-- For ROI calculation (Cost vs. Raised)
	"notes" TEXT NULL COLLATE NOCASE,
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	...
	FOREIGN KEY ("campaignId") REFERENCES "Campaign"("id"),		-- Foreign key to the Campaign table.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Appeal table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"appealId" INTEGER NOT NULL,		-- Link to the Appeal table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"schedulingTargetId" INTEGER NULL,		-- Link to the SchedulingTarget table.
	"formalSalutation" VARCHAR(250) NULL COLLATE NOCASE,		-- ex. "Mr. and Mrs. John Smith"
	"informalSalutation" VARCHAR(250) NULL COLLATE NOCASE,		-- ex. "John and Jane"
	"addressee" VARCHAR(250) NULL COLLATE NOCASE,		-- The label for the envelope
	"totalHouseholdGiving" NUMERIC NOT NULL DEFAULT 0,
	...
	FOREIGN KEY ("schedulingTargetId") REFERENCES "SchedulingTarget"("id"),		-- Foreign key to the SchedulingTarget table.
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Household table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"householdId" INTEGER NOT NULL,		-- Link to the Household table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"contactId" INTEGER NULL,		-- Polymorphic Link: A constituent is EITHER a generic Contact OR a Client (Company)
	"clientId" INTEGER NULL,		-- Polymorphic Link: A constituent is EITHER a generic Contact OR a Client (Company)
	"householdId" INTEGER NULL,		-- Links a constituent to a household
	"constituentNumber" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The distinct 'Donor ID'
	"doNotSolicit" BIT NOT NULL DEFAULT 0,
	"doNotEmail" BIT NOT NULL DEFAULT 0,
	"doNotMail" BIT NOT NULL DEFAULT 0,
	"totalLifetimeGiving" NUMERIC NOT NULL DEFAULT 0,		-- Caching Calculated Metrics(DP Style for fast reporting)
	"totalYTDGiving" NUMERIC NOT NULL DEFAULT 0,		-- Caching Calculated Metrics(DP Style for fast reporting)
	"lastGiftDate" DATE NULL,		-- Caching Calculated Metrics(DP Style for fast reporting)
	"lastGiftAmount" NUMERIC NULL,		-- Caching Calculated Metrics(DP Style for fast reporting)
	"largestGiftAmount" NUMERIC NULL,		-- Caching Calculated Metrics(DP Style for fast reporting)
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"constituentId" INTEGER NOT NULL,		-- Link to the Constituent table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"balanceAmount" NUMERIC NOT NULL,		-- Calculated: Total - Sum(LinkedGifts)
	"pledgeDate" DATE NOT NULL,
	"startDate" DATE NULL,
	"endDate" DATE NULL,
	"recurrenceFrequencyId" INTEGER NULL,		-- Link to the RecurrenceFrequency table.
	"fundId" INTEGER NOT NULL,		-- Link to the Fund table.
	"campaignId" INTEGER NULL,		-- Link to the Campaign table.
	"appealId" INTEGER NULL,		-- Link to the Appeal table.
	"writeOffAmount" NUMERIC NOT NULL,		-- If they default on the pledge
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"pledgeId" INTEGER NOT NULL,		-- Link to the Pledge table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"batchNumber" VARCHAR(100) NOT NULL COLLATE NOCASE,		-- User-facing ID (e.g., "2026-01-15-MAIL"
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"dateOpened" DATETIME NOT NULL,
	"datePosted" DATETIME NULL,
	"batchStatusId" INTEGER NOT NULL,		-- Link to the BatchStatus table.
	"controlAmount" NUMERIC NOT NULL DEFAULT 0,
	"controlCount" INTEGER NOT NULL DEFAULT 0,
	"defaultFundId" INTEGER NULL,		-- Optional default fund
	"defaultCampaignId" INTEGER NULL,		-- Optional default campaign
	"defaultAppealId" INTEGER NULL,		-- Optional default appeal
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"batchId" INTEGER NOT NULL,		-- Link to the Batch table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"tributeTypeId" INTEGER NULL,		-- Link to the TributeType table.
	"defaultAcknowledgeeId" INTEGER NULL,		-- Constituent to notify (e.g., the widow)
	"startDate" DATE NULL,
	"endDate" DATE NULL,
	"iconId" INTEGER NULL,		-- Icon to use for UI display
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Hex color for UI display
	...
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the Tribute table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"tributeId" INTEGER NOT NULL,		-- Link to the Tribute table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"officeId" INTEGER NULL,		-- Which office received/owns this gift
	"constituentId" INTEGER NOT NULL,		-- Link to the Constituent table.
	"pledgeId" INTEGER NULL,		-- Link to the Pledge table.
	...
	"appealId" INTEGER NULL,		-- Link to the Appeal table.
	"paymentTypeId" INTEGER NOT NULL,		-- Link to the PaymentType table.
	"referenceNumber" VARCHAR(100) NULL COLLATE NOCASE,		-- Check # or Transaction ID
	"batchId" INTEGER NULL,		-- Link to processing batch
	"receiptTypeId" INTEGER NULL,		-- Link to the ReceiptType table.
	"receiptDate" DATETIME NULL,
	"tributeId" INTEGER NULL,		-- Link to the Tribute table.
	"notes" TEXT NULL COLLATE NOCASE,
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"giftId" INTEGER NOT NULL,		-- Link to the Gift table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"giftId" INTEGER NOT NULL,		-- Link to the Gift table.
	"constituentId" INTEGER NOT NULL,		-- The person getting the soft credit
	"amount" NUMERIC NOT NULL,		-- Might be full amount or partial
	"notes" TEXT NULL COLLATE NOCASE,
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"softCreditId" INTEGER NOT NULL,		-- Link to the SoftCredit table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"name" VARCHAR(250) NOT NULL COLLATE NOCASE,		-- Folder display name.
	"description" VARCHAR(500) NULL COLLATE NOCASE,		-- Optional folder description.
	"parentDocumentFolderId" INTEGER NULL,		-- Self-referencing FK for folder hierarchy. NULL = root-level folder.
	"iconId" INTEGER NULL,		-- Optional custom folder icon for UI display.
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Optional folder color for UI display.
	"sequence" INTEGER NOT NULL DEFAULT 0,		-- Display order among sibling folders.
	"notes" TEXT NULL COLLATE NOCASE,		-- Optional notes about the folder.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	...
	FOREIGN KEY ("iconId") REFERENCES "Icon"("id"),		-- Foreign key to the Icon table.
	UNIQUE ( "tenantGuid", "parentDocumentFolderId", "name") 		-- Uniqueness enforced on the DocumentFolder table's tenantGuid and parentDocumentFolderId and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"documentFolderId" INTEGER NOT NULL,		-- Link to the DocumentFolder table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"name" VARCHAR(100) NOT NULL COLLATE NOCASE,
	"description" VARCHAR(500) NULL COLLATE NOCASE,
	"color" VARCHAR(10) NULL COLLATE NOCASE,		-- Tag badge color for UI display.
	"sequence" INTEGER NULL,		-- Sequence to use for sorting.
	"versionNumber" INTEGER NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	"objectGuid" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,		-- Unique identifier for this table.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE ( "tenantGuid", "name") 		-- Uniqueness enforced on the DocumentTag table's tenantGuid and name fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"documentTagId" INTEGER NOT NULL,		-- Link to the DocumentTag table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"documentTypeId" INTEGER NULL,		-- Optional type classification (Rental Agreement, Receipt, Photo, etc.). Nullable because general-purpose files in the file manager may not need a type.
	"documentFolderId" INTEGER NULL,		-- Optional folder placement. NULL = root level / unfiled document.
...
(
	...
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	"documentId" INTEGER NOT NULL,		-- The document being tagged.
	"documentTagId" INTEGER NOT NULL,		-- The tag applied to the document.
	FOREIGN KEY ("documentId") REFERENCES "Document"("id"),		-- Foreign key to the Document table.
	FOREIGN KEY ("documentTagId") REFERENCES "DocumentTag"("id"),		-- Foreign key to the DocumentTag table.
	UNIQUE ( "tenantGuid", "documentId", "documentTagId") 		-- Uniqueness enforced on the DocumentDocumentTag table's tenantGuid and documentId and documentTagId fields.
);
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"tenantGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- The guid for the Tenant to which this record belongs.
	"documentDocumentTagId" INTEGER NOT NULL,		-- Link to the DocumentDocumentTag table.
	"versionNumber" INTEGER NOT NULL,		-- This is the version number that is being historized.
	"timeStamp" DATETIME NOT NULL,		-- The time that the record version was created.
	"userId" INTEGER NOT NULL,
	"data" TEXT NOT NULL COLLATE NOCASE,		-- This stores the JSON representing the object's historical state.
...
(
	...
	"loginUrl" VARCHAR(250) NULL COLLATE NOCASE,		-- Salesforce login endpoint (e.g. https://login.salesforce.com/services/oauth2/token)
