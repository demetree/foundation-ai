...
@Component({
  selector: 'app-document-download-add-edit',
  templateUrl: './document-download-add-edit.component.html',
  styleUrls: ['./document-download-add-edit.component.scss']
})
export class DocumentDownloadAddEditComponent {
  @ViewChild('documentDownloadModal') documentDownloadModal!: TemplateRef<any>;
  @Output() documentDownloadChanged = new Subject<DocumentDownloadData[]>();
  @Input() documentDownloadSubmitData: DocumentDownloadSubmitData | null = null;
  ...
  openModal(documentDownloadData?: DocumentDownloadData) {

    if (documentDownloadData != null) {

      if (!this.documentDownloadService.userIsCommunityDocumentDownloadReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Document Downloads`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.documentDownloadSubmitData = this.documentDownloadService.ConvertToDocumentDownloadSubmitData(documentDownloadData);
      this.isEditMode = true;
      this.objectGuid = documentDownloadData.objectGuid;

      this.buildFormValues(documentDownloadData);

    } else {

      if (!this.documentDownloadService.userIsCommunityDocumentDownloadWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Document Downloads`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.documentDownloadModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const documentDownloadSubmitData: DocumentDownloadSubmitData = {
        id: this.documentDownloadSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateDocumentDownload(documentDownloadSubmitData);
      } else {
        this.addDocumentDownload(documentDownloadSubmitData);
      }
  }

  private addDocumentDownload(documentDownloadData: DocumentDownloadSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    documentDownloadData.active = true;
    documentDownloadData.deleted = false;
    this.documentDownloadService.PostDocumentDownload(documentDownloadData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newDocumentDownload) => {

        this.documentDownloadService.ClearAllCaches();

        this.documentDownloadChanged.next([newDocumentDownload]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/documentdownload', newDocumentDownload.id]);
        }
      },
      error: (err) => {
  ...
  private updateDocumentDownload(documentDownloadData: DocumentDownloadSubmitData) {
    this.documentDownloadService.PutDocumentDownload(documentDownloadData.id, documentDownloadData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedDocumentDownload) => {

        this.documentDownloadService.ClearAllCaches();

        this.documentDownloadChanged.next([updatedDocumentDownload]);

        this.alertService.showMessage("Document Download updated successfully", '', MessageSeverity.success);
