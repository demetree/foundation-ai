...
| `OpenAiInferenceProvider` | OpenAI / Azure OpenAI / Ollama | GPT-4o, GPT-4o-mini, Llama 3, Mistral | Cloud / Local |
| `BitNetInferenceProvider` *(future)* | Microsoft BitNet | 1.58-bit ternary models | CPU only ✅ |
...
> [!NOTE]
> **BitNet (Future):** Microsoft's [BitNet](https://github.com/microsoft/BitNet) enables 1.58-bit LLM inference on CPU using ternary weights ({-1, 0, 1}) and bitwise operations. Models are ~3× smaller than GGUF Q4 and run without GPU. Currently C++/Python only — will add `BitNetInferenceProvider` when C# bindings mature. Our `IInferenceProvider` abstraction requires zero changes to support it.
