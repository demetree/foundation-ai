...
@Component({
  selector: 'app-receipt-type-add-edit',
  templateUrl: './receipt-type-add-edit.component.html',
  styleUrls: ['./receipt-type-add-edit.component.scss']
})
export class ReceiptTypeAddEditComponent {
  @ViewChild('receiptTypeModal') receiptTypeModal!: TemplateRef<any>;
  @Output() receiptTypeChanged = new Subject<ReceiptTypeData[]>();
  @Input() receiptTypeSubmitData: ReceiptTypeSubmitData | null = null;
  ...
  openModal(receiptTypeData?: ReceiptTypeData) {

    if (receiptTypeData != null) {

      if (!this.receiptTypeService.userIsSchedulerReceiptTypeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Receipt Types`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.receiptTypeSubmitData = this.receiptTypeService.ConvertToReceiptTypeSubmitData(receiptTypeData);
      this.isEditMode = true;
      this.objectGuid = receiptTypeData.objectGuid;

      this.buildFormValues(receiptTypeData);

    } else {

      if (!this.receiptTypeService.userIsSchedulerReceiptTypeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Receipt Types`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.receiptTypeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const receiptTypeSubmitData: ReceiptTypeSubmitData = {
      ...
      if (this.isEditMode) {
        this.updateReceiptType(receiptTypeSubmitData);
      } else {
        this.addReceiptType(receiptTypeSubmitData);
      }
  }
  ...
  private addReceiptType(receiptTypeData: ReceiptTypeSubmitData) {
    ...
    this.receiptTypeService.PostReceiptType(receiptTypeData).pipe(
      ...
      next: (newReceiptType) => {
        ...
        this.receiptTypeChanged.next([newReceiptType]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/receipttype', newReceiptType.id]);
        }
      },
      error: (err) => {
  ...
  private updateReceiptType(receiptTypeData: ReceiptTypeSubmitData) {
    this.receiptTypeService.PutReceiptType(receiptTypeData.id, receiptTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedReceiptType) => {

        this.receiptTypeService.ClearAllCaches();

        this.receiptTypeChanged.next([updatedReceiptType]);

        this.alertService.showMessage("Receipt Type updated successfully", '', MessageSeverity.success);
