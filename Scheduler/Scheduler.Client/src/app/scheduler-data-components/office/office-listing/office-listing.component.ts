...
@Component({
  ...
  styleUrls: ['./office-listing.component.scss']
})
export class OfficeListingComponent implements OnInit, AfterViewInit, CanComponentDeactivate {
  @ViewChild(OfficeAddEditComponent) addEditOfficeComponent!: OfficeAddEditComponent;
  @ViewChild(OfficeTableComponent) officeTableComponent!: OfficeTableComponent;
  ...
  private loadCounts(): void {
    ...
    if (this.filterText) {
      ...
      this.filteredOfficeCount$ = this.totalOfficeCount$; // No filter, so assign to be same as total observable
      this.loadingFilteredCount = false;
    }
  ...
  public onFilterChange(): void {
    ...
    this.debounceTimeout = setTimeout(() => {
      this.officeTableComponent.resetToFirstPage(); // Reset to page 1 on filter change
