...
@Component({
  selector: 'app-document-folder-change-history-add-edit',
  templateUrl: './document-folder-change-history-add-edit.component.html',
  styleUrls: ['./document-folder-change-history-add-edit.component.scss']
})
export class DocumentFolderChangeHistoryAddEditComponent {
  @ViewChild('documentFolderChangeHistoryModal') documentFolderChangeHistoryModal!: TemplateRef<any>;
  @Output() documentFolderChangeHistoryChanged = new Subject<DocumentFolderChangeHistoryData[]>();
  @Input() documentFolderChangeHistorySubmitData: DocumentFolderChangeHistorySubmitData | null = null;
  ...
  openModal(documentFolderChangeHistoryData?: DocumentFolderChangeHistoryData) {
    ...
    if (documentFolderChangeHistoryData != null) {
      ...
      if (!this.documentFolderChangeHistoryService.userIsSchedulerDocumentFolderChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Document Folder Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.documentFolderChangeHistorySubmitData = this.documentFolderChangeHistoryService.ConvertToDocumentFolderChangeHistorySubmitData(documentFolderChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(documentFolderChangeHistoryData);

    } else {

      if (!this.documentFolderChangeHistoryService.userIsSchedulerDocumentFolderChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Document Folder Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.documentFolderChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const documentFolderChangeHistorySubmitData: DocumentFolderChangeHistorySubmitData = {
        id: this.documentFolderChangeHistorySubmitData?.id || 0,
        documentFolderId: Number(formValue.documentFolderId),
        versionNumber: this.documentFolderChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateDocumentFolderChangeHistory(documentFolderChangeHistorySubmitData);
      } else {
        this.addDocumentFolderChangeHistory(documentFolderChangeHistorySubmitData);
      }
  }

  private addDocumentFolderChangeHistory(documentFolderChangeHistoryData: DocumentFolderChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    documentFolderChangeHistoryData.versionNumber = 0;
    this.documentFolderChangeHistoryService.PostDocumentFolderChangeHistory(documentFolderChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newDocumentFolderChangeHistory) => {

        this.documentFolderChangeHistoryService.ClearAllCaches();

        this.documentFolderChangeHistoryChanged.next([newDocumentFolderChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/documentfolderchangehistory', newDocumentFolderChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateDocumentFolderChangeHistory(documentFolderChangeHistoryData: DocumentFolderChangeHistorySubmitData) {
    this.documentFolderChangeHistoryService.PutDocumentFolderChangeHistory(documentFolderChangeHistoryData.id, documentFolderChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedDocumentFolderChangeHistory) => {

        this.documentFolderChangeHistoryService.ClearAllCaches();

        this.documentFolderChangeHistoryChanged.next([updatedDocumentFolderChangeHistory]);

        this.alertService.showMessage("Document Folder Change History updated successfully", '', MessageSeverity.success);
