...
@Component({
  selector: 'app-scheduling-target-type-add-edit',
  templateUrl: './scheduling-target-type-add-edit.component.html',
  styleUrls: ['./scheduling-target-type-add-edit.component.scss']
})
export class SchedulingTargetTypeAddEditComponent {
  @ViewChild('schedulingTargetTypeModal') schedulingTargetTypeModal!: TemplateRef<any>;
  @Output() schedulingTargetTypeChanged = new Subject<SchedulingTargetTypeData[]>();
  @Input() schedulingTargetTypeSubmitData: SchedulingTargetTypeSubmitData | null = null;
  ...
  openModal(schedulingTargetTypeData?: SchedulingTargetTypeData) {

    if (schedulingTargetTypeData != null) {

      if (!this.schedulingTargetTypeService.userIsSchedulerSchedulingTargetTypeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduling Target Types`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.schedulingTargetTypeSubmitData = this.schedulingTargetTypeService.ConvertToSchedulingTargetTypeSubmitData(schedulingTargetTypeData);
      this.isEditMode = true;
      this.objectGuid = schedulingTargetTypeData.objectGuid;

      this.buildFormValues(schedulingTargetTypeData);

    } else {

      if (!this.schedulingTargetTypeService.userIsSchedulerSchedulingTargetTypeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduling Target Types`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.schedulingTargetTypeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const schedulingTargetTypeSubmitData: SchedulingTargetTypeSubmitData = {
        id: this.schedulingTargetTypeSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateSchedulingTargetType(schedulingTargetTypeSubmitData);
      } else {
        this.addSchedulingTargetType(schedulingTargetTypeSubmitData);
      }
  }

  private addSchedulingTargetType(schedulingTargetTypeData: SchedulingTargetTypeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    schedulingTargetTypeData.active = true;
    schedulingTargetTypeData.deleted = false;
    this.schedulingTargetTypeService.PostSchedulingTargetType(schedulingTargetTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newSchedulingTargetType) => {

        this.schedulingTargetTypeService.ClearAllCaches();

        this.schedulingTargetTypeChanged.next([newSchedulingTargetType]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/schedulingtargettype', newSchedulingTargetType.id]);
        }
      },
      error: (err) => {
  ...
  private updateSchedulingTargetType(schedulingTargetTypeData: SchedulingTargetTypeSubmitData) {
    this.schedulingTargetTypeService.PutSchedulingTargetType(schedulingTargetTypeData.id, schedulingTargetTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedSchedulingTargetType) => {

        this.schedulingTargetTypeService.ClearAllCaches();

        this.schedulingTargetTypeChanged.next([updatedSchedulingTargetType]);

        this.alertService.showMessage("Scheduling Target Type updated successfully", '', MessageSeverity.success);
