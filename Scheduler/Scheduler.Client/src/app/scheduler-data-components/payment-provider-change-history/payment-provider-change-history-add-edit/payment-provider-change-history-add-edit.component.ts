...
@Component({
  selector: 'app-payment-provider-change-history-add-edit',
  templateUrl: './payment-provider-change-history-add-edit.component.html',
  styleUrls: ['./payment-provider-change-history-add-edit.component.scss']
})
export class PaymentProviderChangeHistoryAddEditComponent {
  @ViewChild('paymentProviderChangeHistoryModal') paymentProviderChangeHistoryModal!: TemplateRef<any>;
  @Output() paymentProviderChangeHistoryChanged = new Subject<PaymentProviderChangeHistoryData[]>();
  @Input() paymentProviderChangeHistorySubmitData: PaymentProviderChangeHistorySubmitData | null = null;
  ...
  openModal(paymentProviderChangeHistoryData?: PaymentProviderChangeHistoryData) {
    ...
    if (paymentProviderChangeHistoryData != null) {
      ...
      if (!this.paymentProviderChangeHistoryService.userIsSchedulerPaymentProviderChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Payment Provider Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.paymentProviderChangeHistorySubmitData = this.paymentProviderChangeHistoryService.ConvertToPaymentProviderChangeHistorySubmitData(paymentProviderChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(paymentProviderChangeHistoryData);

    } else {

      if (!this.paymentProviderChangeHistoryService.userIsSchedulerPaymentProviderChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Payment Provider Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.paymentProviderChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const paymentProviderChangeHistorySubmitData: PaymentProviderChangeHistorySubmitData = {
        id: this.paymentProviderChangeHistorySubmitData?.id || 0,
        paymentProviderId: Number(formValue.paymentProviderId),
        versionNumber: this.paymentProviderChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updatePaymentProviderChangeHistory(paymentProviderChangeHistorySubmitData);
      } else {
        this.addPaymentProviderChangeHistory(paymentProviderChangeHistorySubmitData);
      }
  }

  private addPaymentProviderChangeHistory(paymentProviderChangeHistoryData: PaymentProviderChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    paymentProviderChangeHistoryData.versionNumber = 0;
    this.paymentProviderChangeHistoryService.PostPaymentProviderChangeHistory(paymentProviderChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newPaymentProviderChangeHistory) => {

        this.paymentProviderChangeHistoryService.ClearAllCaches();

        this.paymentProviderChangeHistoryChanged.next([newPaymentProviderChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/paymentproviderchangehistory', newPaymentProviderChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updatePaymentProviderChangeHistory(paymentProviderChangeHistoryData: PaymentProviderChangeHistorySubmitData) {
    this.paymentProviderChangeHistoryService.PutPaymentProviderChangeHistory(paymentProviderChangeHistoryData.id, paymentProviderChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedPaymentProviderChangeHistory) => {

        this.paymentProviderChangeHistoryService.ClearAllCaches();

        this.paymentProviderChangeHistoryChanged.next([updatedPaymentProviderChangeHistory]);

        this.alertService.showMessage("Payment Provider Change History updated successfully", '', MessageSeverity.success);
