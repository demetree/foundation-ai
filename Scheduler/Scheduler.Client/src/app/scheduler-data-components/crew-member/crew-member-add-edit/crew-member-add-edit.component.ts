...
@Component({
  selector: 'app-crew-member-add-edit',
  templateUrl: './crew-member-add-edit.component.html',
  styleUrls: ['./crew-member-add-edit.component.scss']
})
export class CrewMemberAddEditComponent {
  @ViewChild('crewMemberModal') crewMemberModal!: TemplateRef<any>;
  @Output() crewMemberChanged = new Subject<CrewMemberData[]>();
  @Input() crewMemberSubmitData: CrewMemberSubmitData | null = null;
  ...
  openModal(crewMemberData?: CrewMemberData) {

    if (crewMemberData != null) {

      if (!this.crewMemberService.userIsSchedulerCrewMemberReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Crew Members`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.crewMemberSubmitData = this.crewMemberService.ConvertToCrewMemberSubmitData(crewMemberData);
      this.isEditMode = true;
      this.objectGuid = crewMemberData.objectGuid;

      this.buildFormValues(crewMemberData);

    } else {

      if (!this.crewMemberService.userIsSchedulerCrewMemberWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Crew Members`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.crewMemberModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const crewMemberSubmitData: CrewMemberSubmitData = {
        id: this.crewMemberSubmitData?.id || 0,
        ...
        versionNumber: this.crewMemberSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateCrewMember(crewMemberSubmitData);
      } else {
        this.addCrewMember(crewMemberSubmitData);
      }
  }

  private addCrewMember(crewMemberData: CrewMemberSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    crewMemberData.versionNumber = 0;
    crewMemberData.active = true;
    crewMemberData.deleted = false;
    this.crewMemberService.PostCrewMember(crewMemberData).pipe(
      ...
      next: (newCrewMember) => {
        ...
        this.crewMemberChanged.next([newCrewMember]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/crewmember', newCrewMember.id]);
        }
      },
      error: (err) => {
  ...
  private updateCrewMember(crewMemberData: CrewMemberSubmitData) {
    this.crewMemberService.PutCrewMember(crewMemberData.id, crewMemberData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedCrewMember) => {

        this.crewMemberService.ClearAllCaches();

        this.crewMemberChanged.next([updatedCrewMember]);

        this.alertService.showMessage("Crew Member updated successfully", '', MessageSeverity.success);
