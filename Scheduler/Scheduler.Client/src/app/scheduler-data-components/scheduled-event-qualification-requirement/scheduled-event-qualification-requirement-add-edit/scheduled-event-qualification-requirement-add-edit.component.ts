...
@Component({
  selector: 'app-scheduled-event-qualification-requirement-add-edit',
  templateUrl: './scheduled-event-qualification-requirement-add-edit.component.html',
  styleUrls: ['./scheduled-event-qualification-requirement-add-edit.component.scss']
})
export class ScheduledEventQualificationRequirementAddEditComponent {
  @ViewChild('scheduledEventQualificationRequirementModal') scheduledEventQualificationRequirementModal!: TemplateRef<any>;
  @Output() scheduledEventQualificationRequirementChanged = new Subject<ScheduledEventQualificationRequirementData[]>();
  @Input() scheduledEventQualificationRequirementSubmitData: ScheduledEventQualificationRequirementSubmitData | null = null;
  ...
  openModal(scheduledEventQualificationRequirementData?: ScheduledEventQualificationRequirementData) {
    ...
    if (scheduledEventQualificationRequirementData != null) {
      ...
      if (!this.scheduledEventQualificationRequirementService.userIsSchedulerScheduledEventQualificationRequirementReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduled Event Qualification Requirements`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.scheduledEventQualificationRequirementSubmitData = this.scheduledEventQualificationRequirementService.ConvertToScheduledEventQualificationRequirementSubmitData(scheduledEventQualificationRequirementData);
      this.isEditMode = true;
      this.objectGuid = scheduledEventQualificationRequirementData.objectGuid;

      this.buildFormValues(scheduledEventQualificationRequirementData);

    } else {

      if (!this.scheduledEventQualificationRequirementService.userIsSchedulerScheduledEventQualificationRequirementWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduled Event Qualification Requirements`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.scheduledEventQualificationRequirementModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const scheduledEventQualificationRequirementSubmitData: ScheduledEventQualificationRequirementSubmitData = {
        id: this.scheduledEventQualificationRequirementSubmitData?.id || 0,
        scheduledEventId: Number(formValue.scheduledEventId),
        qualificationId: Number(formValue.qualificationId),
        versionNumber: this.scheduledEventQualificationRequirementSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateScheduledEventQualificationRequirement(scheduledEventQualificationRequirementSubmitData);
      } else {
        this.addScheduledEventQualificationRequirement(scheduledEventQualificationRequirementSubmitData);
      }
  }

  private addScheduledEventQualificationRequirement(scheduledEventQualificationRequirementData: ScheduledEventQualificationRequirementSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    scheduledEventQualificationRequirementData.versionNumber = 0;
    scheduledEventQualificationRequirementData.active = true;
    scheduledEventQualificationRequirementData.deleted = false;
    this.scheduledEventQualificationRequirementService.PostScheduledEventQualificationRequirement(scheduledEventQualificationRequirementData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newScheduledEventQualificationRequirement) => {

        this.scheduledEventQualificationRequirementService.ClearAllCaches();

        this.scheduledEventQualificationRequirementChanged.next([newScheduledEventQualificationRequirement]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/scheduledeventqualificationrequirement', newScheduledEventQualificationRequirement.id]);
        }
      },
      error: (err) => {
  ...
  private updateScheduledEventQualificationRequirement(scheduledEventQualificationRequirementData: ScheduledEventQualificationRequirementSubmitData) {
    this.scheduledEventQualificationRequirementService.PutScheduledEventQualificationRequirement(scheduledEventQualificationRequirementData.id, scheduledEventQualificationRequirementData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedScheduledEventQualificationRequirement) => {

        this.scheduledEventQualificationRequirementService.ClearAllCaches();

        this.scheduledEventQualificationRequirementChanged.next([updatedScheduledEventQualificationRequirement]);

        this.alertService.showMessage("Scheduled Event Qualification Requirement updated successfully", '', MessageSeverity.success);
