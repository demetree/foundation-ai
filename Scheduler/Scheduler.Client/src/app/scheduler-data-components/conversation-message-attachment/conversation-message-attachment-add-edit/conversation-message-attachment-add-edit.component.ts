...
@Component({
  selector: 'app-conversation-message-attachment-add-edit',
  templateUrl: './conversation-message-attachment-add-edit.component.html',
  styleUrls: ['./conversation-message-attachment-add-edit.component.scss']
})
export class ConversationMessageAttachmentAddEditComponent {
  @ViewChild('conversationMessageAttachmentModal') conversationMessageAttachmentModal!: TemplateRef<any>;
  @Output() conversationMessageAttachmentChanged = new Subject<ConversationMessageAttachmentData[]>();
  @Input() conversationMessageAttachmentSubmitData: ConversationMessageAttachmentSubmitData | null = null;
  ...
  openModal(conversationMessageAttachmentData?: ConversationMessageAttachmentData) {
    ...
    if (conversationMessageAttachmentData != null) {
      ...
      if (!this.conversationMessageAttachmentService.userIsSchedulerConversationMessageAttachmentReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Conversation Message Attachments`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.conversationMessageAttachmentSubmitData = this.conversationMessageAttachmentService.ConvertToConversationMessageAttachmentSubmitData(conversationMessageAttachmentData);
      this.isEditMode = true;
      this.objectGuid = conversationMessageAttachmentData.objectGuid;

      this.buildFormValues(conversationMessageAttachmentData);

    } else {

      if (!this.conversationMessageAttachmentService.userIsSchedulerConversationMessageAttachmentWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Conversation Message Attachments`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.conversationMessageAttachmentModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const conversationMessageAttachmentSubmitData: ConversationMessageAttachmentSubmitData = {
        id: this.conversationMessageAttachmentSubmitData?.id || 0,
        ...
        versionNumber: this.conversationMessageAttachmentSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateConversationMessageAttachment(conversationMessageAttachmentSubmitData);
      } else {
        this.addConversationMessageAttachment(conversationMessageAttachmentSubmitData);
      }
  }

  private addConversationMessageAttachment(conversationMessageAttachmentData: ConversationMessageAttachmentSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    conversationMessageAttachmentData.versionNumber = 0;
    conversationMessageAttachmentData.active = true;
    conversationMessageAttachmentData.deleted = false;
    this.conversationMessageAttachmentService.PostConversationMessageAttachment(conversationMessageAttachmentData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newConversationMessageAttachment) => {

        this.conversationMessageAttachmentService.ClearAllCaches();

        this.conversationMessageAttachmentChanged.next([newConversationMessageAttachment]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/conversationmessageattachment', newConversationMessageAttachment.id]);
        }
      },
      error: (err) => {
  ...
  private updateConversationMessageAttachment(conversationMessageAttachmentData: ConversationMessageAttachmentSubmitData) {
    this.conversationMessageAttachmentService.PutConversationMessageAttachment(conversationMessageAttachmentData.id, conversationMessageAttachmentData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedConversationMessageAttachment) => {

        this.conversationMessageAttachmentService.ClearAllCaches();

        this.conversationMessageAttachmentChanged.next([updatedConversationMessageAttachment]);

        this.alertService.showMessage("Conversation Message Attachment updated successfully", '', MessageSeverity.success);
