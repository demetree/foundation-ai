...
@Component({
  selector: 'app-scheduled-event-template-add-edit',
  templateUrl: './scheduled-event-template-add-edit.component.html',
  styleUrls: ['./scheduled-event-template-add-edit.component.scss']
})
export class ScheduledEventTemplateAddEditComponent {
  @ViewChild('scheduledEventTemplateModal') scheduledEventTemplateModal!: TemplateRef<any>;
  @Output() scheduledEventTemplateChanged = new Subject<ScheduledEventTemplateData[]>();
  @Input() scheduledEventTemplateSubmitData: ScheduledEventTemplateSubmitData | null = null;
  ...
  openModal(scheduledEventTemplateData?: ScheduledEventTemplateData) {

    if (scheduledEventTemplateData != null) {

      if (!this.scheduledEventTemplateService.userIsSchedulerScheduledEventTemplateReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduled Event Templates`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.scheduledEventTemplateSubmitData = this.scheduledEventTemplateService.ConvertToScheduledEventTemplateSubmitData(scheduledEventTemplateData);
      this.isEditMode = true;
      this.objectGuid = scheduledEventTemplateData.objectGuid;

      this.buildFormValues(scheduledEventTemplateData);

    } else {

      if (!this.scheduledEventTemplateService.userIsSchedulerScheduledEventTemplateWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduled Event Templates`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.scheduledEventTemplateModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const scheduledEventTemplateSubmitData: ScheduledEventTemplateSubmitData = {
        id: this.scheduledEventTemplateSubmitData?.id || 0,
        ...
        versionNumber: this.scheduledEventTemplateSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateScheduledEventTemplate(scheduledEventTemplateSubmitData);
      } else {
        this.addScheduledEventTemplate(scheduledEventTemplateSubmitData);
      }
  }

  private addScheduledEventTemplate(scheduledEventTemplateData: ScheduledEventTemplateSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    scheduledEventTemplateData.versionNumber = 0;
    scheduledEventTemplateData.active = true;
    scheduledEventTemplateData.deleted = false;
    this.scheduledEventTemplateService.PostScheduledEventTemplate(scheduledEventTemplateData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newScheduledEventTemplate) => {

        this.scheduledEventTemplateService.ClearAllCaches();

        this.scheduledEventTemplateChanged.next([newScheduledEventTemplate]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/scheduledeventtemplate', newScheduledEventTemplate.id]);
        }
      },
      error: (err) => {
  ...
  private updateScheduledEventTemplate(scheduledEventTemplateData: ScheduledEventTemplateSubmitData) {
    this.scheduledEventTemplateService.PutScheduledEventTemplate(scheduledEventTemplateData.id, scheduledEventTemplateData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedScheduledEventTemplate) => {

        this.scheduledEventTemplateService.ClearAllCaches();

        this.scheduledEventTemplateChanged.next([updatedScheduledEventTemplate]);

        this.alertService.showMessage("Scheduled Event Template updated successfully", '', MessageSeverity.success);
