...
@Component({
  selector: 'app-scheduled-event-template-change-history-add-edit',
  templateUrl: './scheduled-event-template-change-history-add-edit.component.html',
  styleUrls: ['./scheduled-event-template-change-history-add-edit.component.scss']
})
export class ScheduledEventTemplateChangeHistoryAddEditComponent {
  @ViewChild('scheduledEventTemplateChangeHistoryModal') scheduledEventTemplateChangeHistoryModal!: TemplateRef<any>;
  @Output() scheduledEventTemplateChangeHistoryChanged = new Subject<ScheduledEventTemplateChangeHistoryData[]>();
  @Input() scheduledEventTemplateChangeHistorySubmitData: ScheduledEventTemplateChangeHistorySubmitData | null = null;
  ...
  openModal(scheduledEventTemplateChangeHistoryData?: ScheduledEventTemplateChangeHistoryData) {
    ...
    if (scheduledEventTemplateChangeHistoryData != null) {
      ...
      if (!this.scheduledEventTemplateChangeHistoryService.userIsSchedulerScheduledEventTemplateChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduled Event Template Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.scheduledEventTemplateChangeHistorySubmitData = this.scheduledEventTemplateChangeHistoryService.ConvertToScheduledEventTemplateChangeHistorySubmitData(scheduledEventTemplateChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(scheduledEventTemplateChangeHistoryData);

    } else {

      if (!this.scheduledEventTemplateChangeHistoryService.userIsSchedulerScheduledEventTemplateChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduled Event Template Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.scheduledEventTemplateChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const scheduledEventTemplateChangeHistorySubmitData: ScheduledEventTemplateChangeHistorySubmitData = {
        id: this.scheduledEventTemplateChangeHistorySubmitData?.id || 0,
        scheduledEventTemplateId: Number(formValue.scheduledEventTemplateId),
        versionNumber: this.scheduledEventTemplateChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistorySubmitData);
      } else {
        this.addScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistorySubmitData);
      }
  }

  private addScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistoryData: ScheduledEventTemplateChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    scheduledEventTemplateChangeHistoryData.versionNumber = 0;
    this.scheduledEventTemplateChangeHistoryService.PostScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newScheduledEventTemplateChangeHistory) => {

        this.scheduledEventTemplateChangeHistoryService.ClearAllCaches();

        this.scheduledEventTemplateChangeHistoryChanged.next([newScheduledEventTemplateChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/scheduledeventtemplatechangehistory', newScheduledEventTemplateChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistoryData: ScheduledEventTemplateChangeHistorySubmitData) {
    this.scheduledEventTemplateChangeHistoryService.PutScheduledEventTemplateChangeHistory(scheduledEventTemplateChangeHistoryData.id, scheduledEventTemplateChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedScheduledEventTemplateChangeHistory) => {

        this.scheduledEventTemplateChangeHistoryService.ClearAllCaches();

        this.scheduledEventTemplateChangeHistoryChanged.next([updatedScheduledEventTemplateChangeHistory]);

        this.alertService.showMessage("Scheduled Event Template Change History updated successfully", '', MessageSeverity.success);
