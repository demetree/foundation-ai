...
export class CalendarDetailComponent implements OnInit, CanComponentDeactivate {
  ...
  public calendarChangeHistories$ = this.calendarChangeHistoryService.GetCalendarChangeHistoryList();
  ...
  public canDeactivate(): boolean {
    if (this.calendarForm.dirty) {
      return confirm('You have unsaved Calendar changes. Are you sure you want to leave this page?');
    }
    return true;
  }
  ...
  */
    ...
    this.calendarService.GetCalendar(calendarId, true).pipe(
      ...
      next: (calendarData) => {
        ...
        if (!calendarData) {
          ...
          this.buildFormValues(this.calendarData);
  ...
  public submitForm() {
    ...
    if (this.calendarService.userIsSchedulerCalendarWriter() == false) {
      this.alertService.showMessage(this.authService.currentUser?.userName + " does not have the permission to write to Calendars", 'Access Denied', MessageSeverity.info);
      return;
    }
    ...
    const calendarSubmitData: CalendarSubmitData = {
        ...
        versionNumber: this.calendarData?.versionNumber ?? 0,
   };
    ...
    const saveObservable = this.isEditMode
      ? this.calendarService.PutCalendar(calendarSubmitData.id, calendarSubmitData)
      : this.calendarService.PostCalendar(calendarSubmitData);


    saveObservable.pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (savedCalendarData) => {

        this.calendarService.ClearAllCaches();       // Clear the data service cache because we know we have changed the data.

        if (!this.isEditMode) {
          //
          // Navigate to the newly created Calendar's detail page
          //
          this.calendarForm.markAsPristine();     // Set the form to new state so the deactivate guard won't complain during routing
          this.calendarForm.markAsUntouched();

          this.router.navigate(['/calendars', savedCalendarData.id]);
          ...
          //
          this.calendarData = savedCalendarData;
          this.buildFormValues(this.calendarData);

          this.alertService.showMessage("Calendar saved successfully", '', MessageSeverity.success);
        }
      },
      error: (err) => {
