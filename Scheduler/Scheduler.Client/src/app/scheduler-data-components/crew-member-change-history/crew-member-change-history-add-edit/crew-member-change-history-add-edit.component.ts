...
@Component({
  selector: 'app-crew-member-change-history-add-edit',
  templateUrl: './crew-member-change-history-add-edit.component.html',
  styleUrls: ['./crew-member-change-history-add-edit.component.scss']
})
export class CrewMemberChangeHistoryAddEditComponent {
  @ViewChild('crewMemberChangeHistoryModal') crewMemberChangeHistoryModal!: TemplateRef<any>;
  @Output() crewMemberChangeHistoryChanged = new Subject<CrewMemberChangeHistoryData[]>();
  @Input() crewMemberChangeHistorySubmitData: CrewMemberChangeHistorySubmitData | null = null;
  ...
  openModal(crewMemberChangeHistoryData?: CrewMemberChangeHistoryData) {
    ...
    if (crewMemberChangeHistoryData != null) {
      ...
      if (!this.crewMemberChangeHistoryService.userIsSchedulerCrewMemberChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Crew Member Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.crewMemberChangeHistorySubmitData = this.crewMemberChangeHistoryService.ConvertToCrewMemberChangeHistorySubmitData(crewMemberChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(crewMemberChangeHistoryData);

    } else {

      if (!this.crewMemberChangeHistoryService.userIsSchedulerCrewMemberChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Crew Member Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.crewMemberChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const crewMemberChangeHistorySubmitData: CrewMemberChangeHistorySubmitData = {
        id: this.crewMemberChangeHistorySubmitData?.id || 0,
        crewMemberId: Number(formValue.crewMemberId),
        versionNumber: this.crewMemberChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateCrewMemberChangeHistory(crewMemberChangeHistorySubmitData);
      } else {
        this.addCrewMemberChangeHistory(crewMemberChangeHistorySubmitData);
      }
  }

  private addCrewMemberChangeHistory(crewMemberChangeHistoryData: CrewMemberChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    crewMemberChangeHistoryData.versionNumber = 0;
    this.crewMemberChangeHistoryService.PostCrewMemberChangeHistory(crewMemberChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newCrewMemberChangeHistory) => {

        this.crewMemberChangeHistoryService.ClearAllCaches();

        this.crewMemberChangeHistoryChanged.next([newCrewMemberChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/crewmemberchangehistory', newCrewMemberChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateCrewMemberChangeHistory(crewMemberChangeHistoryData: CrewMemberChangeHistorySubmitData) {
    this.crewMemberChangeHistoryService.PutCrewMemberChangeHistory(crewMemberChangeHistoryData.id, crewMemberChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedCrewMemberChangeHistory) => {

        this.crewMemberChangeHistoryService.ClearAllCaches();

        this.crewMemberChangeHistoryChanged.next([updatedCrewMemberChangeHistory]);

        this.alertService.showMessage("Crew Member Change History updated successfully", '', MessageSeverity.success);
