...
@Component({
  selector: 'app-receipt-change-history-add-edit',
  templateUrl: './receipt-change-history-add-edit.component.html',
  styleUrls: ['./receipt-change-history-add-edit.component.scss']
})
export class ReceiptChangeHistoryAddEditComponent {
  @ViewChild('receiptChangeHistoryModal') receiptChangeHistoryModal!: TemplateRef<any>;
  @Output() receiptChangeHistoryChanged = new Subject<ReceiptChangeHistoryData[]>();
  @Input() receiptChangeHistorySubmitData: ReceiptChangeHistorySubmitData | null = null;
  ...
  openModal(receiptChangeHistoryData?: ReceiptChangeHistoryData) {

    if (receiptChangeHistoryData != null) {

      if (!this.receiptChangeHistoryService.userIsSchedulerReceiptChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Receipt Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.receiptChangeHistorySubmitData = this.receiptChangeHistoryService.ConvertToReceiptChangeHistorySubmitData(receiptChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(receiptChangeHistoryData);

    } else {

      if (!this.receiptChangeHistoryService.userIsSchedulerReceiptChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Receipt Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.receiptChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const receiptChangeHistorySubmitData: ReceiptChangeHistorySubmitData = {
        id: this.receiptChangeHistorySubmitData?.id || 0,
        receiptId: Number(formValue.receiptId),
        versionNumber: this.receiptChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateReceiptChangeHistory(receiptChangeHistorySubmitData);
      } else {
        this.addReceiptChangeHistory(receiptChangeHistorySubmitData);
      }
  }

  private addReceiptChangeHistory(receiptChangeHistoryData: ReceiptChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    receiptChangeHistoryData.versionNumber = 0;
    this.receiptChangeHistoryService.PostReceiptChangeHistory(receiptChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newReceiptChangeHistory) => {

        this.receiptChangeHistoryService.ClearAllCaches();

        this.receiptChangeHistoryChanged.next([newReceiptChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/receiptchangehistory', newReceiptChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateReceiptChangeHistory(receiptChangeHistoryData: ReceiptChangeHistorySubmitData) {
    this.receiptChangeHistoryService.PutReceiptChangeHistory(receiptChangeHistoryData.id, receiptChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedReceiptChangeHistory) => {

        this.receiptChangeHistoryService.ClearAllCaches();

        this.receiptChangeHistoryChanged.next([updatedReceiptChangeHistory]);

        this.alertService.showMessage("Receipt Change History updated successfully", '', MessageSeverity.success);
