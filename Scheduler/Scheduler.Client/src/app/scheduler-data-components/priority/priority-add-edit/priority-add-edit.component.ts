...
@Component({
  selector: 'app-priority-add-edit',
  templateUrl: './priority-add-edit.component.html',
  styleUrls: ['./priority-add-edit.component.scss']
})
export class PriorityAddEditComponent {
  @ViewChild('priorityModal') priorityModal!: TemplateRef<any>;
  @Output() priorityChanged = new Subject<PriorityData[]>();
  @Input() prioritySubmitData: PrioritySubmitData | null = null;
  ...
  openModal(priorityData?: PriorityData) {

    if (priorityData != null) {

      if (!this.priorityService.userIsSchedulerPriorityReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Priorities`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.prioritySubmitData = this.priorityService.ConvertToPrioritySubmitData(priorityData);
      this.isEditMode = true;
      this.objectGuid = priorityData.objectGuid;

      this.buildFormValues(priorityData);

    } else {

      if (!this.priorityService.userIsSchedulerPriorityWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Priorities`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.priorityModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const prioritySubmitData: PrioritySubmitData = {
        id: this.prioritySubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updatePriority(prioritySubmitData);
      } else {
        this.addPriority(prioritySubmitData);
      }
  }

  private addPriority(priorityData: PrioritySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    priorityData.active = true;
    priorityData.deleted = false;
    this.priorityService.PostPriority(priorityData).pipe(
      ...
      next: (newPriority) => {
        ...
        this.priorityChanged.next([newPriority]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/priority', newPriority.id]);
        }
      },
      error: (err) => {
  ...
  private updatePriority(priorityData: PrioritySubmitData) {
    this.priorityService.PutPriority(priorityData.id, priorityData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedPriority) => {
        ...
        this.priorityChanged.next([updatedPriority]);

        this.alertService.showMessage("Priority updated successfully", '', MessageSeverity.success);
