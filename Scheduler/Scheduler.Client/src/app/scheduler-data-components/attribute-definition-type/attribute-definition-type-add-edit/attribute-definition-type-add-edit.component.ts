...
@Component({
  selector: 'app-attribute-definition-type-add-edit',
  templateUrl: './attribute-definition-type-add-edit.component.html',
  styleUrls: ['./attribute-definition-type-add-edit.component.scss']
})
export class AttributeDefinitionTypeAddEditComponent {
  @ViewChild('attributeDefinitionTypeModal') attributeDefinitionTypeModal!: TemplateRef<any>;
  @Output() attributeDefinitionTypeChanged = new Subject<AttributeDefinitionTypeData[]>();
  @Input() attributeDefinitionTypeSubmitData: AttributeDefinitionTypeSubmitData | null = null;
  ...
  openModal(attributeDefinitionTypeData?: AttributeDefinitionTypeData) {
    ...
    if (attributeDefinitionTypeData != null) {
      ...
      if (!this.attributeDefinitionTypeService.userIsSchedulerAttributeDefinitionTypeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Attribute Definition Types`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.attributeDefinitionTypeSubmitData = this.attributeDefinitionTypeService.ConvertToAttributeDefinitionTypeSubmitData(attributeDefinitionTypeData);
      this.isEditMode = true;
      this.objectGuid = attributeDefinitionTypeData.objectGuid;

      this.buildFormValues(attributeDefinitionTypeData);

    } else {

      if (!this.attributeDefinitionTypeService.userIsSchedulerAttributeDefinitionTypeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Attribute Definition Types`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.attributeDefinitionTypeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const attributeDefinitionTypeSubmitData: AttributeDefinitionTypeSubmitData = {
        id: this.attributeDefinitionTypeSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateAttributeDefinitionType(attributeDefinitionTypeSubmitData);
      } else {
        this.addAttributeDefinitionType(attributeDefinitionTypeSubmitData);
      }
  }

  private addAttributeDefinitionType(attributeDefinitionTypeData: AttributeDefinitionTypeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    attributeDefinitionTypeData.active = true;
    attributeDefinitionTypeData.deleted = false;
    this.attributeDefinitionTypeService.PostAttributeDefinitionType(attributeDefinitionTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newAttributeDefinitionType) => {

        this.attributeDefinitionTypeService.ClearAllCaches();

        this.attributeDefinitionTypeChanged.next([newAttributeDefinitionType]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/attributedefinitiontype', newAttributeDefinitionType.id]);
        }
      },
      error: (err) => {
  ...
  private updateAttributeDefinitionType(attributeDefinitionTypeData: AttributeDefinitionTypeSubmitData) {
    this.attributeDefinitionTypeService.PutAttributeDefinitionType(attributeDefinitionTypeData.id, attributeDefinitionTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedAttributeDefinitionType) => {

        this.attributeDefinitionTypeService.ClearAllCaches();

        this.attributeDefinitionTypeChanged.next([updatedAttributeDefinitionType]);

        this.alertService.showMessage("Attribute Definition Type updated successfully", '', MessageSeverity.success);
