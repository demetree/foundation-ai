...
@Component({
  selector: 'app-document-share-link-change-history-add-edit',
  templateUrl: './document-share-link-change-history-add-edit.component.html',
  styleUrls: ['./document-share-link-change-history-add-edit.component.scss']
})
export class DocumentShareLinkChangeHistoryAddEditComponent {
  @ViewChild('documentShareLinkChangeHistoryModal') documentShareLinkChangeHistoryModal!: TemplateRef<any>;
  @Output() documentShareLinkChangeHistoryChanged = new Subject<DocumentShareLinkChangeHistoryData[]>();
  @Input() documentShareLinkChangeHistorySubmitData: DocumentShareLinkChangeHistorySubmitData | null = null;
  ...
  openModal(documentShareLinkChangeHistoryData?: DocumentShareLinkChangeHistoryData) {
    ...
    if (documentShareLinkChangeHistoryData != null) {
      ...
      if (!this.documentShareLinkChangeHistoryService.userIsSchedulerDocumentShareLinkChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Document Share Link Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.documentShareLinkChangeHistorySubmitData = this.documentShareLinkChangeHistoryService.ConvertToDocumentShareLinkChangeHistorySubmitData(documentShareLinkChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(documentShareLinkChangeHistoryData);

    } else {

      if (!this.documentShareLinkChangeHistoryService.userIsSchedulerDocumentShareLinkChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Document Share Link Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.documentShareLinkChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const documentShareLinkChangeHistorySubmitData: DocumentShareLinkChangeHistorySubmitData = {
        id: this.documentShareLinkChangeHistorySubmitData?.id || 0,
        documentShareLinkId: Number(formValue.documentShareLinkId),
        versionNumber: this.documentShareLinkChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateDocumentShareLinkChangeHistory(documentShareLinkChangeHistorySubmitData);
      } else {
        this.addDocumentShareLinkChangeHistory(documentShareLinkChangeHistorySubmitData);
      }
  }

  private addDocumentShareLinkChangeHistory(documentShareLinkChangeHistoryData: DocumentShareLinkChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    documentShareLinkChangeHistoryData.versionNumber = 0;
    this.documentShareLinkChangeHistoryService.PostDocumentShareLinkChangeHistory(documentShareLinkChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newDocumentShareLinkChangeHistory) => {

        this.documentShareLinkChangeHistoryService.ClearAllCaches();

        this.documentShareLinkChangeHistoryChanged.next([newDocumentShareLinkChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/documentsharelinkchangehistory', newDocumentShareLinkChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateDocumentShareLinkChangeHistory(documentShareLinkChangeHistoryData: DocumentShareLinkChangeHistorySubmitData) {
    this.documentShareLinkChangeHistoryService.PutDocumentShareLinkChangeHistory(documentShareLinkChangeHistoryData.id, documentShareLinkChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedDocumentShareLinkChangeHistory) => {

        this.documentShareLinkChangeHistoryService.ClearAllCaches();

        this.documentShareLinkChangeHistoryChanged.next([updatedDocumentShareLinkChangeHistory]);

        this.alertService.showMessage("Document Share Link Change History updated successfully", '', MessageSeverity.success);
