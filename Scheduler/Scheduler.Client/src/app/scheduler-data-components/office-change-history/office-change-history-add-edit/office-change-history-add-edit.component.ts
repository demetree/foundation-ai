...
@Component({
  selector: 'app-office-change-history-add-edit',
  templateUrl: './office-change-history-add-edit.component.html',
  styleUrls: ['./office-change-history-add-edit.component.scss']
})
export class OfficeChangeHistoryAddEditComponent {
  @ViewChild('officeChangeHistoryModal') officeChangeHistoryModal!: TemplateRef<any>;
  @Output() officeChangeHistoryChanged = new Subject<OfficeChangeHistoryData[]>();
  @Input() officeChangeHistorySubmitData: OfficeChangeHistorySubmitData | null = null;
  ...
  openModal(officeChangeHistoryData?: OfficeChangeHistoryData) {

    if (officeChangeHistoryData != null) {

      if (!this.officeChangeHistoryService.userIsSchedulerOfficeChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Office Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.officeChangeHistorySubmitData = this.officeChangeHistoryService.ConvertToOfficeChangeHistorySubmitData(officeChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(officeChangeHistoryData);

    } else {

      if (!this.officeChangeHistoryService.userIsSchedulerOfficeChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Office Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.officeChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const officeChangeHistorySubmitData: OfficeChangeHistorySubmitData = {
        id: this.officeChangeHistorySubmitData?.id || 0,
        officeId: Number(formValue.officeId),
        versionNumber: this.officeChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateOfficeChangeHistory(officeChangeHistorySubmitData);
      } else {
        this.addOfficeChangeHistory(officeChangeHistorySubmitData);
      }
  }

  private addOfficeChangeHistory(officeChangeHistoryData: OfficeChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    officeChangeHistoryData.versionNumber = 0;
    this.officeChangeHistoryService.PostOfficeChangeHistory(officeChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newOfficeChangeHistory) => {

        this.officeChangeHistoryService.ClearAllCaches();

        this.officeChangeHistoryChanged.next([newOfficeChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/officechangehistory', newOfficeChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateOfficeChangeHistory(officeChangeHistoryData: OfficeChangeHistorySubmitData) {
    this.officeChangeHistoryService.PutOfficeChangeHistory(officeChangeHistoryData.id, officeChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedOfficeChangeHistory) => {

        this.officeChangeHistoryService.ClearAllCaches();

        this.officeChangeHistoryChanged.next([updatedOfficeChangeHistory]);

        this.alertService.showMessage("Office Change History updated successfully", '', MessageSeverity.success);
