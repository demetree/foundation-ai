...
@Component({
  selector: 'app-contact-contact-change-history-add-edit',
  templateUrl: './contact-contact-change-history-add-edit.component.html',
  styleUrls: ['./contact-contact-change-history-add-edit.component.scss']
})
export class ContactContactChangeHistoryAddEditComponent {
  @ViewChild('contactContactChangeHistoryModal') contactContactChangeHistoryModal!: TemplateRef<any>;
  @Output() contactContactChangeHistoryChanged = new Subject<ContactContactChangeHistoryData[]>();
  @Input() contactContactChangeHistorySubmitData: ContactContactChangeHistorySubmitData | null = null;
  ...
  openModal(contactContactChangeHistoryData?: ContactContactChangeHistoryData) {
    ...
    if (contactContactChangeHistoryData != null) {
      ...
      if (!this.contactContactChangeHistoryService.userIsSchedulerContactContactChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Contact Contact Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.contactContactChangeHistorySubmitData = this.contactContactChangeHistoryService.ConvertToContactContactChangeHistorySubmitData(contactContactChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(contactContactChangeHistoryData);

    } else {

      if (!this.contactContactChangeHistoryService.userIsSchedulerContactContactChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Contact Contact Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.contactContactChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const contactContactChangeHistorySubmitData: ContactContactChangeHistorySubmitData = {
        id: this.contactContactChangeHistorySubmitData?.id || 0,
        contactContactId: Number(formValue.contactContactId),
        versionNumber: this.contactContactChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateContactContactChangeHistory(contactContactChangeHistorySubmitData);
      } else {
        this.addContactContactChangeHistory(contactContactChangeHistorySubmitData);
      }
  }

  private addContactContactChangeHistory(contactContactChangeHistoryData: ContactContactChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    contactContactChangeHistoryData.versionNumber = 0;
    this.contactContactChangeHistoryService.PostContactContactChangeHistory(contactContactChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newContactContactChangeHistory) => {

        this.contactContactChangeHistoryService.ClearAllCaches();

        this.contactContactChangeHistoryChanged.next([newContactContactChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/contactcontactchangehistory', newContactContactChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateContactContactChangeHistory(contactContactChangeHistoryData: ContactContactChangeHistorySubmitData) {
    this.contactContactChangeHistoryService.PutContactContactChangeHistory(contactContactChangeHistoryData.id, contactContactChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedContactContactChangeHistory) => {

        this.contactContactChangeHistoryService.ClearAllCaches();

        this.contactContactChangeHistoryChanged.next([updatedContactContactChangeHistory]);

        this.alertService.showMessage("Contact Contact Change History updated successfully", '', MessageSeverity.success);
