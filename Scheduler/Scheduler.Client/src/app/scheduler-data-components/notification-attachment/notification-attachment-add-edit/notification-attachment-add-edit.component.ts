...
@Component({
  selector: 'app-notification-attachment-add-edit',
  templateUrl: './notification-attachment-add-edit.component.html',
  styleUrls: ['./notification-attachment-add-edit.component.scss']
})
export class NotificationAttachmentAddEditComponent {
  @ViewChild('notificationAttachmentModal') notificationAttachmentModal!: TemplateRef<any>;
  @Output() notificationAttachmentChanged = new Subject<NotificationAttachmentData[]>();
  @Input() notificationAttachmentSubmitData: NotificationAttachmentSubmitData | null = null;
  ...
  openModal(notificationAttachmentData?: NotificationAttachmentData) {

    if (notificationAttachmentData != null) {

      if (!this.notificationAttachmentService.userIsSchedulerNotificationAttachmentReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Notification Attachments`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.notificationAttachmentSubmitData = this.notificationAttachmentService.ConvertToNotificationAttachmentSubmitData(notificationAttachmentData);
      this.isEditMode = true;
      this.objectGuid = notificationAttachmentData.objectGuid;

      this.buildFormValues(notificationAttachmentData);

    } else {

      if (!this.notificationAttachmentService.userIsSchedulerNotificationAttachmentWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Notification Attachments`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.notificationAttachmentModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const notificationAttachmentSubmitData: NotificationAttachmentSubmitData = {
        id: this.notificationAttachmentSubmitData?.id || 0,
        ...
        versionNumber: this.notificationAttachmentSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateNotificationAttachment(notificationAttachmentSubmitData);
      } else {
        this.addNotificationAttachment(notificationAttachmentSubmitData);
      }
  }

  private addNotificationAttachment(notificationAttachmentData: NotificationAttachmentSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    notificationAttachmentData.versionNumber = 0;
    notificationAttachmentData.active = true;
    notificationAttachmentData.deleted = false;
    this.notificationAttachmentService.PostNotificationAttachment(notificationAttachmentData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newNotificationAttachment) => {

        this.notificationAttachmentService.ClearAllCaches();

        this.notificationAttachmentChanged.next([newNotificationAttachment]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/notificationattachment', newNotificationAttachment.id]);
        }
      },
      error: (err) => {
  ...
  private updateNotificationAttachment(notificationAttachmentData: NotificationAttachmentSubmitData) {
    this.notificationAttachmentService.PutNotificationAttachment(notificationAttachmentData.id, notificationAttachmentData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedNotificationAttachment) => {

        this.notificationAttachmentService.ClearAllCaches();

        this.notificationAttachmentChanged.next([updatedNotificationAttachment]);

        this.alertService.showMessage("Notification Attachment updated successfully", '', MessageSeverity.success);
