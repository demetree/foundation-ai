...
@Component({
  selector: 'app-scheduling-target-change-history-add-edit',
  templateUrl: './scheduling-target-change-history-add-edit.component.html',
  styleUrls: ['./scheduling-target-change-history-add-edit.component.scss']
})
export class SchedulingTargetChangeHistoryAddEditComponent {
  @ViewChild('schedulingTargetChangeHistoryModal') schedulingTargetChangeHistoryModal!: TemplateRef<any>;
  @Output() schedulingTargetChangeHistoryChanged = new Subject<SchedulingTargetChangeHistoryData[]>();
  @Input() schedulingTargetChangeHistorySubmitData: SchedulingTargetChangeHistorySubmitData | null = null;
  ...
  openModal(schedulingTargetChangeHistoryData?: SchedulingTargetChangeHistoryData) {
    ...
    if (schedulingTargetChangeHistoryData != null) {
      ...
      if (!this.schedulingTargetChangeHistoryService.userIsSchedulerSchedulingTargetChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduling Target Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.schedulingTargetChangeHistorySubmitData = this.schedulingTargetChangeHistoryService.ConvertToSchedulingTargetChangeHistorySubmitData(schedulingTargetChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(schedulingTargetChangeHistoryData);

    } else {

      if (!this.schedulingTargetChangeHistoryService.userIsSchedulerSchedulingTargetChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduling Target Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.schedulingTargetChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const schedulingTargetChangeHistorySubmitData: SchedulingTargetChangeHistorySubmitData = {
        id: this.schedulingTargetChangeHistorySubmitData?.id || 0,
        schedulingTargetId: Number(formValue.schedulingTargetId),
        versionNumber: this.schedulingTargetChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateSchedulingTargetChangeHistory(schedulingTargetChangeHistorySubmitData);
      } else {
        this.addSchedulingTargetChangeHistory(schedulingTargetChangeHistorySubmitData);
      }
  }

  private addSchedulingTargetChangeHistory(schedulingTargetChangeHistoryData: SchedulingTargetChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    schedulingTargetChangeHistoryData.versionNumber = 0;
    this.schedulingTargetChangeHistoryService.PostSchedulingTargetChangeHistory(schedulingTargetChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newSchedulingTargetChangeHistory) => {

        this.schedulingTargetChangeHistoryService.ClearAllCaches();

        this.schedulingTargetChangeHistoryChanged.next([newSchedulingTargetChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/schedulingtargetchangehistory', newSchedulingTargetChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateSchedulingTargetChangeHistory(schedulingTargetChangeHistoryData: SchedulingTargetChangeHistorySubmitData) {
    this.schedulingTargetChangeHistoryService.PutSchedulingTargetChangeHistory(schedulingTargetChangeHistoryData.id, schedulingTargetChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedSchedulingTargetChangeHistory) => {

        this.schedulingTargetChangeHistoryService.ClearAllCaches();

        this.schedulingTargetChangeHistoryChanged.next([updatedSchedulingTargetChangeHistory]);

        this.alertService.showMessage("Scheduling Target Change History updated successfully", '', MessageSeverity.success);
