...
@Component({
  selector: 'app-client-change-history-add-edit',
  templateUrl: './client-change-history-add-edit.component.html',
  styleUrls: ['./client-change-history-add-edit.component.scss']
})
export class ClientChangeHistoryAddEditComponent {
  @ViewChild('clientChangeHistoryModal') clientChangeHistoryModal!: TemplateRef<any>;
  @Output() clientChangeHistoryChanged = new Subject<ClientChangeHistoryData[]>();
  @Input() clientChangeHistorySubmitData: ClientChangeHistorySubmitData | null = null;
  ...
  openModal(clientChangeHistoryData?: ClientChangeHistoryData) {

    if (clientChangeHistoryData != null) {

      if (!this.clientChangeHistoryService.userIsSchedulerClientChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Client Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.clientChangeHistorySubmitData = this.clientChangeHistoryService.ConvertToClientChangeHistorySubmitData(clientChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(clientChangeHistoryData);

    } else {

      if (!this.clientChangeHistoryService.userIsSchedulerClientChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Client Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.clientChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const clientChangeHistorySubmitData: ClientChangeHistorySubmitData = {
        id: this.clientChangeHistorySubmitData?.id || 0,
        clientId: Number(formValue.clientId),
        versionNumber: this.clientChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateClientChangeHistory(clientChangeHistorySubmitData);
      } else {
        this.addClientChangeHistory(clientChangeHistorySubmitData);
      }
  }

  private addClientChangeHistory(clientChangeHistoryData: ClientChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    clientChangeHistoryData.versionNumber = 0;
    this.clientChangeHistoryService.PostClientChangeHistory(clientChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newClientChangeHistory) => {

        this.clientChangeHistoryService.ClearAllCaches();

        this.clientChangeHistoryChanged.next([newClientChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/clientchangehistory', newClientChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateClientChangeHistory(clientChangeHistoryData: ClientChangeHistorySubmitData) {
    this.clientChangeHistoryService.PutClientChangeHistory(clientChangeHistoryData.id, clientChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedClientChangeHistory) => {

        this.clientChangeHistoryService.ClearAllCaches();

        this.clientChangeHistoryChanged.next([updatedClientChangeHistory]);

        this.alertService.showMessage("Client Change History updated successfully", '', MessageSeverity.success);
