...
@Component({
  selector: 'app-batch-change-history-add-edit',
  templateUrl: './batch-change-history-add-edit.component.html',
  styleUrls: ['./batch-change-history-add-edit.component.scss']
})
export class BatchChangeHistoryAddEditComponent {
  @ViewChild('batchChangeHistoryModal') batchChangeHistoryModal!: TemplateRef<any>;
  @Output() batchChangeHistoryChanged = new Subject<BatchChangeHistoryData[]>();
  @Input() batchChangeHistorySubmitData: BatchChangeHistorySubmitData | null = null;
  ...
  openModal(batchChangeHistoryData?: BatchChangeHistoryData) {

    if (batchChangeHistoryData != null) {

      if (!this.batchChangeHistoryService.userIsSchedulerBatchChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Batch Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.batchChangeHistorySubmitData = this.batchChangeHistoryService.ConvertToBatchChangeHistorySubmitData(batchChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(batchChangeHistoryData);

    } else {

      if (!this.batchChangeHistoryService.userIsSchedulerBatchChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Batch Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.batchChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const batchChangeHistorySubmitData: BatchChangeHistorySubmitData = {
        id: this.batchChangeHistorySubmitData?.id || 0,
        batchId: Number(formValue.batchId),
        versionNumber: this.batchChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateBatchChangeHistory(batchChangeHistorySubmitData);
      } else {
        this.addBatchChangeHistory(batchChangeHistorySubmitData);
      }
  }

  private addBatchChangeHistory(batchChangeHistoryData: BatchChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    batchChangeHistoryData.versionNumber = 0;
    this.batchChangeHistoryService.PostBatchChangeHistory(batchChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newBatchChangeHistory) => {

        this.batchChangeHistoryService.ClearAllCaches();

        this.batchChangeHistoryChanged.next([newBatchChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/batchchangehistory', newBatchChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateBatchChangeHistory(batchChangeHistoryData: BatchChangeHistorySubmitData) {
    this.batchChangeHistoryService.PutBatchChangeHistory(batchChangeHistoryData.id, batchChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedBatchChangeHistory) => {

        this.batchChangeHistoryService.ClearAllCaches();

        this.batchChangeHistoryChanged.next([updatedBatchChangeHistory]);

        this.alertService.showMessage("Batch Change History updated successfully", '', MessageSeverity.success);
