...
@Component({
  selector: 'app-event-status-add-edit',
  templateUrl: './event-status-add-edit.component.html',
  styleUrls: ['./event-status-add-edit.component.scss']
})
export class EventStatusAddEditComponent {
  @ViewChild('eventStatusModal') eventStatusModal!: TemplateRef<any>;
  @Output() eventStatusChanged = new Subject<EventStatusData[]>();
  @Input() eventStatusSubmitData: EventStatusSubmitData | null = null;
  ...
  openModal(eventStatusData?: EventStatusData) {

    if (eventStatusData != null) {

      if (!this.eventStatusService.userIsSchedulerEventStatusReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Event Statuses`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.eventStatusSubmitData = this.eventStatusService.ConvertToEventStatusSubmitData(eventStatusData);
      this.isEditMode = true;
      this.objectGuid = eventStatusData.objectGuid;

      this.buildFormValues(eventStatusData);

    } else {

      if (!this.eventStatusService.userIsSchedulerEventStatusWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Event Statuses`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.eventStatusModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const eventStatusSubmitData: EventStatusSubmitData = {
        id: this.eventStatusSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateEventStatus(eventStatusSubmitData);
      } else {
        this.addEventStatus(eventStatusSubmitData);
      }
  }

  private addEventStatus(eventStatusData: EventStatusSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    eventStatusData.active = true;
    eventStatusData.deleted = false;
    this.eventStatusService.PostEventStatus(eventStatusData).pipe(
      ...
      next: (newEventStatus) => {
        ...
        this.eventStatusChanged.next([newEventStatus]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/eventstatus', newEventStatus.id]);
        }
      },
      error: (err) => {
  ...
  private updateEventStatus(eventStatusData: EventStatusSubmitData) {
    this.eventStatusService.PutEventStatus(eventStatusData.id, eventStatusData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedEventStatus) => {

        this.eventStatusService.ClearAllCaches();

        this.eventStatusChanged.next([updatedEventStatus]);

        this.alertService.showMessage("Event Status updated successfully", '', MessageSeverity.success);
