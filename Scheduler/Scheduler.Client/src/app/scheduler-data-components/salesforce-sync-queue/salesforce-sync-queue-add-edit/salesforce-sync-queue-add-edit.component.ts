...
@Component({
  selector: 'app-salesforce-sync-queue-add-edit',
  templateUrl: './salesforce-sync-queue-add-edit.component.html',
  styleUrls: ['./salesforce-sync-queue-add-edit.component.scss']
})
export class SalesforceSyncQueueAddEditComponent {
  @ViewChild('salesforceSyncQueueModal') salesforceSyncQueueModal!: TemplateRef<any>;
  @Output() salesforceSyncQueueChanged = new Subject<SalesforceSyncQueueData[]>();
  @Input() salesforceSyncQueueSubmitData: SalesforceSyncQueueSubmitData | null = null;
  ...
  constructor(
    ...
    private salesforceSyncQueueService: SalesforceSyncQueueService,
  ...
  openModal(salesforceSyncQueueData?: SalesforceSyncQueueData) {

    if (salesforceSyncQueueData != null) {

      if (!this.salesforceSyncQueueService.userIsSchedulerSalesforceSyncQueueReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Salesforce Sync Queues`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.salesforceSyncQueueSubmitData = this.salesforceSyncQueueService.ConvertToSalesforceSyncQueueSubmitData(salesforceSyncQueueData);
      this.isEditMode = true;
      this.objectGuid = salesforceSyncQueueData.objectGuid;

      this.buildFormValues(salesforceSyncQueueData);

    } else {

      if (!this.salesforceSyncQueueService.userIsSchedulerSalesforceSyncQueueWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Salesforce Sync Queues`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.salesforceSyncQueueModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const salesforceSyncQueueSubmitData: SalesforceSyncQueueSubmitData = {
        id: this.salesforceSyncQueueSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateSalesforceSyncQueue(salesforceSyncQueueSubmitData);
      } else {
        this.addSalesforceSyncQueue(salesforceSyncQueueSubmitData);
      }
  }

  private addSalesforceSyncQueue(salesforceSyncQueueData: SalesforceSyncQueueSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    salesforceSyncQueueData.active = true;
    salesforceSyncQueueData.deleted = false;
    this.salesforceSyncQueueService.PostSalesforceSyncQueue(salesforceSyncQueueData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newSalesforceSyncQueue) => {

        this.salesforceSyncQueueService.ClearAllCaches();

        this.salesforceSyncQueueChanged.next([newSalesforceSyncQueue]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/salesforcesyncqueue', newSalesforceSyncQueue.id]);
        }
      },
      error: (err) => {
  ...
  private updateSalesforceSyncQueue(salesforceSyncQueueData: SalesforceSyncQueueSubmitData) {
    this.salesforceSyncQueueService.PutSalesforceSyncQueue(salesforceSyncQueueData.id, salesforceSyncQueueData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedSalesforceSyncQueue) => {

        this.salesforceSyncQueueService.ClearAllCaches();

        this.salesforceSyncQueueChanged.next([updatedSalesforceSyncQueue]);

        this.alertService.showMessage("Salesforce Sync Queue updated successfully", '', MessageSeverity.success);
