...
@Component({
  selector: 'app-scheduled-event-change-history-add-edit',
  templateUrl: './scheduled-event-change-history-add-edit.component.html',
  styleUrls: ['./scheduled-event-change-history-add-edit.component.scss']
})
export class ScheduledEventChangeHistoryAddEditComponent {
  @ViewChild('scheduledEventChangeHistoryModal') scheduledEventChangeHistoryModal!: TemplateRef<any>;
  @Output() scheduledEventChangeHistoryChanged = new Subject<ScheduledEventChangeHistoryData[]>();
  @Input() scheduledEventChangeHistorySubmitData: ScheduledEventChangeHistorySubmitData | null = null;
  ...
  openModal(scheduledEventChangeHistoryData?: ScheduledEventChangeHistoryData) {
    ...
    if (scheduledEventChangeHistoryData != null) {
      ...
      if (!this.scheduledEventChangeHistoryService.userIsSchedulerScheduledEventChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Scheduled Event Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.scheduledEventChangeHistorySubmitData = this.scheduledEventChangeHistoryService.ConvertToScheduledEventChangeHistorySubmitData(scheduledEventChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(scheduledEventChangeHistoryData);

    } else {

      if (!this.scheduledEventChangeHistoryService.userIsSchedulerScheduledEventChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Scheduled Event Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.scheduledEventChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const scheduledEventChangeHistorySubmitData: ScheduledEventChangeHistorySubmitData = {
        id: this.scheduledEventChangeHistorySubmitData?.id || 0,
        scheduledEventId: Number(formValue.scheduledEventId),
        versionNumber: this.scheduledEventChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateScheduledEventChangeHistory(scheduledEventChangeHistorySubmitData);
      } else {
        this.addScheduledEventChangeHistory(scheduledEventChangeHistorySubmitData);
      }
  }

  private addScheduledEventChangeHistory(scheduledEventChangeHistoryData: ScheduledEventChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    scheduledEventChangeHistoryData.versionNumber = 0;
    this.scheduledEventChangeHistoryService.PostScheduledEventChangeHistory(scheduledEventChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newScheduledEventChangeHistory) => {

        this.scheduledEventChangeHistoryService.ClearAllCaches();

        this.scheduledEventChangeHistoryChanged.next([newScheduledEventChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/scheduledeventchangehistory', newScheduledEventChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateScheduledEventChangeHistory(scheduledEventChangeHistoryData: ScheduledEventChangeHistorySubmitData) {
    this.scheduledEventChangeHistoryService.PutScheduledEventChangeHistory(scheduledEventChangeHistoryData.id, scheduledEventChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedScheduledEventChangeHistory) => {

        this.scheduledEventChangeHistoryService.ClearAllCaches();

        this.scheduledEventChangeHistoryChanged.next([updatedScheduledEventChangeHistory]);

        this.alertService.showMessage("Scheduled Event Change History updated successfully", '', MessageSeverity.success);
