...
@Component({
  selector: 'app-rate-sheet-change-history-add-edit',
  templateUrl: './rate-sheet-change-history-add-edit.component.html',
  styleUrls: ['./rate-sheet-change-history-add-edit.component.scss']
})
export class RateSheetChangeHistoryAddEditComponent {
  @ViewChild('rateSheetChangeHistoryModal') rateSheetChangeHistoryModal!: TemplateRef<any>;
  @Output() rateSheetChangeHistoryChanged = new Subject<RateSheetChangeHistoryData[]>();
  @Input() rateSheetChangeHistorySubmitData: RateSheetChangeHistorySubmitData | null = null;
  ...
  openModal(rateSheetChangeHistoryData?: RateSheetChangeHistoryData) {

    if (rateSheetChangeHistoryData != null) {

      if (!this.rateSheetChangeHistoryService.userIsSchedulerRateSheetChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Rate Sheet Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.rateSheetChangeHistorySubmitData = this.rateSheetChangeHistoryService.ConvertToRateSheetChangeHistorySubmitData(rateSheetChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(rateSheetChangeHistoryData);

    } else {

      if (!this.rateSheetChangeHistoryService.userIsSchedulerRateSheetChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Rate Sheet Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.rateSheetChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const rateSheetChangeHistorySubmitData: RateSheetChangeHistorySubmitData = {
        id: this.rateSheetChangeHistorySubmitData?.id || 0,
        rateSheetId: Number(formValue.rateSheetId),
        versionNumber: this.rateSheetChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateRateSheetChangeHistory(rateSheetChangeHistorySubmitData);
      } else {
        this.addRateSheetChangeHistory(rateSheetChangeHistorySubmitData);
      }
  }

  private addRateSheetChangeHistory(rateSheetChangeHistoryData: RateSheetChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    rateSheetChangeHistoryData.versionNumber = 0;
    this.rateSheetChangeHistoryService.PostRateSheetChangeHistory(rateSheetChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newRateSheetChangeHistory) => {

        this.rateSheetChangeHistoryService.ClearAllCaches();

        this.rateSheetChangeHistoryChanged.next([newRateSheetChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/ratesheetchangehistory', newRateSheetChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateRateSheetChangeHistory(rateSheetChangeHistoryData: RateSheetChangeHistorySubmitData) {
    this.rateSheetChangeHistoryService.PutRateSheetChangeHistory(rateSheetChangeHistoryData.id, rateSheetChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedRateSheetChangeHistory) => {

        this.rateSheetChangeHistoryService.ClearAllCaches();

        this.rateSheetChangeHistoryChanged.next([updatedRateSheetChangeHistory]);

        this.alertService.showMessage("Rate Sheet Change History updated successfully", '', MessageSeverity.success);
