...
@Component({
  selector: 'app-contact-tag-change-history-add-edit',
  templateUrl: './contact-tag-change-history-add-edit.component.html',
  styleUrls: ['./contact-tag-change-history-add-edit.component.scss']
})
export class ContactTagChangeHistoryAddEditComponent {
  @ViewChild('contactTagChangeHistoryModal') contactTagChangeHistoryModal!: TemplateRef<any>;
  @Output() contactTagChangeHistoryChanged = new Subject<ContactTagChangeHistoryData[]>();
  @Input() contactTagChangeHistorySubmitData: ContactTagChangeHistorySubmitData | null = null;
  ...
  openModal(contactTagChangeHistoryData?: ContactTagChangeHistoryData) {
    ...
    if (contactTagChangeHistoryData != null) {
      ...
      if (!this.contactTagChangeHistoryService.userIsSchedulerContactTagChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Contact Tag Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.contactTagChangeHistorySubmitData = this.contactTagChangeHistoryService.ConvertToContactTagChangeHistorySubmitData(contactTagChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(contactTagChangeHistoryData);

    } else {

      if (!this.contactTagChangeHistoryService.userIsSchedulerContactTagChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Contact Tag Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.contactTagChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const contactTagChangeHistorySubmitData: ContactTagChangeHistorySubmitData = {
        id: this.contactTagChangeHistorySubmitData?.id || 0,
        contactTagId: Number(formValue.contactTagId),
        versionNumber: this.contactTagChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateContactTagChangeHistory(contactTagChangeHistorySubmitData);
      } else {
        this.addContactTagChangeHistory(contactTagChangeHistorySubmitData);
      }
  }

  private addContactTagChangeHistory(contactTagChangeHistoryData: ContactTagChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    contactTagChangeHistoryData.versionNumber = 0;
    this.contactTagChangeHistoryService.PostContactTagChangeHistory(contactTagChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newContactTagChangeHistory) => {

        this.contactTagChangeHistoryService.ClearAllCaches();

        this.contactTagChangeHistoryChanged.next([newContactTagChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/contacttagchangehistory', newContactTagChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateContactTagChangeHistory(contactTagChangeHistoryData: ContactTagChangeHistorySubmitData) {
    this.contactTagChangeHistoryService.PutContactTagChangeHistory(contactTagChangeHistoryData.id, contactTagChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedContactTagChangeHistory) => {

        this.contactTagChangeHistoryService.ClearAllCaches();

        this.contactTagChangeHistoryChanged.next([updatedContactTagChangeHistory]);

        this.alertService.showMessage("Contact Tag Change History updated successfully", '', MessageSeverity.success);
