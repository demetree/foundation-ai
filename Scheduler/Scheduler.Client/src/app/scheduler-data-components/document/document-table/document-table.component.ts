...
export class DocumentTableComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @Input() queryParams: Partial<DocumentQueryParameters> = { } // Optional query parameters
  ...
  @Output() edit = new EventEmitter<DocumentData>(); // Emitted for custom edit handling
  @Output() delete = new EventEmitter<DocumentData>(); // Emitted for custom delete handling
  @Output() undelete = new EventEmitter<DocumentData>(); // Emitted for custom undelete handling

  @Input() columns: TableColumn[] = [];     // Default set built in ngOnInit

  public filteredDocuments: DocumentData[] | null = null;        // Stores the filtered/sorted data
  ...
  ngAfterViewInit(): void {
    ...
    //
    // Subscribe to the documentChanged observable on the add/edit component so that when a Document changes we can reload the list, if component is available and not disabled..
    //
    if (this.addEditDocumentComponent && !this.disableDefaultEdit) {
 ...
 /**
  ...
  private buildDefaultColumns(): void {
    ...
    // { key: 'fileDataFileName', label: 'File Data File Name', width: undefined },
    ...
    { key: 'storageKey', label: 'Storage Key', width: undefined },
    ...
    { key: 'statusDate', label: 'Status Date', width: undefined, template: 'date' },
    { key: 'statusChangedBy', label: 'Status Changed By', width: undefined },
    { key: 'uploadedDate', label: 'Uploaded Date', width: undefined, template: 'date' },
    ...
    //
    const isWriter = this.documentService.userIsSchedulerDocumentWriter();
  ...
  public loadData(): void {
    ...
    if (this.documentService.userIsSchedulerDocumentReader() == false) {
      this.alertService.showMessage(this.authService.currentUser?.userName + " does not have the permission to read from Documents", '', MessageSeverity.info);
      return;
    }
  ...
  public handleEdit(document: DocumentData): void {
    if (this.disableDefaultEdit)
    {
        this.edit.emit(document); // Let parent handle edit
    }
    else if (this.addEditDocumentComponent)
    {
        this.addEditDocumentComponent.openModal(document); // Default edit behavior
    }
    else
    {
  ...
  public handleAdd(): void {
    if (this.addEditDocumentComponent)
    {
        this.addEditDocumentComponent.openModal(); // Open in add mode (no data)
    }
}
  ...
  public handleDelete(document: DocumentData): void {
    ...
    {
        this.delete.emit(document); // Let parent handle delete
    }
    else
    {
  ...
  public handleUndelete(document: DocumentData): void {
    ...
    {
        this.undelete.emit(document); // Let parent handle undelete
    }
    else
    {
        this.confirmationService
  ...
  private undeleteDocument(documentData: DocumentData): void {
      ...
      var documentToSubmit = this.documentService.ConvertToDocumentSubmitData(documentData); // Convert Document data to post object for undeleting
  ...
  // Build routerLink arrays like ['/document', documentId]
