...
export interface ContactChangeMetaData {
    versionNumber: number;
    timeStamp: string;
    userId: number;
    // user?: string; // if we hydration on server
}
...
export class ContactData {
    ...
    private _changeMetaData: ContactChangeMetaData | null = null;
    private _changeMetaDataPromise: Promise<ContactChangeMetaData> | null = null;
    private _changeMetaDataSubject = new BehaviorSubject<ContactChangeMetaData | null>(null);
    ...
    public ChangeMetaData$ = this._changeMetaDataSubject.asObservable().pipe(
        tap(() => {
            if (this._changeMetaData === null && this._changeMetaDataPromise === null) {
                this.loadChangeMetaData();
            }
        }),
        shareReplay(1)
    );
    ...
    private clearAllLazyCaches(): void {
        ...
        this._changeMetaData = null;
        this._changeMetaDataPromise = null;
        this._changeMetaDataSubject.next(null);
    ...
    */
    ...
    //
    // Change Metadata Loading
    //
    public get ChangeMetaData(): Promise<ContactChangeMetaData> {
        if (this._changeMetaData !== null) {
            return Promise.resolve(this._changeMetaData);
        }
        if (this._changeMetaDataPromise !== null) {
            return this._changeMetaDataPromise;
        }
        this.loadChangeMetaData();
        return this._changeMetaDataPromise!;
    }

    private loadChangeMetaData(): void {
        this._changeMetaDataPromise = lastValueFrom(
            ContactService.Instance.GetContactChangeMetadata(this.id, this.versionNumber)
        )
            .then(data => {
                this._changeMetaData = data;
                this._changeMetaDataSubject.next(data);
                // Hydrate user name if we want?
                return data;
            })
            .catch(err => {
                this._changeMetaData = null;
    ...
    public ClearChangeMetaDataCache(): void {
        this._changeMetaData = null;
        this._changeMetaDataPromise = null;
        this._changeMetaDataSubject.next(null);
    }
...
export class ContactService extends SecureEndpointBase {
    ...
    public GetContactChangeMetadata(id: bigint | number, versionNumber: bigint | number): Observable<ContactChangeMetaData> {
        let params = new HttpParams();
        params = params.set('versionNumber', versionNumber.toString());

        const authenticationHeaders = this.authService.GetAuthenticationHeaders();
        return this.http.get<ContactChangeMetaData>(this.baseUrl + `api/Contact/${id}/ChangeMetadata`, { headers: authenticationHeaders, params: params })
            .pipe(catchError((error) => this.handleError(error, () => this.GetContactChangeMetadata(id, versionNumber))));
    }

    public GetContactAuditHistory(id: bigint | number, includeJson: boolean = false): Observable<AuditEntry[]> {
        let params = new HttpParams();
        params = params.set('includeJson', includeJson.toString());

        const authenticationHeaders = this.authService.GetAuthenticationHeaders();
        return this.http.get<AuditEntry[]>(this.baseUrl + `api/Contact/${id}/AuditHistory`, { headers: authenticationHeaders, params: params })
            .pipe(catchError((error) => this.handleError(error, () => this.GetContactAuditHistory(id, includeJson))));
    }

    public GetContactVersion(id: bigint | number, version: bigint | number): Observable<ContactData> {
        const authenticationHeaders = this.authService.GetAuthenticationHeaders();
        return this.http.get<ContactData>(this.baseUrl + `api/Contact/${id}/Version/${version}`, { headers: authenticationHeaders })
            .pipe(
                map(raw => this.ReviveContact(raw)),
                catchError((error) => this.handleError(error, () => this.GetContactVersion(id, version)))
            );
    }
    ...
    public GetContactStateAtTime(id: bigint | number, time: string): Observable<ContactData> {
        ...
        const authenticationHeaders = this.authService.GetAuthenticationHeaders();
        return this.http.get<ContactData>(this.baseUrl + `api/Contact/${id}/StateAtTime`, { headers: authenticationHeaders, params: params })
            .pipe(
                map(raw => this.ReviveContact(raw)),
                catchError((error) => this.handleError(error, () => this.GetContactStateAtTime(id, time)))
            );
    }
    ...
    public ReviveContact(raw: any): ContactData {
        ...
        (revived as any)._changeMetaData = null;
        (revived as any)._changeMetaDataPromise = null;
        (revived as any)._changeMetaDataSubject = new BehaviorSubject<ContactChangeMetaData | null>(null);
        ...
        (revived as any).ChangeMetaData$ = (revived as any)._changeMetaDataSubject.asObservable().pipe(
            tap(() => {
                if ((revived as any)._changeMetaData === null && (revived as any)._changeMetaDataPromise === null) {
                    (revived as any).loadChangeMetaData();
                }
            }),
            shareReplay(1)
        );
