...
export class TributeData {
    ...
    public get DocumentsCount$(): Observable<bigint | number> {
        if (this._documentsCount$ === null) {
            this._documentsCount$ = DocumentService.Instance.GetDocumentsRowCount({tributeId: this.id,
              active: true,
              deleted: false
            });
        }
  ...
  private clearAllLazyCaches(): void {
    ...
    */
    ...
    /**
     *
     * Gets the Documents for this Tribute.
     *
     * If already loaded, returns cached array.
     *
     * If not, fetches from server and caches the result.
     * 
     * Usage in components:
     *   this.tribute.Documents.then(tributes => { ... })
     *   or
     *   await this.tribute.tributes
     *
    */
    ...
    private loadDocuments(): void {
        ...
        this._documentsPromise = lastValueFrom(
            TributeService.Instance.GetDocumentsForTribute(this.id)
        )
        .then(Documents => {
...
export class TributeService extends SecureEndpointBase {
    ...
    public GetDocumentsForTribute(tributeId: number | bigint, active: boolean = true, deleted: boolean = false): Observable<DocumentData[]> {
 ...
 /**
  ...
  public ReviveTribute(raw: any): TributeData {
    ...
    (revived as any)._currentVersionInfoSubject = new BehaviorSubject<VersionInformation<TributeData> | null>(null);
