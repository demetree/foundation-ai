...
import { OfficeContactService, OfficeContactSubmitData } from '../../../scheduler-data-services/office-contact.service';
import { RelationshipTypeService } from '../../../scheduler-data-services/relationship-type.service';

/**
 * Modal component for adding or editing a OfficeContact link.
 *
 * Used from the Office Contacts tab to:
 * - Create a new contact and link it to the office
 * - Link an existing contact to the office
 * - Edit relationship type or primary flag on existing link
 *
 * Supports both modes:
 * - Add: create new Contact + OfficeContact
 * - Edit: modify existing OfficeContact (relationship, primary)
 */
@Component({
  selector: 'app-office-contact-custom-add-edit-modal',
  templateUrl: './office-contact-custom-add-edit-modal.component.html',
  styleUrls: ['./office-contact-custom-add-edit-modal.component.scss']
})
export class OfficeContactCustomAddEditModalComponent {
  @ViewChild('officeContactModal') officeContactModal!: TemplateRef<any>;

  // Optional pre-configured office (when opened from office detail)
  @Input() office: OfficeData | null = null;
  ...
  public submitForm(): void {
    ...
    if (!this.office) {
      this.alertService.showMessage('No office selected', '', MessageSeverity.error);
      return;
    }
    ...
    if (formValue.createNewContact) {
      ...
      const newContact: ContactSubmitData = {
        ...
        timeZoneId: this.office.timeZoneId,     // reasonable enough to assume that the office contact is in the same time zone as the office..
      ...
      this.contactService.PostContact(newContact).pipe(
        ...
        next: (createdContact) => {
          this.linkContactToOffice(createdContact.id as number);
        },
        error: (err) => {
      ...
      this.linkContactToOffice(formValue.contactId);
    }
  }

  private linkContactToOffice(contactId: number): void {
    const formValue = this.contactForm.getRawValue();

    const linkData: OfficeContactSubmitData = {
    ...
    this.officeContactService.PostOfficeContact(linkData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: () => {
