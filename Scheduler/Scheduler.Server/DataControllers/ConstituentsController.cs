...
namespace Foundation.Scheduler.Controllers.WebAPI
{
	...
	public partial class ConstituentsController : SecureWebAPIController
	{
        ...
        /// <summary>
        /// 
        /// Gets the change metadata (version info, timestamp, user) for a specific version of a Constituent.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        /// <param name="id">The primary key of the Constituent</param>
        /// <param name="versionNumber">The version number to retrieve metadata for</param>
        /// <returns>VersionInformation containing timestamp and user details</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Constituent/{id}/ChangeMetadata")]
		public async Task<IActionResult> GetConstituentChangeMetadata(int id, int versionNumber, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			Database.Constituent constituent = await _context.Constituents.Where(x => x.id == id
				&& x.tenantGuid == userTenantGuid
			...
			try
			{
				constituent.SetupVersionInquiry(_context, userTenantGuid);

				VersionInformation<Database.Constituent> versionInfo = await constituent.GetVersionAsync(versionNumber, includeData: false, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets the full audit history for a Constituent.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Constituent/{id}/AuditHistory")]
		public async Task<IActionResult> GetConstituentAuditHistory(int id, bool includeData = false, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				List<VersionInformation<Database.Constituent>> versions = await constituent.GetAllVersionsAsync(includeData: includeData, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets a specific version of a Constituent.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        ...
        /// <returns>The Constituent object at that version</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Constituent/{id}/Version/{version}")]
		public async Task<IActionResult> GetConstituentVersion(int id, int version, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.Constituent> versionInfo = await constituent.GetVersionAsync(version, includeData: true, cancellationToken).ConfigureAwait(false);
        ...
        /// <returns>The Constituent object at that time</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Constituent/{id}/StateAtTime")]
		public async Task<IActionResult> GetConstituentStateAtTime(int id, DateTime time, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.Constituent> versionInfo = await constituent.GetVersionAtTimeAsync(time, includeData: true, cancellationToken).ConfigureAwait(false);
