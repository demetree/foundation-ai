...
namespace Foundation.Scheduler.Controllers.WebAPI
{
	...
	public partial class FundsController : SecureWebAPIController
	{
        ...
        /// <summary>
        /// 
        /// Gets the change metadata (version info, timestamp, user) for a specific version of a Fund.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Fund/{id}/ChangeMetadata")]
		public async Task<IActionResult> GetFundChangeMetadata(int id, int versionNumber, CancellationToken cancellationToken = default)
		{
			...
			Database.Fund fund = await _context.Funds.Where(x => x.id == id
				&& x.tenantGuid == userTenantGuid
			...
			try
			{
				fund.SetupVersionInquiry(_context, userTenantGuid);

				VersionInformation<Database.Fund> versionInfo = await fund.GetVersionAsync(versionNumber, includeData: false, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets the full audit history for a Fund.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Fund/{id}/AuditHistory")]
		public async Task<IActionResult> GetFundAuditHistory(int id, bool includeData = false, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				List<VersionInformation<Database.Fund>> versions = await fund.GetAllVersionsAsync(includeData: includeData, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets a specific version of a Fund.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        ...
        /// <returns>The Fund object at that version</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Fund/{id}/Version/{version}")]
		public async Task<IActionResult> GetFundVersion(int id, int version, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				VersionInformation<Database.Fund> versionInfo = await fund.GetVersionAsync(version, includeData: true, cancellationToken).ConfigureAwait(false);
        ...
        /// <returns>The Fund object at that time</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Fund/{id}/StateAtTime")]
		public async Task<IActionResult> GetFundStateAtTime(int id, DateTime time, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				VersionInformation<Database.Fund> versionInfo = await fund.GetVersionAtTimeAsync(time, includeData: true, cancellationToken).ConfigureAwait(false);
