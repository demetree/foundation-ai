...
namespace Foundation.Scheduler.Controllers.WebAPI
{
	...
	public partial class AppealsController : SecureWebAPIController
	{
		...
		public async Task<IActionResult> PutAppeal(int id, [FromBody]Database.Appeal.AppealDTO appealDTO, CancellationToken cancellationToken = default)
		{
			...
			//
			// Scheduler Fundraising Writer role needed to write to this table, or Scheduler Administrator role.  Note we do not check the user's write permission level here.  Role membership is the key to write access.
			//
			if (await DoesUserHaveCustomRoleSecurityCheckAsync("Scheduler Fundraising Writer", cancellationToken) == false && await DoesUserHaveAdminPrivilegeSecurityCheckAsync(cancellationToken) == false)
			{
			   return Forbid();
			}
        ...
        /// <summary>
        /// 
        /// Gets the change metadata (version info, timestamp, user) for a specific version of a Appeal.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        /// <param name="id">The primary key of the Appeal</param>
        /// <param name="versionNumber">The version number to retrieve metadata for</param>
        /// <returns>VersionInformation containing timestamp and user details</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Appeal/{id}/ChangeMetadata")]
		public async Task<IActionResult> GetAppealChangeMetadata(int id, int versionNumber, CancellationToken cancellationToken = default)
		{
			...
			Database.Appeal appeal = await _context.Appeals.Where(x => x.id == id
				&& x.tenantGuid == userTenantGuid
			...
			try
			{
				appeal.SetupVersionInquiry(_context, userTenantGuid);

				VersionInformation<Database.Appeal> versionInfo = await appeal.GetVersionAsync(versionNumber, includeData: false, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets the full audit history for a Appeal.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Appeal/{id}/AuditHistory")]
		public async Task<IActionResult> GetAppealAuditHistory(int id, bool includeData = false, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				List<VersionInformation<Database.Appeal>> versions = await appeal.GetAllVersionsAsync(includeData: includeData, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets a specific version of a Appeal.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        ...
        /// <returns>The Appeal object at that version</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Appeal/{id}/Version/{version}")]
		public async Task<IActionResult> GetAppealVersion(int id, int version, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				VersionInformation<Database.Appeal> versionInfo = await appeal.GetVersionAsync(version, includeData: true, cancellationToken).ConfigureAwait(false);
        ...
        /// <returns>The Appeal object at that time</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Appeal/{id}/StateAtTime")]
		public async Task<IActionResult> GetAppealStateAtTime(int id, DateTime time, CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				VersionInformation<Database.Appeal> versionInfo = await appeal.GetVersionAtTimeAsync(time, includeData: true, cancellationToken).ConfigureAwait(false);
