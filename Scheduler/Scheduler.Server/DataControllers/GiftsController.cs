...
*/
        ...
        /// <summary>
        /// 
        /// Gets the change metadata (version info, timestamp, user) for a specific version of a Gift.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Gift/{id}/ChangeMetadata")]
		public async Task<IActionResult> GetGiftChangeMetadata(int id, int versionNumber, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			Database.Gift gift = await _context.Gifts.Where(x => x.id == id
				&& x.tenantGuid == userTenantGuid
			...
			try
			{
				gift.SetupVersionInquiry(_context, userTenantGuid);

				VersionInformation<Database.Gift> versionInfo = await gift.GetVersionAsync(versionNumber, includeData: false, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets the full audit history for a Gift.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Gift/{id}/AuditHistory")]
		public async Task<IActionResult> GetGiftAuditHistory(int id, bool includeData = false, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				List<VersionInformation<Database.Gift>> versions = await gift.GetAllVersionsAsync(includeData: includeData, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets a specific version of a Gift.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        ...
        /// <returns>The Gift object at that version</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Gift/{id}/Version/{version}")]
		public async Task<IActionResult> GetGiftVersion(int id, int version, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.Gift> versionInfo = await gift.GetVersionAsync(version, includeData: true, cancellationToken).ConfigureAwait(false);
        ...
        /// <returns>The Gift object at that time</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/Gift/{id}/StateAtTime")]
		public async Task<IActionResult> GetGiftStateAtTime(int id, DateTime time, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.Gift> versionInfo = await gift.GetVersionAtTimeAsync(time, includeData: true, cancellationToken).ConfigureAwait(false);
