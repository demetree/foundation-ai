...
namespace Foundation.Scheduler.Controllers.WebAPI
{
	...
	public partial class OfficeContactsController : SecureWebAPIController
	{
        ...
        /// <summary>
        /// 
        /// Gets the change metadata (version info, timestamp, user) for a specific version of a OfficeContact.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        /// <param name="id">The primary key of the OfficeContact</param>
        /// <param name="versionNumber">The version number to retrieve metadata for</param>
        /// <returns>VersionInformation containing timestamp and user details</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/OfficeContact/{id}/ChangeMetadata")]
		public async Task<IActionResult> GetOfficeContactChangeMetadata(int id, int versionNumber, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			Database.OfficeContact officeContact = await _context.OfficeContacts.Where(x => x.id == id
				&& x.tenantGuid == userTenantGuid
			...
			try
			{
				officeContact.SetupVersionInquiry(_context, userTenantGuid);

				VersionInformation<Database.OfficeContact> versionInfo = await officeContact.GetVersionAsync(versionNumber, includeData: false, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets the full audit history for a OfficeContact.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
		...
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/OfficeContact/{id}/AuditHistory")]
		public async Task<IActionResult> GetOfficeContactAuditHistory(int id, bool includeData = false, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				List<VersionInformation<Database.OfficeContact>> versions = await officeContact.GetAllVersionsAsync(includeData: includeData, cancellationToken).ConfigureAwait(false);
        ...
        /// <summary>
        /// 
        /// Gets a specific version of a OfficeContact.
        ///
        /// The rate limit is 2 per second per user.
        /// 
        /// </summary>
        ...
        /// <returns>The OfficeContact object at that version</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/OfficeContact/{id}/Version/{version}")]
		public async Task<IActionResult> GetOfficeContactVersion(int id, int version, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.OfficeContact> versionInfo = await officeContact.GetVersionAsync(version, includeData: true, cancellationToken).ConfigureAwait(false);
        ...
        /// <returns>The OfficeContact object at that time</returns>
		[HttpGet]
		[RateLimit(RateLimitOption.TwoPerSecond, Scope = RateLimitScope.PerUser)]
		[Route("api/OfficeContact/{id}/StateAtTime")]
		public async Task<IActionResult> GetOfficeContactStateAtTime(int id, DateTime time, CancellationToken cancellationToken = default)
		{
			if (await DoesUserHaveReadPrivilegeSecurityCheckAsync(READ_PERMISSION_LEVEL_REQUIRED, cancellationToken) == false)
			{
				return Forbid();
			...
			try
			{
				...
				VersionInformation<Database.OfficeContact> versionInfo = await officeContact.GetVersionAtTimeAsync(time, includeData: true, cancellationToken).ConfigureAwait(false);
