...
export class LocalStoreManager {
    ...
    private testForInvalidKeys(key: string) {
        ...
        if (this.reservedKeys.some(x => x === key)) {
            throw new Error(`The storage key "${key}" is reserved and cannot be used.`);
        }
    }

    private syncKeysContains(key: string) { return this.syncKeys.some(x => x === key); }
    ...
    private getSyncKeysFromStorage(defaultValue: string[] = []): string[] {
        ...
        return data == null ? defaultValue : data as string[];
    }
    ...
    private localStorageGetItem(key: string) {
        ...
        return item === null ? null : Utilities.JsonTryParse(item);
    }
