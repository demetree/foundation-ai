...
@Component({
  selector: 'app-render-preset-add-edit',
  templateUrl: './render-preset-add-edit.component.html',
  styleUrls: ['./render-preset-add-edit.component.scss']
})
export class RenderPresetAddEditComponent {
  @ViewChild('renderPresetModal') renderPresetModal!: TemplateRef<any>;
  @Output() renderPresetChanged = new Subject<RenderPresetData[]>();
  @Input() renderPresetSubmitData: RenderPresetSubmitData | null = null;
  ...
  openModal(renderPresetData?: RenderPresetData) {

    if (renderPresetData != null) {

      if (!this.renderPresetService.userIsBMCRenderPresetReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Render Presets`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.renderPresetSubmitData = this.renderPresetService.ConvertToRenderPresetSubmitData(renderPresetData);
      this.isEditMode = true;
      this.objectGuid = renderPresetData.objectGuid;

      this.buildFormValues(renderPresetData);

    } else {

      if (!this.renderPresetService.userIsBMCRenderPresetWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Render Presets`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.renderPresetModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const renderPresetSubmitData: RenderPresetSubmitData = {
        id: this.renderPresetSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateRenderPreset(renderPresetSubmitData);
      } else {
        this.addRenderPreset(renderPresetSubmitData);
      }
  }

  private addRenderPreset(renderPresetData: RenderPresetSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    renderPresetData.active = true;
    renderPresetData.deleted = false;
    this.renderPresetService.PostRenderPreset(renderPresetData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newRenderPreset) => {

        this.renderPresetService.ClearAllCaches();

        this.renderPresetChanged.next([newRenderPreset]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/renderpreset', newRenderPreset.id]);
        }
      },
      error: (err) => {
  ...
  private updateRenderPreset(renderPresetData: RenderPresetSubmitData) {
    this.renderPresetService.PutRenderPreset(renderPresetData.id, renderPresetData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedRenderPreset) => {

        this.renderPresetService.ClearAllCaches();

        this.renderPresetChanged.next([updatedRenderPreset]);

        this.alertService.showMessage("Render Preset updated successfully", '', MessageSeverity.success);
