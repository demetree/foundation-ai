...
@Component({
  selector: 'app-brick-link-transaction-add-edit',
  templateUrl: './brick-link-transaction-add-edit.component.html',
  styleUrls: ['./brick-link-transaction-add-edit.component.scss']
})
export class BrickLinkTransactionAddEditComponent {
  @ViewChild('brickLinkTransactionModal') brickLinkTransactionModal!: TemplateRef<any>;
  @Output() brickLinkTransactionChanged = new Subject<BrickLinkTransactionData[]>();
  @Input() brickLinkTransactionSubmitData: BrickLinkTransactionSubmitData | null = null;
  ...
  constructor(
    ...
    private brickLinkTransactionService: BrickLinkTransactionService,
  ...
  openModal(brickLinkTransactionData?: BrickLinkTransactionData) {

    if (brickLinkTransactionData != null) {

      if (!this.brickLinkTransactionService.userIsBMCBrickLinkTransactionReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Brick Link Transactions`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.brickLinkTransactionSubmitData = this.brickLinkTransactionService.ConvertToBrickLinkTransactionSubmitData(brickLinkTransactionData);
      this.isEditMode = true;
      this.objectGuid = brickLinkTransactionData.objectGuid;

      this.buildFormValues(brickLinkTransactionData);

    } else {

      if (!this.brickLinkTransactionService.userIsBMCBrickLinkTransactionWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Brick Link Transactions`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.brickLinkTransactionModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const brickLinkTransactionSubmitData: BrickLinkTransactionSubmitData = {
        id: this.brickLinkTransactionSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateBrickLinkTransaction(brickLinkTransactionSubmitData);
      } else {
        this.addBrickLinkTransaction(brickLinkTransactionSubmitData);
      }
  }

  private addBrickLinkTransaction(brickLinkTransactionData: BrickLinkTransactionSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    brickLinkTransactionData.active = true;
    brickLinkTransactionData.deleted = false;
    this.brickLinkTransactionService.PostBrickLinkTransaction(brickLinkTransactionData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newBrickLinkTransaction) => {

        this.brickLinkTransactionService.ClearAllCaches();

        this.brickLinkTransactionChanged.next([newBrickLinkTransaction]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/bricklinktransaction', newBrickLinkTransaction.id]);
        }
      },
      error: (err) => {
  ...
  private updateBrickLinkTransaction(brickLinkTransactionData: BrickLinkTransactionSubmitData) {
    this.brickLinkTransactionService.PutBrickLinkTransaction(brickLinkTransactionData.id, brickLinkTransactionData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedBrickLinkTransaction) => {

        this.brickLinkTransactionService.ClearAllCaches();

        this.brickLinkTransactionChanged.next([updatedBrickLinkTransaction]);

        this.alertService.showMessage("Brick Link Transaction updated successfully", '', MessageSeverity.success);
