...
@Component({
  selector: 'app-part-type-add-edit',
  templateUrl: './part-type-add-edit.component.html',
  styleUrls: ['./part-type-add-edit.component.scss']
})
export class PartTypeAddEditComponent {
  @ViewChild('partTypeModal') partTypeModal!: TemplateRef<any>;
  @Output() partTypeChanged = new Subject<PartTypeData[]>();
  @Input() partTypeSubmitData: PartTypeSubmitData | null = null;
  ...
  openModal(partTypeData?: PartTypeData) {

    if (partTypeData != null) {

      if (!this.partTypeService.userIsBMCPartTypeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Part Types`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.partTypeSubmitData = this.partTypeService.ConvertToPartTypeSubmitData(partTypeData);
      this.isEditMode = true;
      this.objectGuid = partTypeData.objectGuid;

      this.buildFormValues(partTypeData);

    } else {

      if (!this.partTypeService.userIsBMCPartTypeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Part Types`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.partTypeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const partTypeSubmitData: PartTypeSubmitData = {
        id: this.partTypeSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updatePartType(partTypeSubmitData);
      } else {
        this.addPartType(partTypeSubmitData);
      }
  }

  private addPartType(partTypeData: PartTypeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    partTypeData.active = true;
    partTypeData.deleted = false;
    this.partTypeService.PostPartType(partTypeData).pipe(
      ...
      next: (newPartType) => {
        ...
        this.partTypeChanged.next([newPartType]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/parttype', newPartType.id]);
        }
      },
      error: (err) => {
  ...
  private updatePartType(partTypeData: PartTypeSubmitData) {
    this.partTypeService.PutPartType(partTypeData.id, partTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedPartType) => {

        this.partTypeService.ClearAllCaches();

        this.partTypeChanged.next([updatedPartType]);

        this.alertService.showMessage("Part Type updated successfully", '', MessageSeverity.success);
