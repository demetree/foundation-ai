...
@Component({
  selector: 'app-build-challenge-add-edit',
  templateUrl: './build-challenge-add-edit.component.html',
  styleUrls: ['./build-challenge-add-edit.component.scss']
})
export class BuildChallengeAddEditComponent {
  @ViewChild('buildChallengeModal') buildChallengeModal!: TemplateRef<any>;
  @Output() buildChallengeChanged = new Subject<BuildChallengeData[]>();
  @Input() buildChallengeSubmitData: BuildChallengeSubmitData | null = null;
  ...
  openModal(buildChallengeData?: BuildChallengeData) {

    if (buildChallengeData != null) {

      if (!this.buildChallengeService.userIsBMCBuildChallengeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Build Challenges`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.buildChallengeSubmitData = this.buildChallengeService.ConvertToBuildChallengeSubmitData(buildChallengeData);
      this.isEditMode = true;
      this.objectGuid = buildChallengeData.objectGuid;

      this.buildFormValues(buildChallengeData);

    } else {

      if (!this.buildChallengeService.userIsBMCBuildChallengeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Build Challenges`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.buildChallengeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const buildChallengeSubmitData: BuildChallengeSubmitData = {
        id: this.buildChallengeSubmitData?.id || 0,
        ...
        versionNumber: this.buildChallengeSubmitData?.versionNumber ?? 0,
        active: !!formValue.active,
        deleted: !!formValue.deleted,
   };

      if (this.isEditMode) {
        this.updateBuildChallenge(buildChallengeSubmitData);
      } else {
        this.addBuildChallenge(buildChallengeSubmitData);
      }
  }

  private addBuildChallenge(buildChallengeData: BuildChallengeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    buildChallengeData.versionNumber = 0;
    buildChallengeData.active = true;
    buildChallengeData.deleted = false;
    this.buildChallengeService.PostBuildChallenge(buildChallengeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newBuildChallenge) => {

        this.buildChallengeService.ClearAllCaches();

        this.buildChallengeChanged.next([newBuildChallenge]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/buildchallenge', newBuildChallenge.id]);
        }
      },
      error: (err) => {
  ...
  private updateBuildChallenge(buildChallengeData: BuildChallengeSubmitData) {
    this.buildChallengeService.PutBuildChallenge(buildChallengeData.id, buildChallengeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedBuildChallenge) => {

        this.buildChallengeService.ClearAllCaches();

        this.buildChallengeChanged.next([updatedBuildChallenge]);

        this.alertService.showMessage("Build Challenge updated successfully", '', MessageSeverity.success);
