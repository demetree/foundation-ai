...
@Component({
  selector: 'app-brick-owl-transaction-add-edit',
  templateUrl: './brick-owl-transaction-add-edit.component.html',
  styleUrls: ['./brick-owl-transaction-add-edit.component.scss']
})
export class BrickOwlTransactionAddEditComponent {
  @ViewChild('brickOwlTransactionModal') brickOwlTransactionModal!: TemplateRef<any>;
  @Output() brickOwlTransactionChanged = new Subject<BrickOwlTransactionData[]>();
  @Input() brickOwlTransactionSubmitData: BrickOwlTransactionSubmitData | null = null;
  ...
  constructor(
    ...
    private brickOwlTransactionService: BrickOwlTransactionService,
  ...
  openModal(brickOwlTransactionData?: BrickOwlTransactionData) {

    if (brickOwlTransactionData != null) {

      if (!this.brickOwlTransactionService.userIsBMCBrickOwlTransactionReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Brick Owl Transactions`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.brickOwlTransactionSubmitData = this.brickOwlTransactionService.ConvertToBrickOwlTransactionSubmitData(brickOwlTransactionData);
      this.isEditMode = true;
      this.objectGuid = brickOwlTransactionData.objectGuid;

      this.buildFormValues(brickOwlTransactionData);

    } else {

      if (!this.brickOwlTransactionService.userIsBMCBrickOwlTransactionWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Brick Owl Transactions`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.brickOwlTransactionModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const brickOwlTransactionSubmitData: BrickOwlTransactionSubmitData = {
        id: this.brickOwlTransactionSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateBrickOwlTransaction(brickOwlTransactionSubmitData);
      } else {
        this.addBrickOwlTransaction(brickOwlTransactionSubmitData);
      }
  }

  private addBrickOwlTransaction(brickOwlTransactionData: BrickOwlTransactionSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    brickOwlTransactionData.active = true;
    brickOwlTransactionData.deleted = false;
    this.brickOwlTransactionService.PostBrickOwlTransaction(brickOwlTransactionData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newBrickOwlTransaction) => {

        this.brickOwlTransactionService.ClearAllCaches();

        this.brickOwlTransactionChanged.next([newBrickOwlTransaction]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/brickowltransaction', newBrickOwlTransaction.id]);
        }
      },
      error: (err) => {
  ...
  private updateBrickOwlTransaction(brickOwlTransactionData: BrickOwlTransactionSubmitData) {
    this.brickOwlTransactionService.PutBrickOwlTransaction(brickOwlTransactionData.id, brickOwlTransactionData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedBrickOwlTransaction) => {

        this.brickOwlTransactionService.ClearAllCaches();

        this.brickOwlTransactionChanged.next([updatedBrickOwlTransaction]);

        this.alertService.showMessage("Brick Owl Transaction updated successfully", '', MessageSeverity.success);
