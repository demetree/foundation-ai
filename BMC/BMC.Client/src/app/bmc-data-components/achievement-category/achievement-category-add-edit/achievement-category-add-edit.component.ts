...
@Component({
  selector: 'app-achievement-category-add-edit',
  templateUrl: './achievement-category-add-edit.component.html',
  styleUrls: ['./achievement-category-add-edit.component.scss']
})
export class AchievementCategoryAddEditComponent {
  @ViewChild('achievementCategoryModal') achievementCategoryModal!: TemplateRef<any>;
  @Output() achievementCategoryChanged = new Subject<AchievementCategoryData[]>();
  @Input() achievementCategorySubmitData: AchievementCategorySubmitData | null = null;
  ...
  openModal(achievementCategoryData?: AchievementCategoryData) {

    if (achievementCategoryData != null) {

      if (!this.achievementCategoryService.userIsBMCAchievementCategoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Achievement Categories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.achievementCategorySubmitData = this.achievementCategoryService.ConvertToAchievementCategorySubmitData(achievementCategoryData);
      this.isEditMode = true;
      this.objectGuid = achievementCategoryData.objectGuid;

      this.buildFormValues(achievementCategoryData);

    } else {

      if (!this.achievementCategoryService.userIsBMCAchievementCategoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Achievement Categories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.achievementCategoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const achievementCategorySubmitData: AchievementCategorySubmitData = {
        id: this.achievementCategorySubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateAchievementCategory(achievementCategorySubmitData);
      } else {
        this.addAchievementCategory(achievementCategorySubmitData);
      }
  }

  private addAchievementCategory(achievementCategoryData: AchievementCategorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    achievementCategoryData.active = true;
    achievementCategoryData.deleted = false;
    this.achievementCategoryService.PostAchievementCategory(achievementCategoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newAchievementCategory) => {

        this.achievementCategoryService.ClearAllCaches();

        this.achievementCategoryChanged.next([newAchievementCategory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/achievementcategory', newAchievementCategory.id]);
        }
      },
      error: (err) => {
  ...
  private updateAchievementCategory(achievementCategoryData: AchievementCategorySubmitData) {
    this.achievementCategoryService.PutAchievementCategory(achievementCategoryData.id, achievementCategoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedAchievementCategory) => {

        this.achievementCategoryService.ClearAllCaches();

        this.achievementCategoryChanged.next([updatedAchievementCategory]);

        this.alertService.showMessage("Achievement Category updated successfully", '', MessageSeverity.success);
