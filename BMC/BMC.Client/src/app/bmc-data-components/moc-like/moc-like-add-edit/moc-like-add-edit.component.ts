...
@Component({
  selector: 'app-moc-like-add-edit',
  templateUrl: './moc-like-add-edit.component.html',
  styleUrls: ['./moc-like-add-edit.component.scss']
})
export class MocLikeAddEditComponent {
  @ViewChild('mocLikeModal') mocLikeModal!: TemplateRef<any>;
  @Output() mocLikeChanged = new Subject<MocLikeData[]>();
  @Input() mocLikeSubmitData: MocLikeSubmitData | null = null;
  ...
  openModal(mocLikeData?: MocLikeData) {

    if (mocLikeData != null) {

      if (!this.mocLikeService.userIsBMCMocLikeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Moc Likes`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.mocLikeSubmitData = this.mocLikeService.ConvertToMocLikeSubmitData(mocLikeData);
      this.isEditMode = true;
      this.objectGuid = mocLikeData.objectGuid;

      this.buildFormValues(mocLikeData);

    } else {

      if (!this.mocLikeService.userIsBMCMocLikeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Moc Likes`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.mocLikeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const mocLikeSubmitData: MocLikeSubmitData = {
        id: this.mocLikeSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateMocLike(mocLikeSubmitData);
      } else {
        this.addMocLike(mocLikeSubmitData);
      }
  }

  private addMocLike(mocLikeData: MocLikeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    mocLikeData.active = true;
    mocLikeData.deleted = false;
    this.mocLikeService.PostMocLike(mocLikeData).pipe(
      ...
      next: (newMocLike) => {
        ...
        this.mocLikeChanged.next([newMocLike]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/moclike', newMocLike.id]);
        }
      },
      error: (err) => {
  ...
  private updateMocLike(mocLikeData: MocLikeSubmitData) {
    this.mocLikeService.PutMocLike(mocLikeData.id, mocLikeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedMocLike) => {
        ...
        this.mocLikeChanged.next([updatedMocLike]);

        this.alertService.showMessage("Moc Like updated successfully", '', MessageSeverity.success);
