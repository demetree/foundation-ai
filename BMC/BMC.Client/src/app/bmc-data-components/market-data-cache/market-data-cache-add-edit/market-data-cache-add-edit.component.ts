...
@Component({
  selector: 'app-market-data-cache-add-edit',
  templateUrl: './market-data-cache-add-edit.component.html',
  styleUrls: ['./market-data-cache-add-edit.component.scss']
})
export class MarketDataCacheAddEditComponent {
  @ViewChild('marketDataCacheModal') marketDataCacheModal!: TemplateRef<any>;
  @Output() marketDataCacheChanged = new Subject<MarketDataCacheData[]>();
  @Input() marketDataCacheSubmitData: MarketDataCacheSubmitData | null = null;
  ...
  openModal(marketDataCacheData?: MarketDataCacheData) {

    if (marketDataCacheData != null) {

      if (!this.marketDataCacheService.userIsBMCMarketDataCacheReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Market Data Caches`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.marketDataCacheSubmitData = this.marketDataCacheService.ConvertToMarketDataCacheSubmitData(marketDataCacheData);
      this.isEditMode = true;
      this.objectGuid = marketDataCacheData.objectGuid;

      this.buildFormValues(marketDataCacheData);

    } else {

      if (!this.marketDataCacheService.userIsBMCMarketDataCacheWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Market Data Caches`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.marketDataCacheModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const marketDataCacheSubmitData: MarketDataCacheSubmitData = {
        id: this.marketDataCacheSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateMarketDataCache(marketDataCacheSubmitData);
      } else {
        this.addMarketDataCache(marketDataCacheSubmitData);
      }
  }

  private addMarketDataCache(marketDataCacheData: MarketDataCacheSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    marketDataCacheData.active = true;
    marketDataCacheData.deleted = false;
    this.marketDataCacheService.PostMarketDataCache(marketDataCacheData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newMarketDataCache) => {

        this.marketDataCacheService.ClearAllCaches();

        this.marketDataCacheChanged.next([newMarketDataCache]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/marketdatacache', newMarketDataCache.id]);
        }
      },
      error: (err) => {
  ...
  private updateMarketDataCache(marketDataCacheData: MarketDataCacheSubmitData) {
    this.marketDataCacheService.PutMarketDataCache(marketDataCacheData.id, marketDataCacheData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedMarketDataCache) => {

        this.marketDataCacheService.ClearAllCaches();

        this.marketDataCacheChanged.next([updatedMarketDataCache]);

        this.alertService.showMessage("Market Data Cache updated successfully", '', MessageSeverity.success);
