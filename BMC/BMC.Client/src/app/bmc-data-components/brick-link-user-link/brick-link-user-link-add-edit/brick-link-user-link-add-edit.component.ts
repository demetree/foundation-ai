...
@Component({
  selector: 'app-brick-link-user-link-add-edit',
  templateUrl: './brick-link-user-link-add-edit.component.html',
  styleUrls: ['./brick-link-user-link-add-edit.component.scss']
})
export class BrickLinkUserLinkAddEditComponent {
  @ViewChild('brickLinkUserLinkModal') brickLinkUserLinkModal!: TemplateRef<any>;
  @Output() brickLinkUserLinkChanged = new Subject<BrickLinkUserLinkData[]>();
  @Input() brickLinkUserLinkSubmitData: BrickLinkUserLinkSubmitData | null = null;
  ...
  constructor(
    ...
    private brickLinkUserLinkService: BrickLinkUserLinkService,
  ...
  openModal(brickLinkUserLinkData?: BrickLinkUserLinkData) {

    if (brickLinkUserLinkData != null) {

      if (!this.brickLinkUserLinkService.userIsBMCBrickLinkUserLinkReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Brick Link User Links`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.brickLinkUserLinkSubmitData = this.brickLinkUserLinkService.ConvertToBrickLinkUserLinkSubmitData(brickLinkUserLinkData);
      this.isEditMode = true;
      this.objectGuid = brickLinkUserLinkData.objectGuid;

      this.buildFormValues(brickLinkUserLinkData);

    } else {

      if (!this.brickLinkUserLinkService.userIsBMCBrickLinkUserLinkWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Brick Link User Links`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.brickLinkUserLinkModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const brickLinkUserLinkSubmitData: BrickLinkUserLinkSubmitData = {
        id: this.brickLinkUserLinkSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateBrickLinkUserLink(brickLinkUserLinkSubmitData);
      } else {
        this.addBrickLinkUserLink(brickLinkUserLinkSubmitData);
      }
  }

  private addBrickLinkUserLink(brickLinkUserLinkData: BrickLinkUserLinkSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    brickLinkUserLinkData.active = true;
    brickLinkUserLinkData.deleted = false;
    this.brickLinkUserLinkService.PostBrickLinkUserLink(brickLinkUserLinkData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newBrickLinkUserLink) => {

        this.brickLinkUserLinkService.ClearAllCaches();

        this.brickLinkUserLinkChanged.next([newBrickLinkUserLink]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/bricklinkuserlink', newBrickLinkUserLink.id]);
        }
      },
      error: (err) => {
  ...
  private updateBrickLinkUserLink(brickLinkUserLinkData: BrickLinkUserLinkSubmitData) {
    this.brickLinkUserLinkService.PutBrickLinkUserLink(brickLinkUserLinkData.id, brickLinkUserLinkData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedBrickLinkUserLink) => {

        this.brickLinkUserLinkService.ClearAllCaches();

        this.brickLinkUserLinkChanged.next([updatedBrickLinkUserLink]);

        this.alertService.showMessage("Brick Link User Link updated successfully", '', MessageSeverity.success);
