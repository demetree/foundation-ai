...
// Theme Service
//
// AI-DEVELOPED: Manages the active UI theme for the Foundation application.
//
// Uses a two-tier resolution approach:
// 1. User-level preference — read/written via UserSettingsService
// 2. Built-in default ('default')
//
...
/**
 * ThemeService — manages the active UI theme for Foundation.
 *
 * On startup, applies the last theme from localStorage immediately.
 * After login, resolves the correct theme via user preference.
 */
@Injectable({ providedIn: 'root' })
export class ThemeService {

    private static readonly STORAGE_KEY = 'foundation-theme';
    private static readonly SETTINGS_KEY = 'foundation-theme';
     ...
     * 1. User-level preference (from UserSettings)
     * 2. Built-in default ('default')
     */
    public initializeAfterLogin(): void {

        //
        // Try to get the user's personal theme preference
        //
        this.userSettingsService.getSetting(ThemeService.SETTINGS_KEY).subscribe(userTheme => {
            ...
            //
            // If no user preference, the constructor's localStorage
    ...
    public setTheme(themeId: string): void {
        ...
        //
        this.userSettingsService.setSetting(ThemeService.SETTINGS_KEY, themeId).subscribe();
    }
