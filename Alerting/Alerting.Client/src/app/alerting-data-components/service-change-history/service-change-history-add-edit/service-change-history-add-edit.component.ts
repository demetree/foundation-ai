...
@Component({
  selector: 'app-service-change-history-add-edit',
  templateUrl: './service-change-history-add-edit.component.html',
  styleUrls: ['./service-change-history-add-edit.component.scss']
})
export class ServiceChangeHistoryAddEditComponent {
  @ViewChild('serviceChangeHistoryModal') serviceChangeHistoryModal!: TemplateRef<any>;
  @Output() serviceChangeHistoryChanged = new Subject<ServiceChangeHistoryData[]>();
  @Input() serviceChangeHistorySubmitData: ServiceChangeHistorySubmitData | null = null;
  ...
  openModal(serviceChangeHistoryData?: ServiceChangeHistoryData) {

    if (serviceChangeHistoryData != null) {

      if (!this.serviceChangeHistoryService.userIsAlertingServiceChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Service Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.serviceChangeHistorySubmitData = this.serviceChangeHistoryService.ConvertToServiceChangeHistorySubmitData(serviceChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(serviceChangeHistoryData);

    } else {

      if (!this.serviceChangeHistoryService.userIsAlertingServiceChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Service Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.serviceChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const serviceChangeHistorySubmitData: ServiceChangeHistorySubmitData = {
        id: this.serviceChangeHistorySubmitData?.id || 0,
        serviceId: Number(formValue.serviceId),
        versionNumber: this.serviceChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateServiceChangeHistory(serviceChangeHistorySubmitData);
      } else {
        this.addServiceChangeHistory(serviceChangeHistorySubmitData);
      }
  }

  private addServiceChangeHistory(serviceChangeHistoryData: ServiceChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    serviceChangeHistoryData.versionNumber = 0;
    this.serviceChangeHistoryService.PostServiceChangeHistory(serviceChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newServiceChangeHistory) => {

        this.serviceChangeHistoryService.ClearAllCaches();

        this.serviceChangeHistoryChanged.next([newServiceChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/servicechangehistory', newServiceChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateServiceChangeHistory(serviceChangeHistoryData: ServiceChangeHistorySubmitData) {
    this.serviceChangeHistoryService.PutServiceChangeHistory(serviceChangeHistoryData.id, serviceChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedServiceChangeHistory) => {

        this.serviceChangeHistoryService.ClearAllCaches();

        this.serviceChangeHistoryChanged.next([updatedServiceChangeHistory]);

        this.alertService.showMessage("Service Change History updated successfully", '', MessageSeverity.success);
