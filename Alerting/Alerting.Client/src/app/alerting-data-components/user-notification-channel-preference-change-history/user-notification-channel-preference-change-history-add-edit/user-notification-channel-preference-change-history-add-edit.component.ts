...
@Component({
  selector: 'app-user-notification-channel-preference-change-history-add-edit',
  templateUrl: './user-notification-channel-preference-change-history-add-edit.component.html',
  styleUrls: ['./user-notification-channel-preference-change-history-add-edit.component.scss']
})
export class UserNotificationChannelPreferenceChangeHistoryAddEditComponent {
  @ViewChild('userNotificationChannelPreferenceChangeHistoryModal') userNotificationChannelPreferenceChangeHistoryModal!: TemplateRef<any>;
  @Output() userNotificationChannelPreferenceChangeHistoryChanged = new Subject<UserNotificationChannelPreferenceChangeHistoryData[]>();
  @Input() userNotificationChannelPreferenceChangeHistorySubmitData: UserNotificationChannelPreferenceChangeHistorySubmitData | null = null;
  ...
  openModal(userNotificationChannelPreferenceChangeHistoryData?: UserNotificationChannelPreferenceChangeHistoryData) {
    ...
    if (userNotificationChannelPreferenceChangeHistoryData != null) {
      ...
      if (!this.userNotificationChannelPreferenceChangeHistoryService.userIsAlertingUserNotificationChannelPreferenceChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read User Notification Channel Preference Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.userNotificationChannelPreferenceChangeHistorySubmitData = this.userNotificationChannelPreferenceChangeHistoryService.ConvertToUserNotificationChannelPreferenceChangeHistorySubmitData(userNotificationChannelPreferenceChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(userNotificationChannelPreferenceChangeHistoryData);

    } else {

      if (!this.userNotificationChannelPreferenceChangeHistoryService.userIsAlertingUserNotificationChannelPreferenceChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write User Notification Channel Preference Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.userNotificationChannelPreferenceChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const userNotificationChannelPreferenceChangeHistorySubmitData: UserNotificationChannelPreferenceChangeHistorySubmitData = {
        id: this.userNotificationChannelPreferenceChangeHistorySubmitData?.id || 0,
        userNotificationChannelPreferenceId: Number(formValue.userNotificationChannelPreferenceId),
        versionNumber: this.userNotificationChannelPreferenceChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistorySubmitData);
      } else {
        this.addUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistorySubmitData);
      }
  }

  private addUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistoryData: UserNotificationChannelPreferenceChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    userNotificationChannelPreferenceChangeHistoryData.versionNumber = 0;
    this.userNotificationChannelPreferenceChangeHistoryService.PostUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newUserNotificationChannelPreferenceChangeHistory) => {

        this.userNotificationChannelPreferenceChangeHistoryService.ClearAllCaches();

        this.userNotificationChannelPreferenceChangeHistoryChanged.next([newUserNotificationChannelPreferenceChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/usernotificationchannelpreferencechangehistory', newUserNotificationChannelPreferenceChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistoryData: UserNotificationChannelPreferenceChangeHistorySubmitData) {
    this.userNotificationChannelPreferenceChangeHistoryService.PutUserNotificationChannelPreferenceChangeHistory(userNotificationChannelPreferenceChangeHistoryData.id, userNotificationChannelPreferenceChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedUserNotificationChannelPreferenceChangeHistory) => {

        this.userNotificationChannelPreferenceChangeHistoryService.ClearAllCaches();

        this.userNotificationChannelPreferenceChangeHistoryChanged.next([updatedUserNotificationChannelPreferenceChangeHistory]);

        this.alertService.showMessage("User Notification Channel Preference Change History updated successfully", '', MessageSeverity.success);
