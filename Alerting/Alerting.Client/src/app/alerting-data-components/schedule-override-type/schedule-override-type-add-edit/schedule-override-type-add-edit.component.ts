...
@Component({
  selector: 'app-schedule-override-type-add-edit',
  templateUrl: './schedule-override-type-add-edit.component.html',
  styleUrls: ['./schedule-override-type-add-edit.component.scss']
})
export class ScheduleOverrideTypeAddEditComponent {
  @ViewChild('scheduleOverrideTypeModal') scheduleOverrideTypeModal!: TemplateRef<any>;
  @Output() scheduleOverrideTypeChanged = new Subject<ScheduleOverrideTypeData[]>();
  @Input() scheduleOverrideTypeSubmitData: ScheduleOverrideTypeSubmitData | null = null;
  ...
  openModal(scheduleOverrideTypeData?: ScheduleOverrideTypeData) {

    if (scheduleOverrideTypeData != null) {

      if (!this.scheduleOverrideTypeService.userIsAlertingScheduleOverrideTypeReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Schedule Override Types`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.scheduleOverrideTypeSubmitData = this.scheduleOverrideTypeService.ConvertToScheduleOverrideTypeSubmitData(scheduleOverrideTypeData);
      this.isEditMode = true;

      this.buildFormValues(scheduleOverrideTypeData);

    } else {

      if (!this.scheduleOverrideTypeService.userIsAlertingScheduleOverrideTypeWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Schedule Override Types`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.scheduleOverrideTypeModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const scheduleOverrideTypeSubmitData: ScheduleOverrideTypeSubmitData = {
        id: this.scheduleOverrideTypeSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateScheduleOverrideType(scheduleOverrideTypeSubmitData);
      } else {
        this.addScheduleOverrideType(scheduleOverrideTypeSubmitData);
      }
  }

  private addScheduleOverrideType(scheduleOverrideTypeData: ScheduleOverrideTypeSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    scheduleOverrideTypeData.active = true;
    scheduleOverrideTypeData.deleted = false;
    this.scheduleOverrideTypeService.PostScheduleOverrideType(scheduleOverrideTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newScheduleOverrideType) => {

        this.scheduleOverrideTypeService.ClearAllCaches();

        this.scheduleOverrideTypeChanged.next([newScheduleOverrideType]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/scheduleoverridetype', newScheduleOverrideType.id]);
        }
      },
      error: (err) => {
  ...
  private updateScheduleOverrideType(scheduleOverrideTypeData: ScheduleOverrideTypeSubmitData) {
    this.scheduleOverrideTypeService.PutScheduleOverrideType(scheduleOverrideTypeData.id, scheduleOverrideTypeData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedScheduleOverrideType) => {

        this.scheduleOverrideTypeService.ClearAllCaches();

        this.scheduleOverrideTypeChanged.next([updatedScheduleOverrideType]);

        this.alertService.showMessage("Schedule Override Type updated successfully", '', MessageSeverity.success);
