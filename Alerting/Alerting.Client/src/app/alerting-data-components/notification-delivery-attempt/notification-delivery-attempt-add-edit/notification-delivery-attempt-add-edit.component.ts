...
@Component({
  selector: 'app-notification-delivery-attempt-add-edit',
  templateUrl: './notification-delivery-attempt-add-edit.component.html',
  styleUrls: ['./notification-delivery-attempt-add-edit.component.scss']
})
export class NotificationDeliveryAttemptAddEditComponent {
  @ViewChild('notificationDeliveryAttemptModal') notificationDeliveryAttemptModal!: TemplateRef<any>;
  @Output() notificationDeliveryAttemptChanged = new Subject<NotificationDeliveryAttemptData[]>();
  @Input() notificationDeliveryAttemptSubmitData: NotificationDeliveryAttemptSubmitData | null = null;
  ...
  openModal(notificationDeliveryAttemptData?: NotificationDeliveryAttemptData) {
    ...
    if (notificationDeliveryAttemptData != null) {
      ...
      if (!this.notificationDeliveryAttemptService.userIsAlertingNotificationDeliveryAttemptReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Notification Delivery Attempts`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.notificationDeliveryAttemptSubmitData = this.notificationDeliveryAttemptService.ConvertToNotificationDeliveryAttemptSubmitData(notificationDeliveryAttemptData);
      this.isEditMode = true;
      this.objectGuid = notificationDeliveryAttemptData.objectGuid;

      this.buildFormValues(notificationDeliveryAttemptData);

    } else {

      if (!this.notificationDeliveryAttemptService.userIsAlertingNotificationDeliveryAttemptWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Notification Delivery Attempts`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.notificationDeliveryAttemptModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const notificationDeliveryAttemptSubmitData: NotificationDeliveryAttemptSubmitData = {
        id: this.notificationDeliveryAttemptSubmitData?.id || 0,
      ...
      if (this.isEditMode) {
        this.updateNotificationDeliveryAttempt(notificationDeliveryAttemptSubmitData);
      } else {
        this.addNotificationDeliveryAttempt(notificationDeliveryAttemptSubmitData);
      }
  }

  private addNotificationDeliveryAttempt(notificationDeliveryAttemptData: NotificationDeliveryAttemptSubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    notificationDeliveryAttemptData.active = true;
    notificationDeliveryAttemptData.deleted = false;
    this.notificationDeliveryAttemptService.PostNotificationDeliveryAttempt(notificationDeliveryAttemptData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newNotificationDeliveryAttempt) => {

        this.notificationDeliveryAttemptService.ClearAllCaches();

        this.notificationDeliveryAttemptChanged.next([newNotificationDeliveryAttempt]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/notificationdeliveryattempt', newNotificationDeliveryAttempt.id]);
        }
      },
      error: (err) => {
  ...
  private updateNotificationDeliveryAttempt(notificationDeliveryAttemptData: NotificationDeliveryAttemptSubmitData) {
    this.notificationDeliveryAttemptService.PutNotificationDeliveryAttempt(notificationDeliveryAttemptData.id, notificationDeliveryAttemptData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedNotificationDeliveryAttempt) => {

        this.notificationDeliveryAttemptService.ClearAllCaches();

        this.notificationDeliveryAttemptChanged.next([updatedNotificationDeliveryAttempt]);

        this.alertService.showMessage("Notification Delivery Attempt updated successfully", '', MessageSeverity.success);
