...
@Component({
  selector: 'app-integration-change-history-add-edit',
  templateUrl: './integration-change-history-add-edit.component.html',
  styleUrls: ['./integration-change-history-add-edit.component.scss']
})
export class IntegrationChangeHistoryAddEditComponent {
  @ViewChild('integrationChangeHistoryModal') integrationChangeHistoryModal!: TemplateRef<any>;
  @Output() integrationChangeHistoryChanged = new Subject<IntegrationChangeHistoryData[]>();
  @Input() integrationChangeHistorySubmitData: IntegrationChangeHistorySubmitData | null = null;
  ...
  openModal(integrationChangeHistoryData?: IntegrationChangeHistoryData) {
    ...
    if (integrationChangeHistoryData != null) {
      ...
      if (!this.integrationChangeHistoryService.userIsAlertingIntegrationChangeHistoryReader()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to read Integration Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
      }
      this.integrationChangeHistorySubmitData = this.integrationChangeHistoryService.ConvertToIntegrationChangeHistorySubmitData(integrationChangeHistoryData);
      this.isEditMode = true;

      this.buildFormValues(integrationChangeHistoryData);

    } else {

      if (!this.integrationChangeHistoryService.userIsAlertingIntegrationChangeHistoryWriter()) {
        this.alertService.showMessage(
          `${this.authService.currentUser?.userName} does not have permission to write Integration Change Histories`,
          '',
          MessageSeverity.info
        );
        return;
    ...
    this.modalRef = this.modalService.open(this.integrationChangeHistoryModal, {
      size: 'xl',
      scrollable: true,
      backdrop: 'static',
      keyboard: true,
  ...
  submitForm() {
    ...
    const integrationChangeHistorySubmitData: IntegrationChangeHistorySubmitData = {
        id: this.integrationChangeHistorySubmitData?.id || 0,
        integrationId: Number(formValue.integrationId),
        versionNumber: this.integrationChangeHistorySubmitData?.versionNumber ?? 0,
      ...
      if (this.isEditMode) {
        this.updateIntegrationChangeHistory(integrationChangeHistorySubmitData);
      } else {
        this.addIntegrationChangeHistory(integrationChangeHistorySubmitData);
      }
  }

  private addIntegrationChangeHistory(integrationChangeHistoryData: IntegrationChangeHistorySubmitData) {
    // Assign initial values to non-nullable control fields suitable for adding new data.
    integrationChangeHistoryData.versionNumber = 0;
    this.integrationChangeHistoryService.PostIntegrationChangeHistory(integrationChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (newIntegrationChangeHistory) => {

        this.integrationChangeHistoryService.ClearAllCaches();

        this.integrationChangeHistoryChanged.next([newIntegrationChangeHistory]);
        ...
        if (this.navigateToDetailsAfterAdd) {
          this.router.navigate(['/integrationchangehistory', newIntegrationChangeHistory.id]);
        }
      },
      error: (err) => {
  ...
  private updateIntegrationChangeHistory(integrationChangeHistoryData: IntegrationChangeHistorySubmitData) {
    this.integrationChangeHistoryService.PutIntegrationChangeHistory(integrationChangeHistoryData.id, integrationChangeHistoryData).pipe(
      finalize(() => this.isSaving = false)
    ).subscribe({
      next: (updatedIntegrationChangeHistory) => {

        this.integrationChangeHistoryService.ClearAllCaches();

        this.integrationChangeHistoryChanged.next([updatedIntegrationChangeHistory]);

        this.alertService.showMessage("Integration Change History updated successfully", '', MessageSeverity.success);
