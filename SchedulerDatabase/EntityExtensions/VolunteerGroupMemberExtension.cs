...
namespace Foundation.Scheduler.Database
{
	...
	public partial class VolunteerGroupMember : IVersionTrackedEntity<VolunteerGroupMember>, IAnonymousConvertible
	{
        ...
        /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="Exception"></exception>
        public static Task<ChangeHistoryToolset<VolunteerGroupMember, VolunteerGroupMemberChangeHistory>> GetChangeHistoryToolsetForWritingAsync(SchedulerContext context, Foundation.Security.Database.SecurityUser securityUser, bool insideTransaction = false, CancellationToken cancellationToken = default)
        {
            if (context == null)
            {
                throw new ArgumentNullException(nameof(context));
            ...
            // 
            return Task.FromResult(new ChangeHistoryToolset<VolunteerGroupMember, VolunteerGroupMemberChangeHistory>(context, securityUser.id, insideTransaction, cancellationToken));
        }
		...
		/// <summary>
		///
		/// Converts a VolunteerGroupMember to an OUTPUT Data Transfer Object.  This is the format to be used when serializing data to send back to client requests with nav properties to avoid using the VolunteerGroupMember Entity type directly.
		///
		/// </summary>
