...
namespace Foundation.Scheduler.Database
{
	...
	public partial class Batch : IVersionTrackedEntity<Batch>, IAnonymousConvertible
	{
        ...
        /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="Exception"></exception>
        public static Task<ChangeHistoryToolset<Batch, BatchChangeHistory>> GetChangeHistoryToolsetForWritingAsync(SchedulerContext context, Foundation.Security.Database.SecurityUser securityUser, bool insideTransaction = false, CancellationToken cancellationToken = default)
        {
            if (context == null)
            {
                throw new ArgumentNullException(nameof(context));
            ...
            // 
            return Task.FromResult(new ChangeHistoryToolset<Batch, BatchChangeHistory>(context, securityUser.id, insideTransaction, cancellationToken));
        }
		...
		/// <summary>
		///
		/// Converts a Batch to an OUTPUT Data Transfer Object.  This is the format to be used when serializing data to send back to client requests with nav properties to avoid using the Batch Entity type directly.
		///
		/// </summary>
