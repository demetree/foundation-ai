...
namespace Foundation.Scheduler.Database
{
	...
	public partial class RateSheet : IVersionTrackedEntity<RateSheet>, IAnonymousConvertible
	{
        ...
        /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="Exception"></exception>
        public static Task<ChangeHistoryToolset<RateSheet, RateSheetChangeHistory>> GetChangeHistoryToolsetForWritingAsync(SchedulerContext context, Foundation.Security.Database.SecurityUser securityUser, bool insideTransaction = false, CancellationToken cancellationToken = default)
        {
            if (context == null)
            {
                throw new ArgumentNullException(nameof(context));
            ...
            // 
            return Task.FromResult(new ChangeHistoryToolset<RateSheet, RateSheetChangeHistory>(context, securityUser.id, insideTransaction, cancellationToken));
        }
		...
		/// <summary>
		///
		/// Converts a RateSheet to an OUTPUT Data Transfer Object.  This is the format to be used when serializing data to send back to client requests with nav properties to avoid using the RateSheet Entity type directly.
		///
		/// </summary>
