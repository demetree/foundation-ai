...
namespace Foundation.Scheduler.Database
{
	...
	public partial class ShiftPatternDay : IVersionTrackedEntity<ShiftPatternDay>, IAnonymousConvertible
	{
        ...
        /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="Exception"></exception>
        public static Task<ChangeHistoryToolset<ShiftPatternDay, ShiftPatternDayChangeHistory>> GetChangeHistoryToolsetForWritingAsync(SchedulerContext context, Foundation.Security.Database.SecurityUser securityUser, bool insideTransaction = false, CancellationToken cancellationToken = default)
        {
            if (context == null)
            {
                throw new ArgumentNullException(nameof(context));
            ...
            // 
            return Task.FromResult(new ChangeHistoryToolset<ShiftPatternDay, ShiftPatternDayChangeHistory>(context, securityUser.id, insideTransaction, cancellationToken));
        }
		...
		/// <summary>
		///
		/// Converts a ShiftPatternDay to an OUTPUT Data Transfer Object.  This is the format to be used when serializing data to send back to client requests with nav properties to avoid using the ShiftPatternDay Entity type directly.
		///
		/// </summary>
