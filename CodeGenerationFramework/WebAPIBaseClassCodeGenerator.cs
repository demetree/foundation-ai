...
{
    public partial class WebAPICodeGeneratorFramework : CodeGenerationBase
    {
...
" + ((IsNetFramework48() == true) ? @"
        protected %MODULENAME%Context db = new %MODULENAME%Context();
" : @"
        protected %MODULENAME%Context db;       // Assigned through Dependency injection in implementations
") +
@"
        ...
        private static object fullSynchronizatinSyncRoot = new object();
        private static object userSynchronizationSyncRoot = new object();

        private const string LAST_SYNC_SETTING = ""%MODULENAME%_DataVisibilty_LastSync"";
        ...
        protected async Task<User> GetUserAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
        ...
        protected async Task<User> GetUserAsync(Guid tenantGuid, SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetListOfUserIdsForUserAndPeopleThatReportToThemAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
            ...
            List<User> userAndTheirReports = await GetListOfUsersForUserAndPeopleThatReportToThemAsync(securityUser);
        ...
        protected async Task<List<User>> GetListOfUsersForUserAndPeopleThatReportToThemAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
            ...
            if (user != null)
            {
                return await GetUserReportsAsync(user);
            }
            else
            {
                return null;
        ...
        protected async Task<List<User>> GetUserReportsAsync(User user, List<int> reportingStructureMembers = null)
        {
            //
            ...
            //
            List<User> directReports = await (from x in db.Users
                                        where
        ...
        protected List<User> GetUserReports(User user, List<int> reportingStructureMembers = null)
        {
            ...
            // See if the user has any reports.  If they do, then add them, and find their reports.
            //
            List<User> directReports = (from x in db.Users
                                        where
                                        x.reportsToUserId == user.id &&
                                        x.active == true &&
                                        x.deleted == false
        ...
        protected List<Organization> GetOrganizationsUserIsEntitledToReadAnythingAtAllFrom(SecurityUser securityUser = null, bool userIsAdmin = false)
        {
            ...
            //
            // Orgnaizations with explicit read refusal are excluded, as are implict team links via departments that have explicit read refusal
            //
            if (securityUser == null)
            {
                securityUser = GetSecurityUser();
            ...
            else
            {
                ...
                //
                // Read all data, then split into readable and unreadble lists
                // 
                List<OrganizationUser> ouList = (from
                        o in db.Organizations
                          join ou in db.OrganizationUsers on o.id equals ou.organizationId
                          join u in db.Users on ou.userId equals u.id
                          where
                          o.active == true &&
                          o.deleted == false &&
                          ou.active == true &&
                ...
                output = (from o in db.Organizations where explicitlyReadableOrgIds.Contains(o.id) select o).ToList();
                ...
                List<Organization> organizationsReadableViaDepartmentReadership = (from
                                                                                   o in db.Organizations
                                                                                      join d in db.Departments on o.id equals d.organizationId
                                                                                      join du in db.DepartmentUsers on d.id equals du.departmentId
                ...
                //
                // Exclude any organization with an explicit rule from consideration.  Remove links via explicitly unreadable deprtments
                //
                List<int> explicitlyUnreadableDepartmentIds = (from du in db.DepartmentUsers
                                                             join u in db.Users on du.userId equals u.id
                ...
                List<Organization> organizationsReadableViaTeamReadership = (from
                                                                             o in db.Organizations
                                                                                join d in db.Departments on o.id equals d.organizationId
                                                                                join t in db.Teams on d.id equals t.departmentId
        ...
        protected List<Department> GetDepartmentsUserIsEntitledToReadAnythingAtAllFrom(SecurityUser securityUser = null, bool userIsAdmin = false)
        {
            ...
            else
            {
                ...
                //
                // Read all data, then split into readable and unreadbly lists
                // 
                ...
                output = (from d in db.Departments where explicitlyReadableDepIds.Contains(d.id) select d).ToList();
                ...
                List<Department> departmentsReadableViaOrganizationReadership = (from
                                                                                 d in db.Departments
                                                                                 join o in db.Organizations on d.organizationId equals o.id
                                                                                 join ou in db.OrganizationUsers on o.id equals ou.organizationId
                                                                                 join u in db.Users on ou.userId equals u.id
                                                                                 where
        ...
        protected List<Team> GetTeamsUserIsEntitledToReadAnythingAtAllFrom(SecurityUser securityUser = null, bool userIsAdmin = false)
        {
            ...
            else
            {
                ...
                output = (from t in db.Teams where explicitlyReadableTeamIds.Contains(t.id) select t).ToList();
                ...
                //
                // Remove links via explicitly unreadable deprtments
                //
                List<int> explicitlyUnreadableDepartmentIds = (from du in db.DepartmentUsers
                                                               join u in db.Users on du.userId equals u.id
                                                               where u.objectGuid == securityUser.objectGuid &&
        ...
        protected async Task<List<int>> GetOrganizationIdsUserIsEntitledToReadFromAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
            ...
            List<int> output = await (from ou in db.OrganizationUsers
                                join u in db.Users on ou.userId equals u.id
                                where
                                ou.active == true &&
                                ou.deleted == false &&
        ...
        protected List<int> GetOrganizationIdsUserIsEntitledToReadFrom(SecurityUser securityUser = null)
        {
            ...
            List<int> output = (from ou in db.OrganizationUsers
                                join u in db.Users on ou.userId equals u.id
                                where
                                ou.active == true &&
                                ou.deleted == false &&
        ...
        protected async Task<List<int>> GetOrganizationIdsUserIsEntitledToReadImplicitlyFromForNullDepartmentAndTeamValuesAsync(SecurityUser securityUser = null)
        {
            //
            ...
            List<int> explicitlyUnreadableOrgIds = await (from ou in db.OrganizationUsers
                                                    join u in db.Users on ou.userId equals u.id
                                                    where
                                                    ou.active == true &&
                                                    ou.deleted == false &&
            ...
            List<int> explicitlyUnreadableDepIds = await (from du in db.DepartmentUsers
                                                    join u in db.Users on du.userId equals u.id
                                                    where
                                                    du.active == true &&
                                                    du.deleted == false &&
        ...
        protected List<int> GetOrganizationIdsUserIsEntitledToReadImplicitlyFromForNullDepartmentAndTeamValues(SecurityUser securityUser = null)
        {
            ...
            List<int> explicitlyUnreadableOrgIds = (from ou in db.OrganizationUsers
                                                    join u in db.Users on ou.userId equals u.id
                                                    where
                                                    ou.active == true &&
                                                    ou.deleted == false &&
            ...
            List<int> explicitlyUnreadableDepIds = (from du in db.DepartmentUsers
                                                    join u in db.Users on du.userId equals u.id
                                                    where
                                                    du.active == true &&
                                                    du.deleted == false &&
        ...
        protected async Task<List<int>> GetOrganizationIdsUserIsEntitledToChangeOwnerForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetOrganizationIdsUserIsEntitledToChangeHierarchyForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetOrganizationIdsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected List<Organization> GetOrganizationsUserIsEntitledToWriteTo(SecurityUser securityUser = null)
        {
            ...
            List<Organization> output = (from
                                o in db.Organizations
                                         join
                   ou in db.OrganizationUsers on o.id equals ou.organizationId
                                         join u in db.Users on ou.userId equals u.id
                                         where
                                         o.active == true &&
                                         o.deleted == false &&
        ...
        protected async Task<List<Organization>> GetOrganizationsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<Department>> GetDepartmentsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<Team>> GetTeamsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetDepartmentIdsUserIsEntitledToReadFromAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
            ...
            List<int> output = await (from du in db.DepartmentUsers
                                join u in db.Users on du.userId equals u.id
                                where
                                du.active == true &&
                                du.deleted == false &&
        ...
        protected List<int> GetDepartmentIdsUserIsEntitledToReadFrom(SecurityUser securityUser = null)
        {
            ...
            List<int> output = (from du in db.DepartmentUsers
                                join u in db.Users on du.userId equals u.id
                                where
                                du.active == true &&
                                du.deleted == false &&
        ...
        protected async Task<List<int>> GetDepartmentsIdsUserIsEntitledToReadImplicitlyFromForNullTeamValuesAsync(SecurityUser securityUser = null)
        {
            //
            ...
            List<int> departmentsReadableViaTeamReadership = await (from d in db.Departments
                                                                join t in db.Teams on d.id equals t.departmentId
                                                                join tu in db.TeamUsers on t.id equals tu.teamId
                                                                join u in db.Users on tu.userId equals u.id
                                                                where
        ...
        protected List<int> GetDepartmentsIdsUserIsEntitledToReadImplicitlyFromForNullTeamValues(SecurityUser securityUser = null)
        {
            ...
            List<int> departmentsReadableViaTeamReadership = (from d in db.Departments
                                                                join t in db.Teams on d.id equals t.departmentId
                                                                join tu in db.TeamUsers on t.id equals tu.teamId
                                                                join u in db.Users on tu.userId equals u.id
                                                                where
        ...
        protected async Task<List<int>> GetDepartmentIdsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetDepartmentIdsUserIsEntitledToChangeOwnerForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetDepartmentIdsUserIsEntitledToChangeHierarchyForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetTeamIdsUserIsEntitledToReadFromAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
            ...
            List<int> output = await (from tu in db.TeamUsers
                                join u in db.Users on tu.userId equals u.id
                                where
                                tu.active == true &&
                                tu.deleted == false &&
        ...
        protected List<int> GetTeamIdsUserIsEntitledToReadFrom(SecurityUser securityUser = null)
        {
            ...
            List<int> output = (from tu in db.TeamUsers
                                join u in db.Users on tu.userId equals u.id
                                where
                                tu.active == true &&
                                tu.deleted == false &&
        ...
        protected async Task<List<int>> GetTeamIdsUserIsEntitledToWriteToAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetTeamIdsUserIsEntitledToChangeOwnerForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        protected async Task<List<int>> GetTeamIdsUserIsEntitledToChangeHierarchyForAsync(SecurityUser securityUser = null)
        {
            if (securityUser == null)
            {
                securityUser = await GetSecurityUserAsync();
        ...
        {
            //
            // This will throw an error if the user can't write to the data visibiilty settings provided
            //
            ...
            if (team != null)       // check if the data being updated has a team.  If it does, then test for writeability at all levels in the team's hierarchy, starting at the org.
            {
                ...
                {
                    // write ability verified via team writershiup.  We can get out.
                    return;
                }
        ...
        protected void VerifyDataWritePrivilegeForUpdate(User owner, Team team, Department department, Organization organization, List<int> userAndTheirReports, List<int> teamsUserIsEntitledToWriteTo, List<int> departmentsUserIsEntitledToWriteTo, List<int> organizationsUserIsEntitledToWriteTo)
        {
            ...
            //
            // Any record that a user is the owner of, by way of their user ID being referenced in the userId field of the record, implies that the user can always read and write the 'Data' attriburtes on that record.
            // That means that record ownership grants a user permission to read and change data fields but not the ownership or hierarchy fields whcih are (userid, organizationId, departmentId, and teamId).  Changing those requires hierarchy granted permissions.
        ...
        {
            //
            // This will throw an error if the user can't can't change ownership with  the data visibiilty settings provided
            //
            // team param is the Team from the teamId from the data record
            // department param is the Department from the departmentId from the data record
            // organization param is the Organization from the organizationId from the data record
        ...
        {
            //
            // This will throw an error if the user can't can't change the hiearchy with  the data visibiilty settings provided
            //
            // team param is the Team from the teamId from the data record
            // department param is the Department from the departmentId from the data record
            // organization param is the Organization from the organizationId from the data record
            //
            // The list of int params are the ids of the items to which the current user is entitled to change hiearchy for
            //
            // Start at team level and work upwards to dep and org.  A User only needs to be able to change at to the most specific level of visibility for this to be verify successfully.
            //
            if (team != null)
        ...
        protected Organization GetDefaultOrganization(List<Organization> organizationsUserIsEntitledToWriteTo, SecurityUser securityUser, Department defaultDepartment)
        {
            ...
            //
            // If the security user doesn't explicity have a default organization set, but they have a default department, then use the department's organization to infer the user' default organization
            //
        ...
        protected Department GetDefaultDepartment(List<Department> departmentsUserIsEntitledToWriteTo, SecurityUser securityUser, Team defaultTeam)
        {
            ...
            //
            // If the security user doesn't explicity have a default department set, but they have a default team, then use the team's department to infer the user' default department
            //
        ...
        {
            return (from x in db.Departments
                    where x.id == id
                    select x).FirstOrDefault();
        }
        ...
        {
            return (from x in db.Organizations
                    where x.id == id
                    select x).FirstOrDefault();
        }
        ...
        {
            return await (from x in db.Users
                    where x.id == id
                    select x).FirstOrDefaultAsync();
        }


        protected async Task<Team> GetTeamAsync(int id)
        {
            return await (from x in db.Teams
                    where x.id == id
                    select x).FirstOrDefaultAsync();
        }


        protected async Task<Department> GetDepartmentAsync(int id)
        {
            return await (from x in db.Departments
                    where x.id == id
                    select x).FirstOrDefaultAsync();
        }


        protected async Task<Organization> GetOrganizationAsync(int id)
        {
            return await (from x in db.Organizations
                    where x.id == id
                    select x).FirstOrDefaultAsync();
        }
            ...
            lock (userSynchronizationSyncRoot)
            {
                try
                {
                    ...
                    using (SecurityContext sdb = new SecurityContext())
                    {
                        ...
                        // string has BinaryGuid=False in its options.  For example, <add name=""Security"" connectionString=""data source=|DataDirectory|\Security\Security.db;BinaryGUID=false;"" providerName=""System.Data.SQLite"" />
                        //
                        SecurityUser securityUser = (from x in sdb.SecurityUsers where x.objectGuid == securityUserObjectGuid select x).FirstOrDefault();
                        ...
                        securityTenant = (from x in sdb.SecurityTenants
                        ...
                        if (securityTenant.active == true && securityTenant.deleted == false)
                        {
                            ...
                            securityTenantUser = (from x in sdb.SecurityTenantUsers
                            ...
                            if (securityTenantUser != null)
                            {
                                ...
                                //
                                using (%MODULENAME%Context adb = new %MODULENAME%Context())
                                {
                                    adb.Configuration.AutoDetectChangesEnabled = true;
                                    adb.Configuration.ValidateOnSaveEnabled = false;
                                    ...
                                    {
                                        User managerUser = (from x in adb.Users where x.objectGuid == securityTenantUser.SecurityUser.ReportsToSecurityUser.objectGuid select x).FirstOrDefault();
                                    ...
                                    {
                                        UserTitle userTitle = (from x in adb.UserTitles where x.objectGuid == securityTenantUser.SecurityUser.SecurityUserTitle.objectGuid select x).FirstOrDefault();
                                        ...
                                        if (userTitle == null)
                                        {
                                            ...
                                            using (%MODULENAME%Context utdb = new %MODULENAME%Context())
                                            {
                                                utdb.Configuration.AutoDetectChangesEnabled = true;
                                                utdb.Configuration.ValidateOnSaveEnabled = false;

                                                utdb.UserTitles.Add(userTitle);
                        ...
                        if (userIsModuleAdministrator == true)
                        {
                            ...
                            //
                            securityOrganizations = (from so in sdb.SecurityOrganizations
                                                     where
                        ...
                        else
                        {
                            ...
                            //
                            securityOrganizations = (from so in sdb.SecurityOrganizations
                                                     join sou in sdb.SecurityOrganizationUsers on so.id equals sou.securityOrganizationId
                                                     where
                                                     so.securityTenantId == securityTenant.id &&
                            ...
                            List<SecurityOrganization> securityOrganizationsViaDepartment = (from so in sdb.SecurityOrganizations
                                                                                             join sd in sdb.SecurityDepartments on so.id equals sd.securityOrganizationId
                                                                                             join sdu in sdb.SecurityDepartmentUsers on sd.id equals sdu.securityDepartmentId
                                                                                             where
                            ...
                            //
                            List<SecurityOrganization> securityOrganizationsViaTeam = (from so in sdb.SecurityOrganizations
                                                                                       join sd in sdb.SecurityDepartments on so.id equals sd.securityOrganizationId
                                                                                       join st in sdb.SecurityTeams on sd.id equals st.securityDepartmentId
                                                                                       join stu in sdb.SecurityTeamUsers on st.id equals stu.securityTeamId
                                                                                       where
                                                                                       currentOrganizationIds.Contains(so.id) == false &&
                                                                                       so.securityTenantId == securityTenant.id &&
                        ...
                        foreach (SecurityOrganization securityOrganization in securityOrganizations)
                        {
                            ...
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                organization = (from x in adb.Organizations where x.objectGuid == securityOrganization.objectGuid select x).FirstOrDefault();
                                ...
                                if (organization == null)
                                {
                                    ...
                                    adb.Organizations.Add(organization);
                        ...
                        if (userIsModuleAdministrator == true)
                        {
                            ...
                            //
                            securityDepartments = (from sd in sdb.SecurityDepartments
                                                   join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                                   where
                                                   so.securityTenantId == securityTenant.id 
                                                   orderby sd.id
                                                   select sd)
                        ...
                        else 
                        {
                            ...
                            List<SecurityDepartment> securityDepartmentsViaTeam = (from sd in sdb.SecurityDepartments
                                                                                   join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                                                                   join st in sdb.SecurityTeams on sd.id equals st.securityDepartmentId
                                                                                   join stu in sdb.SecurityTeamUsers on st.id equals stu.securityTeamId
                                                                                   where
                            ...
                            List<SecurityDepartment> securityDepartmentsViaOrg = (from sd in sdb.SecurityDepartments
                                                                                  join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                                                                  join sou in sdb.SecurityOrganizationUsers on so.id equals sou.securityOrganizationId
                                                                                  where
                                                                                  currentDepartmentIds.Contains(sd.id) == false &&
                        ...
                        foreach (SecurityDepartment securityDepartment in securityDepartments)
                        {
                            ...
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                department = (from x in adb.Departments where x.objectGuid == securityDepartment.objectGuid select x).FirstOrDefault();
                                ...
                                if (department == null)
                                {
                                    ...
                                    adb.Departments.Add(department);
                                ...
                                //
                                Organization organization = (from o in adb.Organizations
                        ...
                        if (userIsModuleAdministrator == true)
                        {
                            ...
                            //
                            securityTeams = (from st in sdb.SecurityTeams
                                             join sd in sdb.SecurityDepartments on st.securityDepartmentId equals sd.id
                                             join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                             where
                                             so.securityTenantId == securityTenant.id 
                                             orderby st.id
                        ...
                        else
                        {
                            ...
                            List<SecurityTeam> securityTeamsViaDepartment = (from st in sdb.SecurityTeams
                                                                             join sd in sdb.SecurityDepartments on st.securityDepartmentId equals sd.id
                                                                             join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                                                             join sdu in sdb.SecurityDepartmentUsers on sd.id equals sdu.securityDepartmentId
                                                                             where
                            ...
                            List<SecurityTeam> securityTeamsViaOrg = (from st in sdb.SecurityTeams
                                                                      join sd in sdb.SecurityDepartments on st.securityDepartmentId equals sd.id
                                                                      join so in sdb.SecurityOrganizations on sd.securityOrganizationId equals so.id
                                                                      join sou in sdb.SecurityOrganizationUsers on so.id equals sou.securityOrganizationId
                                                                      where
                        ...
                        foreach (SecurityTeam securityTeam in securityTeams)
                        {
                            ...
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                Team team = (from x in adb.Teams where x.objectGuid == securityTeam.objectGuid select x).FirstOrDefault();
                                ...
                                // Copy the property state from the security department
                                //
                                Department department = (from d in adb.Departments
        ...
        {
            lock (fullSynchronizatinSyncRoot)
            {
                try
                {
                        ...
                        forceSyncInSpiteOfLastRunDate == true)
                    {
                        ...
                        using (SecurityContext sdb = new SecurityContext())
                        {
                            sdb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                            Foundation.Utility.CreateAuditEvent(""User Title synchronization is starting."");

                            List<SecurityUserTitle> securityUserTitles = (from x in sdb.SecurityUserTitles select x).AsNoTracking().ToList();

                            foreach (SecurityUserTitle securityUserTitle in securityUserTitles)
                            {
                                using (%MODULENAME%Context adb = new %MODULENAME%Context())
                                {
                                    adb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                                    adb.Configuration.AutoDetectChangesEnabled = true;
                                    adb.Configuration.ValidateOnSaveEnabled = false;

                                    UserTitle userTitle = (from x in adb.UserTitles where x.objectGuid == securityUserTitle.objectGuid select x).FirstOrDefault();
                            ...
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                List<int> userTitlesOnFile = (from x in adb.UserTitles select x.id).ToList();

                                foreach (int userTitleId in userTitlesOnFile)
                                {
                                    if (userTitlesAccountedFor.ContainsKey(userTitleId) == false)
                                    {
                                        UserTitle ut = (from x in adb.UserTitles where x.id == userTitleId select x).FirstOrDefault();
                        ...
                        using (SecurityContext sdb = new SecurityContext())
                        {
                            securityTenantCount = (from x in sdb.SecurityTenants
                                                   where 
                        ...
                        while (remainingTenantRecords > 0)
                        {
                            ...
                            using (SecurityContext sdb = new SecurityContext())
                            {
                                securityTenants = (from x in sdb.SecurityTenants
                                                   where 
                                                   (tenantGuid == null || x.objectGuid == tenantGuid)
                                                   orderby x.id
                                                   select x)
                            ...
                            foreach (SecurityTenant securityTenant in securityTenants)
                            {
                                ...
                                {
                                    //
                                    // First, go through all the users that are defined for this tenant in the security system, and make sure that they are in the same in the application database.
                                    //
                                    ...
                                    using (SecurityContext sdb = new SecurityContext())
                                    {
                                        securityTenantUserCount = (from x in sdb.SecurityTenantUsers where x.securityTenantId == securityTenant.id select x).Count();
                                    }
                                    ...
                                    while (remainingTenantUserRecords > 0)
                                    {
                                        ...
                                        using (SecurityContext sdb = new SecurityContext())
                                        {
                                            securityTenantUsers = (from x in sdb.SecurityTenantUsers
                                                                   where x.securityTenantId == securityTenant.id
                                                                   orderby x.id
                                                                   select x)
                                                                    .Include(""SecurityUser.ReportsToSecurityUser"")
                                        ...
                                        foreach (SecurityTenantUser securityTenantUser in securityTenantUsers)
                                        {
                                            ...
                                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                                            {
                                                ...
                                                if (securityUser.reportsToSecurityUserId.HasValue == true)
                                                {
                                                    User managerUser = (from x in adb.Users where x.objectGuid == securityUser.ReportsToSecurityUser.objectGuid select x).FirstOrDefault();
                                                ...
                                                if (securityUser.securityUserTitleId.HasValue == true)
                                                {
                                                    UserTitle userTitle = (from x in adb.UserTitles where x.objectGuid == securityUser.SecurityUserTitle.objectGuid select x).FirstOrDefault();
                                                    ...
                                                    if (userTitle == null)
                                                    {
                                                        ...
                                                        using (%MODULENAME%Context utdb = new %MODULENAME%Context())
                                                        {
                                                            utdb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);
                                    ...
                                    //
                                    // If any are found, then deactive and delete them, as they are bad/orphan data and shouldn't be there.
                                    //
                                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                                    {
                                        adb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);
                                        ...
                                        //
                                        // This list of ints is highly unlikely to ever throw an out of memory error, so we're not going to page here.
                                        //
                                        List<int> allUsersInTenant = (from x in adb.Users where x.tenantGuid == securityTenant.objectGuid select x.id).ToList();
                                    ...
                                    using (SecurityContext sdb = new SecurityContext())
                                    {
                                        ...
                                        securityOrganizationCount = (from x in sdb.SecurityOrganizations where x.securityTenantId == securityTenant.id select x).Count();
                                    }
                                    ...
                                    while (remainingOrganizationRecords > 0)
                                    {
                                        ...
                                        using (SecurityContext sdb = new SecurityContext())
                                        {
                                            ...
                                            securityOrganizations = (from o in sdb.SecurityOrganizations
                                        ...
                                        foreach (SecurityOrganization securityOrganization in securityOrganizations)
                                        {
                                            ...
                                            using (SecurityContext sdb = new SecurityContext())
                                            {
                                                ...
                                                securityDepartmentCount = (from x in sdb.SecurityDepartments where x.securityOrganizationId == securityOrganization.id select x).Count();
                                            }
                                            ...
                                            while (remainingDepartmentRecords > 0)
                                            {
                                                ...
                                                using (SecurityContext sdb = new SecurityContext())
                                                {
                                                    ...
                                                    securityDepartments = (from d in sdb.SecurityDepartments
                                                ...
                                                foreach (SecurityDepartment securityDepartment in securityDepartments)
                                                {
                                                    ...
                                                    using (SecurityContext sdb = new SecurityContext())
                                                    {
                                                        ...
                                                        securityTeamCount = (from x in sdb.SecurityTeams where x.securityDepartmentId == securityDepartment.id select x).Count();
                                                    }
                                                    ...
                                                    while (remainingTeamRecords > 0)
                                                    {
                                                        ...
                                                        using (SecurityContext sdb = new SecurityContext())
                                                        {
                                                            ...
                                                            securityTeams = (from t in sdb.SecurityTeams
                                    ...
                                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                                    {
                                        ...
                                        List<int> allOrganizationsInTenant = (from x in adb.Organizations where x.tenantGuid == securityTenant.objectGuid select x.id).ToList();
                                        ...
                                        foreach (int organizationToCheckId in allOrganizationsInTenant)
                                        {
                                            ...
                                            List<int> allDepartmentsInOrganization = (from x in adb.Departments where x.organizationId == organizationToCheckId select x.id).ToList();
                                            ...
                                            foreach (int departmentToCheckId in allDepartmentsInOrganization)
                                            {
                                                ...
                                                List<int> allTeamsInDepartment = (from x in adb.Teams where x.departmentId == departmentToCheckId select x.id).ToList();
                                ...
                                else
                                {
                                    ...
                                    //
                                    using (%MODULENAME%Context orgDb = new %MODULENAME%Context())
                                    {
                                        orgDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);
                                        ...
                                        //
                                        List<Organization> organizations = (from x in orgDb.Organizations where x.tenantGuid == securityTenant.objectGuid select x).ToList();
        ...
        private static void SynchronizeOrganizationUsers(SecurityOrganization securityOrganization, Guid tenantGuid, SecurityUser securityUser = null)
        {
            ...
            using (%MODULENAME%Context adb = new %MODULENAME%Context())
            {
                ...
                applicationOrganization = (from x in adb.Organizations where x.objectGuid == securityOrganization.objectGuid select x).FirstOrDefault();
            }
            ...
            using (SecurityContext sdb = new SecurityContext())
            {
                ...
                if (securityUser != null)
                {
                    securityOrganizationUserCount = (from x in sdb.SecurityOrganizationUsers
            ...
            while (remainingOrganizationUserRecords > 0)
            {
                ...
                using (SecurityContext sdb = new SecurityContext())
                {
                    ...
                    if (securityUser != null)
                    {
                        securityOrganizationUsers = (from x in sdb.SecurityOrganizationUsers
                                                     where x.securityOrganizationId == securityOrganization.id
                                                     && x.securityUserId == securityUser.id
                                                     orderby x.id
                                                     select x)
                ...
                foreach (SecurityOrganizationUser securityOrganizationUser in securityOrganizationUsers)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        OrganizationUser organizationUser = (from x in adb.OrganizationUsers where x.objectGuid == securityOrganizationUser.objectGuid select x).FirstOrDefault();
                        ...
                        if (organizationUser == null)
                        {
                            ...
                            adb.OrganizationUsers.Add(organizationUser);
                ...
                if (reconcileOrphans == true)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        allOrganizationUsersInOrganization = (from x in adb.OrganizationUsers where x.organizationId == applicationOrganization.id select x.id).ToList();
                    }
                    ...
                    foreach (int organizationUserId in allOrganizationUsersInOrganization)
                    {
                        if (applicationDatabaseOrganizationUserIdsThatHaveBeenReconciled.ContainsKey(organizationUserId) == false)
                        {
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                OrganizationUser organizationUser = (from x in adb.OrganizationUsers where x.id == organizationUserId select x).FirstOrDefault();
        ...
        private static void SynchronizeDepartmentUsers(SecurityDepartment securityDepartment, Guid tenantGuid, SecurityUser securityUser)
        {
            ...
            using (%MODULENAME%Context adb = new %MODULENAME%Context())
            {
                ...
                applicationDepartment = (from x in adb.Departments where x.objectGuid == securityDepartment.objectGuid select x).FirstOrDefault();
            }
            ...
            using (SecurityContext sdb = new SecurityContext())
            {
                ...
                if (securityUser != null)
                {
                    securityDepartmentUserCount = (from x in sdb.SecurityDepartmentUsers
            ...
            while (remainingDepartmentUserRecords > 0)
            {
                ...
                using (SecurityContext sdb = new SecurityContext())
                {
                    ...
                    if (securityUser != null)
                    {
                        securityDepartmentUsers = (from x in sdb.SecurityDepartmentUsers
                                                   where x.securityDepartmentId == securityDepartment.id &&
                                                   x.securityUserId == securityUser.id
                                                   orderby x.id
                                                   select x)
                ...
                foreach (SecurityDepartmentUser securityDepartmentUser in securityDepartmentUsers)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        DepartmentUser departmentUser = (from x in adb.DepartmentUsers where x.objectGuid == securityDepartmentUser.objectGuid select x).FirstOrDefault();
                        ...
                        if (departmentUser == null)
                        {
                            ...
                            adb.DepartmentUsers.Add(departmentUser);
                ...
                if (reconcileOrphans == true)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        allDepartmentUsersInDepartment = (from x in adb.DepartmentUsers where x.departmentId == applicationDepartment.id select x.id).ToList();
                    }
                    ...
                    foreach (int departmentUserId in allDepartmentUsersInDepartment)
                    {
                        if (applicationDatabaseDepartmentUserIdsThatHaveBeenReconciled.ContainsKey(departmentUserId) == false)
                        {
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                DepartmentUser departmentUser = (from x in adb.DepartmentUsers where x.id == departmentUserId select x).FirstOrDefault();
        ...
        private static void SynchronizeTeamUsers(SecurityTeam securityTeam, Guid tenantGuid, SecurityUser securityUser = null)
        {
            ...
            using (%MODULENAME%Context adb = new %MODULENAME%Context())
            {
                ...
                applicationTeam = (from x in adb.Teams where x.objectGuid == securityTeam.objectGuid select x).FirstOrDefault();
            }
            ...
            using (SecurityContext sdb = new SecurityContext())
            {
                ...
                if (securityUser != null)
                {
                    securityTeamUserCount = (from x in sdb.SecurityTeamUsers 
            ...
            while (remainingTeamUserRecords > 0)
            {
                ...
                using (SecurityContext sdb = new SecurityContext())
                {
                    ...
                    if (securityUser != null)
                    {
                        securityTeamUsers = (from x in sdb.SecurityTeamUsers
                                             where x.securityTeamId == securityTeam.id &&
                                             x.securityUserId == securityUser.id
                                             orderby x.id
                                             select x)
                ...
                foreach (SecurityTeamUser securityTeamUser in securityTeamUsers)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        TeamUser teamUser = (from x in adb.TeamUsers where x.objectGuid == securityTeamUser.objectGuid select x).FirstOrDefault();
                ...
                if (reconcileOrphans == true)
                {
                    ...
                    using (%MODULENAME%Context adb = new %MODULENAME%Context())
                    {
                        ...
                        allTeamUsersInTeam = (from x in adb.TeamUsers where x.teamId == applicationTeam.id select x.id).ToList();
                    }
                    ...
                    foreach (int teamUserId in allTeamUsersInTeam)
                    {
                        if (applicationDatabaseTeamUserIdsThatHaveBeenReconciled.ContainsKey(teamUserId) == false)
                        {
                            using (%MODULENAME%Context adb = new %MODULENAME%Context())
                            {
                                ...
                                TeamUser teamUser = (from x in adb.TeamUsers where x.id == teamUserId select x).FirstOrDefault();
        ...
        {
            //
            // This is designed to apply an active/deleted state across all elements of an organization and its chidren
            //
            using (%MODULENAME%Context userDb = new %MODULENAME%Context())
            {
                userDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                userDb.Configuration.AutoDetectChangesEnabled = true;
                userDb.Configuration.ValidateOnSaveEnabled = false;

                //
                User user = (from x in userDb.Users where x.id == userId select x).FirstOrDefault();
        ...
        private static void SetOrganizationActiveDeletedState(int organizationId, bool active, bool deleted)
        {
            ...
            using (%MODULENAME%Context orgDb = new %MODULENAME%Context())
            {
                ...
                orgDb.Configuration.AutoDetectChangesEnabled = true;
                orgDb.Configuration.ValidateOnSaveEnabled = false;

                //
                // Get the orgnization sent in as a param
                //
                Organization org = (from x in orgDb.Organizations where x.id == organizationId select x).FirstOrDefault();
                ...
                //
                using (%MODULENAME%Context oudb = new %MODULENAME%Context())
                {
                    oudb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                    oudb.Configuration.AutoDetectChangesEnabled = true;
                    oudb.Configuration.ValidateOnSaveEnabled = false;
                    ...
                    oudb.Database.ExecuteSqlCommand(sql);

                    oudb.SaveChanges();
                }

                using (%MODULENAME%Context departmentDb = new %MODULENAME%Context())
                {
                    departmentDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                    // To-Do: make this loop a SQL update
                    List<int> organizationDepartmentIds = (from x in departmentDb.Departments where x.organizationId == org.id select x.id).ToList();
        ...
        {
            //
            // This is designed to apply an active/deleted state across all elements of a department and its chidren
            //
            using (%MODULENAME%Context departmentDb = new %MODULENAME%Context())
            {
                departmentDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                departmentDb.Configuration.AutoDetectChangesEnabled = true;
                departmentDb.Configuration.ValidateOnSaveEnabled = false;

                Department department = (from x in departmentDb.Departments where x.id == departmentId select x).FirstOrDefault();
                ...
                using (%MODULENAME%Context dudb = new %MODULENAME%Context())
                {
                    dudb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                    dudb.Configuration.AutoDetectChangesEnabled = true;
                    dudb.Configuration.ValidateOnSaveEnabled = false;
                    ...
                    dudb.Database.ExecuteSqlCommand(sql);

                    dudb.SaveChanges();
                }


                using (%MODULENAME%Context teamDb = new %MODULENAME%Context())
                {
                    teamDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                    // To-Do: make this loop a SQL update
                    List<int> departmentTeamIds = (from x in teamDb.Teams where x.departmentId == department.id select x.id).ToList();
        ...
        {
            //
            // This is designed to apply an active/deleted state across all elements of a team and its chidren
            //
            using (%MODULENAME%Context teamDb = new %MODULENAME%Context())
            {
                teamDb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                teamDb.Configuration.AutoDetectChangesEnabled = true;
                teamDb.Configuration.ValidateOnSaveEnabled = false;

                Team team = (from x in teamDb.Teams where x.id == teamId select x).FirstOrDefault();

                team.active = active;
                team.deleted = deleted;

                using (%MODULENAME%Context tudb = new %MODULENAME%Context())
                {
                    tudb.Database.SetCommandTimeout(SYNC_COMMAND_TIMEOUT);

                    tudb.Configuration.AutoDetectChangesEnabled = true;
                    tudb.Configuration.ValidateOnSaveEnabled = false;

                    string sql = ""UPDATE [%MODULENAME%].[TeamUser] SET active = "" + (active == true ? ""1"" : ""0"") + "", deleted = "" + (deleted == true ? ""1"" : ""0"") + "" WHERE teamId = "" + team.id.ToString();
                    tudb.Database.ExecuteSqlCommand(sql);
