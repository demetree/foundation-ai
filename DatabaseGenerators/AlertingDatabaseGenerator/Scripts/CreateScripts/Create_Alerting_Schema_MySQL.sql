...
CREATE TABLE `EscalationPolicy`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_EscalationPolicy_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the EscalationPolicy table's tenantGuid and name fields.
);
...
CREATE TABLE `Service`(
	...
	`name` VARCHAR(100) NOT NULL,
	`description` VARCHAR(500) NULL,
	`ownerTeamObjectGuid` CHAR(38) NULL,		-- References Security.SecurityTeam.objectGuid - logical owner.
	`versionNumber` INT NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`escalationPolicyId`) REFERENCES `EscalationPolicy`(`id`),		-- Foreign key to the EscalationPolicy table.
	UNIQUE `UC_Service_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the Service table's tenantGuid and name fields.
);
...
CREATE TABLE `EscalationRule`(
	...
	`targetObjectGuid` CHAR(38) NULL,		-- References Security.SecurityUser/SecurityTeam or Alerting.OnCallSchedule objectGuid.
	`versionNumber` INT NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
...
CREATE TABLE `OnCallSchedule`(
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_OnCallSchedule_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the OnCallSchedule table's tenantGuid and name fields.
);
...
CREATE TABLE `ScheduleLayer`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`onCallScheduleId`) REFERENCES `OnCallSchedule`(`id`),		-- Foreign key to the OnCallSchedule table.
	UNIQUE `UC_ScheduleLayer_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the ScheduleLayer table's tenantGuid and name fields.
);
...
CREATE TABLE `ScheduleLayerMember`(
	...
	`securityUserObjectGuid` CHAR(38) NOT NULL,		-- References Security.SecurityUser.objectGuid
	`versionNumber` INT NOT NULL DEFAULT 1,		-- The version number of this record.  Increased by one each time the record changes, and the change history is tracked in the table's change history table.
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`scheduleLayerId`) REFERENCES `ScheduleLayer`(`id`),		-- Foreign key to the ScheduleLayer table.
	UNIQUE `UC_ScheduleLayerMember_tenantGuid_scheduleLayerId_position_Unique`( `tenantGuid`, `scheduleLayerId`, `position` ) 		-- Uniqueness enforced on the ScheduleLayerMember table's tenantGuid and scheduleLayerId and position fields.
);
...
CREATE TABLE `Integration`(
	...
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`serviceId`) REFERENCES `Service`(`id`),		-- Foreign key to the Service table.
	UNIQUE `UC_Integration_tenantGuid_name_Unique`( `tenantGuid`, `name` ) 		-- Uniqueness enforced on the Integration table's tenantGuid and name fields.
);
...
CREATE TABLE `UserNotificationPreference`(
	`id` INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`securityUserObjectGuid` CHAR(38) NOT NULL UNIQUE,		-- References Security.SecurityUser.objectGuid - one preference row per user.
	...
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	UNIQUE `UC_UserNotificationPreference_tenantGuid_securityUserObjectGuid_active_deleted_Unique`( `tenantGuid`, `securityUserObjectGuid`, `active`, `deleted` ) 		-- Uniqueness enforced on the UserNotificationPreference table's tenantGuid and securityUserObjectGuid and active and deleted fields.
);
...
CREATE INDEX `I_UserNotificationPreferenceChangeHistory_tenantGuid_versionNumb` ON `UserNotificationPreferenceChangeHistory` (`tenantGuid`, `versionNumber`);
...
CREATE INDEX `I_UserNotificationPreferenceChangeHistory_tenantGuid_userNotific` ON `UserNotificationPreferenceChangeHistory` (`tenantGuid`, `userNotificationPreferenceId`, `versionNumber`, `timeStamp`, `userId`);
...
CREATE TABLE `UserNotificationChannelPreference`(
	...
	UNIQUE `UC_UserNotificationChannelPreference_tenantGuid_userNotificationPreferenceId_notificationChannelTypeId_Unique`( `tenantGuid`, `userNotificationPreferenceId`, `notificationChannelTypeId` ) 		-- Uniqueness enforced on the UserNotificationChannelPreference table's tenantGuid and userNotificationPreferenceId and notificationChannelTypeId fields.
);
...
CREATE INDEX `I_UserNotificationChannelPreference_tenantGuid_userNotificationP` ON `UserNotificationChannelPreference` (`tenantGuid`, `userNotificationPreferenceId`);

-- Index on the UserNotificationChannelPreference table's tenantGuid,notificationChannelTypeId fields.
CREATE INDEX `I_UserNotificationChannelPreference_tenantGuid_notificationChann` ON `UserNotificationChannelPreference` (`tenantGuid`, `notificationChannelTypeId`);
...
CREATE INDEX `I_UserNotificationChannelPreferenceChangeHistory_tenantGuid_vers` ON `UserNotificationChannelPreferenceChangeHistory` (`tenantGuid`, `versionNumber`);

-- Index on the UserNotificationChannelPreferenceChangeHistory table's tenantGuid,timeStamp fields.
CREATE INDEX `I_UserNotificationChannelPreferenceChangeHistory_tenantGuid_time` ON `UserNotificationChannelPreferenceChangeHistory` (`tenantGuid`, `timeStamp`);

-- Index on the UserNotificationChannelPreferenceChangeHistory table's tenantGuid,userId fields.
CREATE INDEX `I_UserNotificationChannelPreferenceChangeHistory_tenantGuid_user` ON `UserNotificationChannelPreferenceChangeHistory` (`tenantGuid`, `userId`);
...
CREATE TABLE `Incident`(
	...
	`currentAssigneeObjectGuid` CHAR(38) NULL,
	...
	FOREIGN KEY (`escalationRuleId`) REFERENCES `EscalationRule`(`id`),		-- Foreign key to the EscalationRule table.
	UNIQUE `UC_Incident_tenantGuid_incidentKey_Unique`( `tenantGuid`, `incidentKey` ) 		-- Uniqueness enforced on the Incident table's tenantGuid and incidentKey fields.
);
...
CREATE TABLE `IncidentNote`(
	...
	`tenantGuid` CHAR(38) NOT NULL,		-- The guid for the Tenant to which this record belongs.
	`incidentId` INT NOT NULL,		-- Link to the Incident table.
	`authorObjectGuid` CHAR(38) NOT NULL,
	`createdAt` DATETIME NOT NULL,
...
CREATE TABLE `IncidentNotification`(
	...
	`userObjectGuid` CHAR(38) NOT NULL,		-- Resolved Security.SecurityUser.objectGuid that was notified.
	`firstNotifiedAt` DATETIME NOT NULL,
	`lastNotifiedAt` DATETIME NULL,
	`acknowledgedAt` DATETIME NULL,
	`acknowledgedByObjectGuid` CHAR(38) NULL,
	`objectGuid` CHAR(38) NOT NULL UNIQUE,		-- Unique identifier for this table.
	`active` BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	`deleted` BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY (`incidentId`) REFERENCES `Incident`(`id`),		-- Foreign key to the Incident table.
...
CREATE INDEX `I_NotificationDeliveryAttempt_tenantGuid_incidentNotificationId` ON `NotificationDeliveryAttempt` (`tenantGuid`, `incidentNotificationId`);

-- Index on the NotificationDeliveryAttempt table's tenantGuid,notificationChannelTypeId fields.
CREATE INDEX `I_NotificationDeliveryAttempt_tenantGuid_notificationChannelType` ON `NotificationDeliveryAttempt` (`tenantGuid`, `notificationChannelTypeId`);
...
CREATE INDEX `I_NotificationDeliveryAttempt_tenantGuid_incidentNotificationId_` ON `NotificationDeliveryAttempt` (`tenantGuid`, `incidentNotificationId`, `notificationChannelTypeId`);
