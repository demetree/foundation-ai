...
INSERT INTO "SecurityTenant" ( "name", "description", "active", "deleted", "objectGuid" ) VALUES  ( 'System Service', 'System Service Tenant - For Administrative purposes or single tenant use.', 1, 0, 'c017cf97-ccbb-4686-98b3-c59efc1a3f45' );
...
CREATE INDEX "I_SecurityTenantUser_isOwner" ON "SecurityTenantUser" ("isOwner")
;

-- Index on the SecurityTenantUser table's canRead field.
CREATE INDEX "I_SecurityTenantUser_canRead" ON "SecurityTenantUser" ("canRead")
;

-- Index on the SecurityTenantUser table's canWrite field.
CREATE INDEX "I_SecurityTenantUser_canWrite" ON "SecurityTenantUser" ("canWrite")
;
...
CREATE TABLE "IpAddressLocation"
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"ipAddress" VARCHAR(50) NOT NULL UNIQUE COLLATE NOCASE,
	"countryCode" VARCHAR(10) NULL COLLATE NOCASE,
	"countryName" VARCHAR(100) NULL COLLATE NOCASE,
...
CREATE INDEX "I_IpAddressLocation_ipAddress" ON "IpAddressLocation" ("ipAddress")
;

-- Index on the IpAddressLocation table's active field.
CREATE INDEX "I_IpAddressLocation_active" ON "IpAddressLocation" ("active")
;

-- Index on the IpAddressLocation table's deleted field.
CREATE INDEX "I_IpAddressLocation_deleted" ON "IpAddressLocation" ("deleted")
;

-- Index on the IpAddressLocation table's id,active,deleted fields.
CREATE INDEX "I_IpAddressLocation_id_active_deleted" ON "IpAddressLocation" ("id", "active", "deleted")
;
...
(
	...
	"securityUserId" INTEGER NULL,		-- Link to user if identified during login attempt
	"ipAddressLocationId" INTEGER NULL,		-- Link to cached geolocation data for this IP.  Populated asynchronously by the IpAddressLocationWorker background service.
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("securityUserId") REFERENCES "SecurityUser"("id"),		-- Foreign key to the SecurityUser table.
	FOREIGN KEY ("ipAddressLocationId") REFERENCES "IpAddressLocation"("id")		-- Foreign key to the IpAddressLocation table.
);
...
CREATE INDEX "I_LoginAttempt_ipAddressLocationId" ON "LoginAttempt" ("ipAddressLocationId")
;
...
(
	"id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
	"securityUserId" INTEGER NOT NULL,		-- Link to the SecurityUser table.
	"objectGuid" VARCHAR(50) NOT NULL COLLATE NOCASE,		-- User's objectGuid for reliable identity resolution
	"tokenId" VARCHAR(250) NULL COLLATE NOCASE,		-- OpenIddict token ID for correlation
	"sessionStart" DATETIME NOT NULL,		-- When the token was issued
	"expiresAt" DATETIME NOT NULL,		-- When the token expires
	"ipAddress" VARCHAR(50) NULL COLLATE NOCASE,		-- Client IP address at login
	"userAgent" VARCHAR(500) NULL COLLATE NOCASE,		-- Browser/client user agent
	"loginMethod" VARCHAR(50) NULL COLLATE NOCASE,		-- Login method: Password, Microsoft, Google, RefreshToken
	"clientApplication" VARCHAR(100) NULL COLLATE NOCASE,		-- Client application name
	...
	"revokedBy" VARCHAR(100) NULL COLLATE NOCASE,		-- Who revoked the session (admin username)
	"revokedReason" VARCHAR(500) NULL COLLATE NOCASE,		-- Reason for revocation
	"active" BIT NOT NULL DEFAULT 1,		-- Active from a business perspective flag.
	"deleted" BIT NOT NULL DEFAULT 0,		-- Soft deletion flag.
	FOREIGN KEY ("securityUserId") REFERENCES "SecurityUser"("id")		-- Foreign key to the SecurityUser table.
);
